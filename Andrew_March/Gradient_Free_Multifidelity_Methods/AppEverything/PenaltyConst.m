...
%
%       Note the parameter "d" is set in this function...documentation is
%       internal.
%
%   Inputs:
...
%
function phi=PenaltyConst(x,sigma,fn,Y2,Coeffs,corr,params,constr,fn_param,fid)
[c,ceq]=constr(x,fn_param{:});
m=evalRBFConst(x,fn,Y2,Coeffs,corr,params,fn_param,fid);
c=c(c>0);
phi=sigma*max(m+1,0)^2; % the parameter d is 1 in this line, and can be
%                         changed
if(~isempty(c))
    phi=phi+sigma*c'*c;
end
