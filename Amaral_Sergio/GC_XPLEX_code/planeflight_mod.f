! $Id: planeflight_mod.f,v 1.2 2009/11/18 07:09:33 daven Exp $
...
!  (7 ) PDATE       (TYPE (XPLEX)   ) : Array of dates     at each flight point
!  (8 ) PTIME       (TYPE (XPLEX)   ) : Array of times     at each flight point
!  (9 ) PTAU        (TYPE (XPLEX)   ) : Array of TAU's     at each flight point
!  (10) PLAT        (TYPE (XPLEX)   ) : Array of latitude  at each flight point
!  (11) PLON        (TYPE (XPLEX)   ) : Array of longitude at each flight point
!  (12) PPRESS      (TYPE (XPLEX)   ) : Array of pressure  at each flight point
...
!  (19) PRRATE      (TYPE (XPLEX)   ) : Array of rxn rates for each entry in PREAC
...
!
      ...
      TYPE (XPLEX),            ALLOCATABLE :: PTAU(:)               

      ! For specifying lat/lon/alt and ID type
      TYPE (XPLEX),            ALLOCATABLE :: PLAT(:)               
      TYPE (XPLEX),            ALLOCATABLE :: PLON(:)               
      TYPE (XPLEX),            ALLOCATABLE :: PPRESS(:)             
...
#     include "comode.h"   ! CSPEC, etc.
      ...
      INTEGER             :: LPLANE
      TYPE (XPLEX)        :: TK, PTAUS, PTAUE, CONSEXP, VPRESH2O, S400nm
      TYPE (XPLEX)              :: VARI(NPVAR)
      TYPE (XPLEX),  PARAMETER  :: MISSING = xplex(-999.99999999d0,0d0)

      TYPE (XPLEX),  PARAMETER  :: TINY = xplex(1.d-36,0d0)   ! arbitary small number to avoid faulty output
...
!  (2 ) VARI (TYPE (XPLEX) ) : Array holding variable values to print to file
!
!  NOTES:
...
!
      ...
      ! Arguments
      INTEGER, INTENT(IN) :: IND
      TYPE (XPLEX),  INTENT(IN) :: VARI(NPVAR)
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) JO1D (TYPE (XPLEX)) : Array w/ JO1D photolysis rates [s-1] from "calcrate.f"
!  (2 ) N2O5 (TYPE (XPLEX)) : Array w/ JO1D photolysis rates [s-1] from "calcrate.f"
!
!  NOTES:
...
#     include "comode.h"  ! RRATE, JLOOPLO, KBLOOP
      ...
      ! Arguments 
      TYPE (XPLEX), INTENT(IN) :: JO1D(KBLOOP)
      TYPE (XPLEX), INTENT(IN) :: N2O5(KBLOOP)
   
      ! Local variables
      INTEGER            :: KLOOP, JLOOP, V, R, I, J, L 
      TYPE (XPLEX)             :: RATE

      ! Smallest, largest TYPE (XPLEX) #'s representable on this machine
      TYPE (XPLEX),PARAMETER::SMALLEST=xplex(TINY(1d0),0d0), 
     &                         LARGEST=xplex(HUGE(1d0),0d0)
