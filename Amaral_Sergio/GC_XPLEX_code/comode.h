! $Id: comode.h,v 1.1.1.1 2009/06/09 21:51:50 daven Exp $
!
!******************************************************************************
...
C
      ...
      ! Avogadro's #
      TYPE (XPLEX), PARAMETER :: AVG      = xplex(6.02252d+23,0d0)

      ! Boltzmann's constant [erg/K]
      TYPE (XPLEX), PARAMETER :: BK       = xplex(1.38054d-16,0d0)
      TYPE (XPLEX), PARAMETER :: BOLTG    = xplex(1.38054d-16,0d0)

      ! Condensation vapor pressure ?????? 
      TYPE (XPLEX), PARAMETER :: CONSVAP=xplex(6.1078d+03/BOLTG%r,0d0)
      
      ! PI (same value as in CMN_GCTM) and related quantities
      TYPE (XPLEX), PARAMETER :: ONEPI=xplex(3.14159265358979323d0,0d0) 
      TYPE (XPLEX), PARAMETER :: EIGHTDPI =xplex(8.d0/ONEPI%r,0d0)

      ! Gas constant [erg/K/mole]
      TYPE (XPLEX), PARAMETER :: RGAS     = xplex(BOLTG%r * AVG%r,0d0)
      
      ! Universal gas constant [g/cm2/s2/mole/K]
      TYPE (XPLEX), PARAMETER :: RSTARG   = xplex(8.31450d+07,0d0)

      ! Seconds per day
      TYPE (XPLEX), PARAMETER :: SCDAY    = xplex(86400.0d0,0d0)
      ...
      TYPE (XPLEX), PARAMETER :: SMAL1    = xplex(1d-06,0d0) 
      TYPE (XPLEX), PARAMETER :: SMAL2    = xplex(1.0d-99,0d0)
      TYPE (XPLEX), PARAMETER :: SMAL3    = xplex(1d-50,0d0)
     ...
     &                   LZOUT
      ...
      TYPE (XPLEX)  ::         TINTERVAL,  CHEMINTV,  TIME, OXYCONS
      TYPE (XPLEX)  ::         HMAXNIT,    FRACDEC
...
C
     ...
     &                   NCSALL,      NCSTRST
      ...
      TYPE (XPLEX)  ::         HMIN,        PLOURB,PLOTROP,TSPMIDC     
      COMMON /DGEAR/     HMIN,        PLOURB,    PLOTROP,   TSPMIDC

      ! /DGEAR2/ needs to be held THREADPRIVATE (hamid, bmy, 7/16/03)
      TYPE (XPLEX)  ::    HMAX,        R1DELT,    DELT,  TIMREMAIN
      TYPE (XPLEX)  ::    XELAPS,      TOLD,      RDELT, XELAPLAST
      TYPE (XPLEX)  ::    RMSERR
     ...
     &                   RMSERR
      ...
      TYPE (XPLEX)  ::   SUMAVGE,     SUMAVHI,   SUMRMSE,   SUMRMHI
      TYPE (XPLEX)  ::  TOTSTEP,     TOTIT,     TELAPS
     ...
     &                   NKHNO4(ICS), NKN2O5, NKHO2
      ...
      ! Added for interannually-varying Methane (bnd, bmy, 7/1/03)
      TYPE (XPLEX)  ::         C3090S,      C0030S,    C0030N,   C3090N
...
C
     ...
     &                  IRMCT( ICP)
      ...
      TYPE (XPLEX)  ::        ABTOL,          ABST2
      TYPE (XPLEX)  ::        ERRMAX,         HMAXUSE,         TIMEINTV
      COMMON /DICS/     ABTOL(6,ICS),   ABST2(ICS),  
     &                  ERRMAX(ICS),    HMAXUSE(ICP),    TIMEINTV(ICS)    

      TYPE (XPLEX)  ::        WTGAS,          GQSUMINI
      TYPE (XPLEX)  ::        BSUMCHEM,       GQSUM,           QBKGAS
      COMMON /DIGAS/    WTGAS(IGAS),    GQSUMINI(IGAS), 
     &                  BSUMCHEM(IGAS), GQSUM(   IGAS),  QBKGAS(IGAS)       

      TYPE (XPLEX)  ::        CPREV,          CMODEL,          APORL
      ...
      TYPE (XPLEX)  ::        ARRT,           BRRT
      TYPE (XPLEX)  ::        FCVT,           FCT1T,           FCT2T
     ...
     2  NKABC(NMTRATE,ICS),  IRORD(NMTRATE,ICS)
      ...
      TYPE (XPLEX) ARR,BRR,FCV,FCTEMP1,FCTEMP2
      COMMON /DNMTRATE/
     ...
     8  NKSURF(  MAXGL4    ), NCOATG(  MAXGL4    ) 
      ...
      TYPE (XPLEX) CNEW,CEST,GLOSS,CHOLD,VDIAG,CBLK,DTLOS,EXPLIC,CONC
      TYPE (XPLEX) RRATE,URATE,TRATE,CORIG
      !***************ADJ_GROUP****************
      TYPE (XPLEX) RRATE_FOR_KPP
      !********************************************
      COMMON /DKBLOOP2/
