! $Id: rpmares_foradj_mod.f,v 1.1.1.1 2009/06/09 21:51:54 daven Exp $
      MODULE RPMARES_FORADJ_MOD
!
!******************************************************************************
!  Module RPMARES_FORADJ_MOD contains the RPMARES_FORADJ routines, which 
!  are essentially the same as those in RPMARES_MOD, plus an extra version
!  for debugging (RPMARES_FORADJ_2) and a driver for simply recalculating 
!  values (RECOMP_RPMARES)
!  (dkh, 9/10/04) 
!
!  Module Variables:
!  ============================================================================
...
!  Module Routines:
!  ============================================================================
!  (1 ) DO_RPMARES_FORADJ : Bridge between GEOS-CHEM and RPMARES_FORADJ
!  (2 ) GET_HNO3        : Gets evolving HNO3; relaxes to monthly mean every 3h
!  (3 ) SET_HNO3        : Saves HNO3 in an array for the next timestep
!  (4 ) RPMARES_FORADJ  : Driver for RPMARES code used during forward run 
!                          of adjoint calculation
...
!  (10) RECOMP_RPMARES  : Recompute RPMARES output rather than checkpoint
!  (11) INIT_RPMARES_FORADJ : Initializes HNO3_sav for adjoint calculation
!  (12) INIT_RPMARES    : Initializes and allocates all module arrays_FORADJ
!  (13) CLEANUP_RPMARES : Deallocates all module arrays
!
...
!  (5 ) checkpt_mod.f   : Module containing routines to save RPMARES data  
! 
!  NOTES:
!  (1 ) The same as RPMARES_MOD except
!             - Added DO_RPMARES_FORADJ and RPMARES_FORADJ
!	      - Added a bit of exit tracking to ACTCOEF
!  (2 ) Added INIT_RPMARES_FORADJ so that gas phase NO3 can be set at the  
!        beginning of each run from within GEOS_CHEM_MOD  (dkh,10/25/04)
!  (3 ) Added RECOMP_RPARES to support L_RECOMP option (dkh, 02/08/05)
!  (4 ) Make GET_HNO3 public so that we can save HNO3 to CHK and OBS arrays
!        from geos_mod.f
!
!******************************************************************************
!
      ...
      ! and routines from being seen outside "rpmares_foradj_mod.f"
      !=================================================================
      ...
      ! Module Routines
      PRIVATE :: SET_HNO3       ! ,  GET_HNO3       (dkh, 03/03/05)
      PRIVATE :: INIT_RPMARES   ! ,  RPMARES_FORADJ (dkh, 02/08/05)
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE DO_RPMARES_FORADJ
!
!******************************************************************************
!  Subroutine DO_RPMARES_FORADJ is the interface between the GEOS-CHEM model
!  and the aerosol thermodynamical equilibrium routine in "rpmares.f"
!  used when doing the forward run of an adjoint calculation
!  (dkh, 9/9/04)
!
!  NOTES
!  (1 ) The same as DO_RPMARES except
!            - Store input to RPMARES in RP_IN
!            - Store output from RPMARES in RP_OUT
!            - Calls RPMARES_FORADJ instead of RPMARES
!            - Reference CHECKPT_MOD, ITS_TIME_FOR_OBS in TIME_MOD,
!               CMN_ADJ and INIT_CHECKPT               
!            - Don't read in HNO3 from monthly averages for LADJ = .TRUE. 
!            - Initialize HNO3_sav from STT_IC
!  (2 )  Do NOT initialize HNO3_sav from STT_IC. DO read in HNO3 from monthly 
!         averages.  The last two changes noted in (1 ) were leading to crashes.
!         (dkh, 02/08/05)
!  (3 ) Remove call to INIT_CHECKPT as we now initialize the checkpt arrays 
!        earlier in the main fwd program. (dkh, 03/16/05)
! 
!******************************************************************************
!
     ...
     &                            ITS_TIME_FOR_OBS
      USE CHECKPT_MOD,     ONLY : RP_IN, RP_OUT, OBS_STT, 
     &                            nitr_max, CHK_STT
      USE TRACERID_MOD,    ONLY : IDTSO4,IDTNH3, IDTNH4,IDTNIT , 
     &                            IDTHNO3 
      USE ADJ_ARAYS_MOD,   ONLY : NOBS, N_CALC, IDADJASO4, 
     &                            IDADJHSO4, IDADJH2O
      USE LOGICAL_ADJ_MOD, ONLY : LADJ, LAERO_THERM
...
#     include "CMN_SIZE"     ! Size parameters
      ...
      IF ( FIRST ) THEN
         ...
         ! IF LADJ = TRUE, 
         ! call INIT_RPMARES_FORADJ from GEOS_MOD so that HNO3_sav can 
         ! be initialized there.   (dkh, 10/25/04)
         IF ( .NOT. LADJ ) CALL INIT_RPMARES
      ...
      !================================================================
      ! BUG FIX   dkh, 02/08/05
      ! Old Code: for doing adjoint with online HNO3 (which doesn't work)
      ! IF ( IDTHNO3 == 0 .AND.  ( .NOT. LADJ ) ) THEN
      ! Original Code: 
      IF ( IDTHNO3 == 0 ) THEN
      !================================================================
      ...
      ! Initialize the iteration counter in each cell to 1
      nitr_max(:,:,:) = 1
...
!$OMP+PRIVATE( ANH4, ANO3, GNH3, GNO3,  ASO4, AHSO4, AH2O, EXIT )
      DO L = 1, LLPAR
      DO J = 1, JJPAR
      DO I = 1, IIPAR
         ...
         ! If the forward run is a perturbed run, then save the input
         ! to RPMARES to the checkpoint array
         IF ( N_CALC .ge. 1 ) THEN
            RP_IN(I,J,L,1)  = SO4
            RP_IN(I,J,L,2)  = GNO3
            RP_IN(I,J,L,3)  = GNH3
     ...
     &                           I,    J,      L,    EXIT )
         ...
         ELSE 
             ! Manually set these to default values so that OBS_STT
             ! doesn't become corrupt with NaNs (dkh, 06/10/05).
             AHSO4 = 0d0
             AH2O  = 0d0 
             ASO4  = SO4
         ENDIF 
         ...
         ! If it is the observation time, save the concentrations of the 
         ! non-tracer species ASO4, AHSO4 and H2O, but only when these 
         ! species are defined (i.e. IDADJxxx > 0 ).
         IF ( ITS_TIME_FOR_OBS( ) .and.
     &         IDADJASO4 > 0 .and.
     &         IDADJHSO4 > 0 .and.
     &         IDADJH2O  > 0       ) THEN

            ! Save to the OBS array during reference calculation
            IF ( N_CALC == 0 ) THEN
               OBS_STT(I,J,L,IDADJASO4) = ASO4
               OBS_STT(I,J,L,IDADJHSO4) = AHSO4
               OBS_STT(I,J,L,IDADJH2O ) = AH2O

            ENDIF
         ENDIF

         ! Save to the CHK_STT array during forward calculation
         ! Since adjoint of rpmares needs this at every chem step, 
         !  we always save these. 
         IF ( N_CALC .GE. 1 ) THEN 
               CHK_STT(I,J,L,IDADJASO4) = ASO4
               CHK_STT(I,J,L,IDADJHSO4) = AHSO4
               CHK_STT(I,J,L,IDADJH2O ) = AH2O
...
!$OMP END PARALLEL DO
      ...
      END SUBROUTINE DO_RPMARES_FORADJ
!------------------------------------------------------------------------------
...
!  (2 ) Now allow for using HNO3_save for offline adjoint simulation. 
!        (dkh, 10/21/04)
!  (3 ) Now allow for rescaling background HNO3 using FD_BKGRND for FDTEST 
!        (dkh, 05/17/05)
!
!******************************************************************************
!
      ...
      USE LOGICAL_ADJ_MD,  ONLY : LFDTEST
...
#     include  "CMN_SIZE"        ! Size parameters 
      ...
      !------------------------------------------------------------
      ! OLD CODE
      ! Reinstated! Go back to standard offline HNO3 for adjoint run.
      IF ( MOD( GET_ELAPSED_MIN(), 180 ) == 0 ) THEN
      ! NEW CODE (dkh, 10/2104) Lead to errors (dkh, 02/08/05)
!      IF (       ( .NOT. LADJ)
!     &     .AND. ( MOD( GET_ELAPSED_MIN(), 180 ) == 0 ) ) THEN
      !------------------------------------------------------------
         HNO3_UGM3 = GET_HNO3_UGM3( I, J, L )
 
         ! Scale the background concentration of HNO3 by FD_BKGRND 
         ! for the finite difference tests.  (dkh, 05/17/05)
         IF ( FDTEST )  THEN
            HNO3_UGM3 = HNO3_UGM3 * FD_BKGRND
         ENDIF
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE RPMARES_FORADJ_2( SO4,  GNO3,  GNH3, RH,   TEMP,
     &                           ASO4, AHSO4, ANO3, AH2O, ANH4, 
     &                           I,    J,     L,    EXIT )    
! NOTE: this code is EXACTLY the same as RPMARES_FORADJ except that includes 
!  print statements regardless.  For debugging, it may only be  called if 
!  I,J,L = IFD,JFD,LFD.
!******************************************************************************
!
! Description:
...
#     include "CMN_SIZE"  ! Size params (needed in CMN_ADJ)
...
! Why did  I add this?  Need it? Take it out? dkh   
      ASO4      = 0.0D0 
      AHSO4     = 0.0D0 
      AH2O      = 0.0D0 
...
!          CALL GEOS_CHEM_STOP
      ...
      print*, ' In RPMARES_FORADJ_2, ratio = ', ratio
      print*, ' In RPMARES_FORADJ_2, tnh4  = ', tnh4
      print*, ' In RPMARES_FORADJ_2, tso4 = ', tso4
...
!         IF ( WH2O .EQ. 0.0d0 ) RETURN
     ...
     &                              'RPMARES_FORADJ' )                                   
      ...
      ! Return to calling program
      END SUBROUTINE RPMARES_FORADJ_2
...
!                          as REAL*8's.  Cleaned up comments.  Also now force
!                          double precision explicitly with "D" exponents.
!  dkh                    Checkpoint variabls for adjoint calculation. 
!******************************************************************************
!
      ! References to F90 modules
      USE ERROR_MOD, ONLY : GEOS_CHEM_STOP, ERROR_STOP
...
#     include "CMN_ADJ"   ! NNNMAX
     ...
     &                              2.0d0 * MWSO4 + 1.0080d0  
      ...
      IF ( RH .LT. 0.01 ) THEN
         ...
         ! dkh add (10/23/07)
         ! Also set ASO4 and AHSO4 to FLOOR.  Otherwise, these
         ! are undefined, which causes problems later if their values
         ! are included in OBS_STT and CHK_STT.  
         ASO4  = FLOOR
         AHSO4 = FLOOR
         EXIT = 1
         RETURN 
...
!
      ...
      !=================================================================
      ! Define the EXIT variable to take the place of all the  
      ! RETURN statements in the original code 
      ! EXIT = 0 means continue
      ! EXIT = 1 means ignore all subsequent statements
      !=================================================================
      INTEGER EXIT
...
#     include "CMN_ADJ"
      ...
      WRITE(6,*) '    - RECOMP_RPMARES'

      ! This should only by run if L_RECOMP = T
      IF ( .NOT. L_RECOMP ) THEN
         CALL ERROR_STOP(
     &        ' Attempted to recomputed when L_RECOMP = F ',
     &        ' RECOMP_RPMARES in checkpt_mod.f ' )
...
!------------------------------------------------------------------------------
      SUBROUTINE INIT_RPMARES_FORADJ
!
!*****************************************************************************
!  Subroutine INIT_RPMARES_FORADJ allocates the array  HNO3_sav 
!   
!
!  NOTES:
!  (1 ) Initialize to 0d0 (as opposed to STT_IC(:,:,:,2).  dkh, 02/08/05
!******************************************************************************
!
      ! F90 modules
...
#     include "CMN_SIZE"
      ...
      !=================================================================
      ! INIT_RPMARES_FORADJ begins here!
      !=================================================================
      ...
      !=================================================================
      ! Go back to using the standard offline HNO3.  (dkh, 02/08/05) 
      ! Old Code:
      !HNO3_sav(:,:,:) = STT_IC(:,:,:,2)
      ! Original Code:
      HNO3_sav(:,:,:) = 0D0
      ...
      ! Return to calling program
      END SUBROUTINE INIT_RPMARES_FORADJ
