!$Id: routines.f,v 1.3 2011/02/23 00:08:48 daven Exp $
c================    L-BFGS-B (version 2.1)   ==========================
     ...
     +                 nbd(n), iwa(3*n), isave(44)
      TYPE (XPLEX) f, factr, pgtol, x(n), l(n), u(n), g(n),
...
c
c     pgtol is a TYPE (XPLEX) variable.
...
c
c     wa is a TYPE (XPLEX) working array of length 
...
c
c     dsave is a TYPE (XPLEX) working array of dimension 29.
c       On exit with 'task' = NEW_X, the following information is
c                                                             available:
...
c       On exit pgtol is unchanged.
c
c     ws, wy, sy, and wt are TYPE (XPLEX) working arrays used to
...
c                                  (2.26) in [3].
c
c     wn is a TYPE (XPLEX) working array of dimension 2m x 2m
...
c                     [ 0  I]
c
c     snd is a TYPE (XPLEX) working array of dimension 2m x 2m
c       used to store the lower triangular part of
c                 N = [Y' ZZ'Y   L_a'+R_z']
c                     [L_a +R_z  S'AA'S   ]
c	     
c     z(n),r(n),d(n),t(n),wa(8*m) are TYPE (XPLEX) working arrays.
c       z is used at different times to store the Cauchy point and
c       the Newton point.
c
c     sg(m),sgo(m),yg(m),ygo(m) are TYPE (XPLEX) working arrays. 
c
...
c     ************
      ...
      character*3      word
      integer          i,k,nnintol,itfile,iback,nskip,
     +                 head,col,iter,itail,iupdat,
     +                 nnint,nfgv,info,ifun,
     +                 iword,nfree,nact,ileave,nenter
      TYPE (XPLEX) theta,fold,ddot,dr,rr,tol,dpmeps,
     +                 xstep,sbgnrm,ddum,dnorm,dtd,epsmch,
     +                 cpu1,cpu2,cachyt,sbtime,lnscht,time1,time2,
     +                 gd,gdold,stp,stpmx,time
      TYPE (XPLEX) one,zero
      parameter        (one=xplex(1.0d0,0d0),zero=xplex(0.0d0,0d0))
...
c        Check the input arguments for errors.
     ...
     +                  iter,nfgv,nnintol,nskip,nact,sbgnrm,
     +                  zero,nnint,word,iback,stp,xstep,k,
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     + 		  wa(1),wa(2*m+1),wa(4*m+1),wa(6*m+1),nnint,
...
c        Print iteration information.
     ...
     +               sbgnrm,nnint,word,iword,iback,stp,xstep)
         goto 1000
      endif
 777  continue
...
c -------------------- the end of the loop -----------------------------
 ...
 999  continue
      ...
      call prn3lb(n,x,f,task,iprint,info,itfile,
     +            iter,nfgv,nnintol,nskip,nact,sbgnrm,
     +            time,nnint,word,iback,stp,xstep,k,
     +            cachyt,sbtime,lnscht)
 1000 continue
...
c     ************
      ...
      integer          nbdd,i
      TYPE (XPLEX) zero
      parameter        (zero=xplex(0.0d0,0d0))
...
c======================= The end of active =============================
      ...
      use complexify
      integer m, col, info
      TYPE (XPLEX) sy(m, m), wt(m, m), v(2*col), p(2*col)
...
c       On exit m is unchanged.
c
c     sy is a TYPE (XPLEX) array of dimension m x m.
c       On entry sy specifies the matrix S'Y.
c       On exit sy is unchanged.
c
c     wt is a TYPE (XPLEX) array of dimension m x m.
...
c
c     v is a TYPE (XPLEX) array of dimension 2col.
c       On entry v specifies vector v.
c       On exit v is unchanged.
c
c     p is a TYPE (XPLEX) array of dimension 2col.
...
c                 =-D^(-1/2)p1+D^(-1)L'p2.  
  ...
  40  continue
      do 60 i = 1, col
         summ = 0.d0
         do 50 k = i + 1, col
            summ = summ + sy(k,i)*p(col+k)/sy(i,i)
  50     continue
         p(i) = p(i) + summ
  60  continue
...
c======================== The end of bmv ===============================
     ...
     +                  v, nnint, sg, yg, iprint, sbgnrm, info, epsmch)
      USE MYTYPE
      use complexify
      integer          n, m, head, col, nnint, iprint, info, 
     +                 nbd(n), iorder(n), iwhere(n)
      TYPE (XPLEX) theta, epsmch,
...
c
c     t is a TYPE (XPLEX) working array of dimension n. 
c       t will be used to store the break points.
c
c     d is a TYPE (XPLEX) array of dimension n used to store
c       the Cauchy direction P(x-tg)-x.
c
c     xcp is a TYPE (XPLEX) array of dimension n used to return the
c       GCP on exit.
c
c     m is an integer variable.
c       On entry m is the maximum number of variable metric corrections 
...
c       On exit m is unchanged.
c
c     ws, wy, sy, and wt are TYPE (XPLEX) arrays.
...
c
c     theta is a TYPE (XPLEX) variable.
...
c       On exit col is unchanged.
c
c     p is a TYPE (XPLEX) working array of dimension 2m.
c       p will be used to store the vector p = W^(T)d.
c
c     c is a TYPE (XPLEX) working array of dimension 2m.
c       c will be used to store the vector c = W^(T)(xcp-x).
c
c     wbp is a TYPE (XPLEX) working array of dimension 2m.
c       wbp will be used to store the row of W corresponding
c         to a breakpoint.
c
c     v is a TYPE (XPLEX) working array of dimension 2m.
c
c     nnint is an integer variable.
c       On exit nnint records the number of quadratic segments explored
c         in searching for the GCP.
c
c     sg and yg are TYPE (XPLEX) arrays of dimension m.
...
c                        summarize the iteration.
c
c     sbgnrm is a TYPE (XPLEX) variable.
...
c     ************
     ...
     +                 ibp,nleft,ibkmin,iter
      TYPE (XPLEX) f1,f2,dt,dtm,tsum,dibp,zibp,dibp2,bkmin,
...
c======================= The end of errclb =============================
     ...
     +                 info, ind(n), indx2(n)
      TYPE (XPLEX) theta, wn(2*m, 2*m), wn1(2*m, 2*m),
...
c
c     wn is a TYPE (XPLEX) array of dimension 2m x 2m.
...
c                     [L_a -R_z           theta*S'AA'S ]
c
c     wn1 is a TYPE (XPLEX) array of dimension 2m x 2m.
...
c       On exit m is unchanged.
c
c     ws, wy, sy, and wtyy are TYPE (XPLEX) arrays;
c     theta is a TYPE (XPLEX) variable;
c     col is an integer variable;
c     head is an integer variable.
...
c     ************
     ...
     +                 col2,pbegin,pend,dbegin,dend,upcl
      TYPE (XPLEX) ddot,temp1,temp2,temp3,temp4
      TYPE (XPLEX) one,zero
      parameter        (one=xplex(1.0d0,0d0),zero=xplex(0.0d0,0d0))
...
c======================= The end of formk ==============================
      ...
      use complexify
      integer          m, col, info
      TYPE (XPLEX) theta, wt(m, m), sy(m, m), ss(m, m)
...
c       On exit n is unchanged.
c
c     t is a TYPE (XPLEX) array of dimension n.
...
c====================== The end of hpsolb ==============================
     ...
     +                 nbd(n), isave(2)
      TYPE (XPLEX) f, fold, gd, gdold, stp, dnorm, dtd, xstep,
...
c     **********
      ...
      integer          i
      TYPE (XPLEX) ddot,a1,a2
      TYPE (XPLEX) one,zero,big
      parameter (one=xplex(1.0d0,0d0),zero=xplex(0.0d0,0d0)
     & ,big=xplex(1.0d+10,0d0))
      TYPE (XPLEX) ftol,gtol,xtol
      parameter (ftol=xplex(1.0d-3,0d0),gtol=xplex(0.9d0,0d0),
     & xtol=xplex(0.1d0,0d0))
...
c======================= The end of lnsrlb =============================
     ...
     +                  iupdat, col, head, theta, rr, dr, stp, dtd)
      ...
      TYPE (XPLEX) theta, rr, dr, stp, dtd, d(n), r(n), 
...
c     ************
 ...
 2001 format ('RUNNING THE L-BFGS-B CODE',/,/,
     ...
     + 'nnint  = number of segments explored during the Cauchy search',/,
 ...
 7001 format ('RUNNING THE L-BFGS-B CODE',/,/,
     + '           * * *',/,/,
     + 'Machine precision =',1p,d10.3)
 9001 format (/,3x,'it',3x,'nf',2x,'nnint',2x,'nact',2x,'sub',2x,'itls',
...
c======================= The end of prn1lb =============================
     ...
     +                  sbgnrm, nnint, word, iword, iback, stp, xstep)
      ...
      USE COMPLEXIFY
      character*3      word
      integer          n, iprint, itfile, iter, nfgv, nact, nnint,
     +                 iword, iback
      TYPE (XPLEX) f, sbgnrm, stp, xstep, x(n), g(n)
...
c                             the truncated Newton step has been used.
     ...
     +          iter,nfgv,nnint,nact,word,iback,stp,xstep,sbgnrm,f
...
c======================= The end of prn2lb =============================
     ...
     +                  cachyt, sbtime, lnscht)
      ...
      character*60     task
      character*3      word
      integer          n, iprint, info, itfile, iter, nfgv, nnintol,
     +                 nskip, nact, nnint, iback, k
      TYPE (XPLEX) f, sbgnrm, time, stp, xstep, cachyt, sbtime,
     +                 lnscht, x(n)
...
c     ************
      ...
      if (iprint .ge. 0) then
         write (6,3003)
         write (6,3004)
         write(6,3005) n,iter,nfgv,nnintol,nskip,nact,sbgnrm,f
     ...
     +	       write (6,*)' l(',k,') > u(',k,').  No feasible solution.'
         ...
         if (iprint .ge. 1) then
            if (info .eq. -4 .or. info .eq. -9) then
               write (itfile,3002)
     +             iter,nfgv,nnint,nact,word,iback,stp,xstep
            endif
 ...
 3003 format (/,
     ...
     + 'Tnnint = total number of segments explored during',
     +           ' Cauchy searches',/,
     ...
     + '           * * *')
 3004 format (/,3x,'N',3x,'Tit',2x,'Tnf',2x,'Tnnint',2x,
...
c======================= The end of prn3lb =============================
      ...
      use complexify
      integer          n, nbd(n)
      TYPE (XPLEX) sbgnrm, x(n), l(n), u(n), g(n)
...
c
c     wv is a TYPE (XPLEX) working array of dimension 2m.
c
c     wn is a TYPE (XPLEX) array of dimension 2m x 2m.
...
c     ************
      ...
      TYPE (XPLEX) alpha,dk,temp1,temp2
      TYPE (XPLEX) one,zero
      parameter        (one=xplex(1.0d0,0d0),zero=xplex(0.0d0,0d0))
...
c====================== The end of subsm ===============================
     ...
     +                  task,isave,dsave)
      ...
      character*(*) task
      integer isave(2)
      TYPE (XPLEX) f,g,stp,ftol,gtol,xtol,stpmin,stpmax
      TYPE (XPLEX) dsave(13)
c     **********
c
c     Subroutine dcsrch
...
c
c	stp is a TYPE (XPLEX) variable. 
...
c         
c       dsave is a TYPE (XPLEX) work array of dimension 13.
c
c     Subprograms called
c
c	MINPACK-2 ... dcstep
...
c
c     **********
      TYPE (XPLEX) zero,p5,p66
      parameter(zero=xplex(0.0d0,0d0),p5=xplex(0.5d0,0d0),
     & p66=xplex(0.66d0,0d0))
      TYPE (XPLEX) xtrapl,xtrapu
      parameter(xtrapl=xplex(1.1d0,0d0),xtrapu=xplex(4.0d0,0d0))

      logical brackt
      integer stage
      TYPE (XPLEX) finit,ftest,fm,fx,fxm,fy,fym,ginit,gtest,
...
c====================== The end of dcsrch ==============================
     ...
     +                  stpmin,stpmax)
      ...
      use complexify
      logical brackt
      TYPE (XPLEX) stx,fx,dx,sty,fy,dy,stp,fp,dp,stpmin,stpmax
c     **********
c
c     Subroutine dcstep
c
...
c     where
c
c       stx is a TYPE (XPLEX) variable.
...
c
c       sty is a TYPE (XPLEX) variable.
...
c
c     **********
      TYPE (XPLEX) zero,p66,two,three
      parameter(zero=xplex(0.0d0,0d0),p66=xplex(0.66d0,0d0),
     & two=xplex(2.0d0,0d0),three=xplex(3.0d0,0d0))
   
      TYPE (XPLEX) gamma,p,q,r,s,sgnd,stpc,stpf,stpq,theta
...
c====================== The end of timer ===============================
      ...
      TYPE (XPLEX) function dnrm2(n,x,incx)
      USE MYTYPE
      use complexify
      integer n,incx
      TYPE (XPLEX) x(n)
...
c
c       TYPE (XPLEX) function dnrm2(n,x,incx)
c
c     where
c
...
c     where
c
c       dpmeps is a TYPE (XPLEX) variable.
...
c     *******
      ...
      TYPE (XPLEX) a,b,beta,betain,betah,temp,tempa,temp1,
     +       zero,one,two
      data zero%r,one%r,two%r /0.0d0,1.0d0,2.0d0/
...
c====================== The end of dcopy ===============================
      ...
      TYPE (XPLEX) function ddot(n,dx,incx,dy,incy)
      USE MYTYPE
      use complexify
c
...
c====================== The end of ddot ================================
      ...
      TYPE (XPLEX) a(lda,1)
c
c     dpofa factors a TYPE (XPLEX) symmetric positive definite
c     matrix.
c
