!$Id: linoz_mod.f,v 1.5 2012/06/14 22:04:10 daven Exp $ 
!------------------------------------------------------------------------------
...
!
      TYPE (XPLEX), ALLOCATABLE :: TPARM(:,:,:,:)
      REAL*8, ALLOCATABLE :: D_TPARM(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: TLSTT(:,:,:)
!
...
!
      ...
      TYPE (XPLEX)  :: CLIMO3,  CLIMPML,    DCO3, DERO3, DERTMP
      TYPE (XPLEX)  :: DERCO3,  DMASS,      DTMP, SSO3
...
!      parameter (je=18) !number of latitudes in look-up table
      ...
      TYPE (XPLEX) STRTX(nlevels_linoz),YSTRT(nlat_linoz)
      TYPE (XPLEX) P0L(llpar+1)
      TYPE (XPLEX) STRT0L(llpar+1),STRT1L(llpar+1),STRT2L(llpar+1)
      TYPE (XPLEX), PARAMETER :: PSF=xplex(1010D0,0d0)
...
!
      ...
      TYPE (XPLEX),  INTENT(IN)  :: STRTX(NLEVELS_LINOZ)
      TYPE (XPLEX),  INTENT(IN)  :: P0L(LLPAR+1)
!
! !OUTPUT PARAMETERS:
!
      TYPE (XPLEX),  INTENT(OUT) :: STRT0L(LLPAR+1)
      TYPE (XPLEX),  INTENT(OUT) :: STRT1L(LLPAR+1)
      TYPE (XPLEX),  INTENT(OUT) :: STRT2L(LLPAR+1)
!
! !REVISION HISTORY: 
!  24 Jun 2003 - B. Field & D. Jones - Further updates for GEOS-Chem
!EOP
...
!
      ...
      TYPE (XPLEX)  P1,P2,F0,F1,F2,PS(NL+1),F(NL)
      TYPE (XPLEX)  XPSD,XPSLM1,XPSL
c-----------------------------------------------------------------------
...
!
      ...
      TYPE (XPLEX),  INTENT(IN)  :: F(NL)
      TYPE (XPLEX),  INTENT(IN)  :: PS(NL+1)
      TYPE (XPLEX),  INTENT(OUT) :: P1
      TYPE (XPLEX),  INTENT(OUT) :: P2
!
! !OUTPUT PARAMETERS:
!
      TYPE (XPLEX),  INTENT(OUT) :: F0
      TYPE (XPLEX),  INTENT(OUT) :: F1
      TYPE (XPLEX),  INTENT(OUT) :: F2
!
! REMARKS:
...
!
      integer  I
      TYPE (XPLEX)   XB,XC,PC,PB,THIRD,sgnf0
      ...
      DO NTBLS = 1,nfields_linoz
         ...
         do M = 1,nmonths_linoz !Month
            do J = 1,nlat_linoz !Latitudes
               READ (IU_FILE,907)  
     &               (D_TPARM(K,J,M,NTBLS),K=nlevels_linoz,1,-1)
               TPARM(:,J,M,NTBLS) = (D_TPARM(:,j,M,NTBLS))
               do K=1,nlevels_linoz
...
c907  FORMAT(20X,6E10.3/(8E10.3))
 912  FORMAT('  Linoz Data:  ',a80,1p,4e10.3)
...
!
      ...
      TYPE (XPLEX),  INTENT(IN)  :: XI(IE) 
      TYPE (XPLEX),  INTENT(IN)  :: XN(ND)
      TYPE (XPLEX),  INTENT(IN)  :: YI(KE,IE)
!
! !OUTPUT PARAMETERS:
!
      TYPE (XPLEX),  INTENT(OUT) :: YN(KE,ND)
...
!
     ...
     &                    NMONTHS_LINOZ, NFIELDS_LINOZ ), STAT=AS )
         ...
         ! Allocate D_TPARM array
         ALLOCATE( D_TPARM( NLEVELS_LINOZ, NLAT_LINOZ,
     &                    NMONTHS_LINOZ, NFIELDS_LINOZ ), STAT=AS )
         IF ( AS /= 0 ) CALL ALLOC_ERR( 'D_TPARM' )
         D_TPARM = 0d0
...
!BOC
      ...
      IF ( ALLOCATED( D_TPARM ) ) DEALLOCATE( D_TPARM )
