...
!                             Declare all REAL variables as TYPE (XPLEX).  Added
...
!
    ...
    TYPE (XPLEX),  INTENT(IN)  :: clat(JM)  ! latitude in radian
!
! !OUTPUT PARAMETERS:
!
...
!                               stratosphere.
!   05 Dec 2008 - R. Yantosca - Updated documentation and added ProTeX headers.
!                               Declare all REAL variables as TYPE (XPLEX).  Also 
...
!
    ...
    !----------------------------------------
    ! Prior to 12/12/08:
    ! Use TYPE (XPLEX) (bmy, 12/12/08)
    !dlon = 2.d0 * PI / float(im)
    !----------------------------------------
    dlon = 2.d0 * PI / XPLX( IM )
...
!
    ...
    TYPE (XPLEX),  INTENT(IN)    :: XMASS(IM,JM,KM)
    TYPE (XPLEX),  INTENT(IN)    :: YMASS(IM,JM,KM)

    ! Grid box surface area for mass flux diag [m2]
    TYPE (XPLEX),  INTENT(IN)    :: AREA_M2(JM)        

    ! Tracer masses for flux diag
    TYPE (XPLEX),  INTENT(IN)    :: TCVV(NQ)              
...
!
    ...
    TYPE (XPLEX),  INTENT(INOUT) :: v(IM, JFIRST-MG:JLAST+MG, KM) 

    ! surface pressure at current time
    TYPE (XPLEX),  INTENT(INOUT) :: ps1(IM, JFIRST:JLAST)  

    ! surface pressure at future time=t+dt
    TYPE (XPLEX),  INTENT(INOUT) :: ps2(IM, JFIRST:JLAST)  

    ! Tracer "mixing ratios" [v/v]
    TYPE (XPLEX),  INTENT(INOUT) :: q(IM, JFIRST-NG:JLAST+NG, KM, NQ)  

    ! E/W, N/S, and up/down diagnostic mass fluxes
!--- Previous to (ccc, 12/3/09)
!    TYPE (XPLEX),  INTENT(INOUT) :: MASSFLEW(IM,JM,KM,NQ)  ! for ND24 diagnostic
!    TYPE (XPLEX),  INTENT(INOUT) :: MASSFLNS(IM,JM,KM,NQ)  ! for ND25 diagnostic
!    TYPE (XPLEX),  INTENT(INOUT) :: MASSFLUP(IM,JM,KM,NQ)  ! for ND26 diagnostic 
    TYPE (XPLEX),  INTENT(INOUT) :: MASSFLEW(:,:,:,:)  ! for ND24 diagnostic
    TYPE (XPLEX),  INTENT(INOUT) :: MASSFLNS(:,:,:,:)  ! for ND25 diagnostic
    TYPE (XPLEX),  INTENT(INOUT) :: MASSFLUP(:,:,:,:)  ! for ND26 diagnostic 
...
!
    ...
    TYPE (XPLEX)             :: dap   (km)
    TYPE (XPLEX)             :: dbk   (km)
    TYPE (XPLEX)             :: cx(im,jfirst-ng:jlast+ng,km)  ! E-W CFL # on C-grid
    TYPE (XPLEX)             :: cy(im,jfirst:jlast+mg,km)     ! N-S CFL # on C-grid
    TYPE (XPLEX)             :: delp1(im, jm, km)  
    TYPE (XPLEX)             :: delp2(im, jm, km)  
    TYPE (XPLEX)             :: delpm(im, jm, km)
    TYPE (XPLEX)             :: pu   (im, jm, km)
    TYPE (XPLEX)             :: dpi(im, jm, km)
    TYPE (XPLEX)             :: geofac  (jm)     ! geometrical factor for meridional
                                           ! advection; geofac uses correct 
                                           ! spherical geometry, and replaces 
                                           ! RGW_25. (ccc, 4/1/09)
    TYPE (XPLEX)             :: geofac_pc        ! geometrical gactor for poles.
    TYPE (XPLEX)             :: dp
    TYPE (XPLEX)             :: dps_ctm(im,jm)
    TYPE (XPLEX)             :: ua (im, jm, km)
    TYPE (XPLEX)             :: va (im, jm, km)
    TYPE (XPLEX)             :: wz(im, jm, km)
    TYPE (XPLEX)             :: dq1(im,jfirst-ng:jlast+ng,km)
    
    ! qqu, qqv, adx and ady are now 2d arrays for parallelization purposes.
    !(ccc, 4/1/08)  
    TYPE (XPLEX)             :: qqu(im, jm)
    TYPE (XPLEX)             :: qqv(im, jm)
    TYPE (XPLEX)             :: adx(im, jm)
    TYPE (XPLEX)             :: ady(im, jm)

    ! fx, fy, fz and qtemp are now 4D arrays for parallelization purposes.
    ! (ccc, 4/1/09) 
    TYPE (XPLEX)             :: fx (im, jm, km, nq)
    TYPE (XPLEX)             :: fy (im, jm+1, km, nq)           ! one more for edges
    TYPE (XPLEX)             :: fz  (im, jm, km, nq)
    TYPE (XPLEX)             :: qtemp (im, jm, km, nq)
    TYPE (XPLEX)             :: DTC(IM,JM,KM)               ! up/down flux temp array
...
!$OMP END PARALLEL DO
    ...
    dps_ctm(:,:)=xplx(Sum(dpi(:,:,:)%r,3),Sum(dpi(:,:,:)%i,3))
  ...
  ! ==============
    ...
    ps = ak(1)+xplx(sum(delp2%r,3),sum(delp2%i,3))
...
!   -----------------------------------------------------------------
    ...
    TYPE (XPLEX)  :: delp1n(i1:i2, j2-1:j2)
    TYPE (XPLEX)  :: delp1s(i1:i2,  ju1:ju1+1)
...
!
    ...
    TYPE (XPLEX),  INTENT(IN) :: crx(ILO:IHI, JULO:JHI)

    ! Courant number in N-S direction
    TYPE (XPLEX),  INTENT(IN) :: cry(ILO:IHI, JULO:JHI)
...
!
    ...
    TYPE (XPLEX), INTENT(OUT) :: ua(ILO:IHI, JULO:JHI)

    ! Average of Courant numbers from ij and ij+1
    TYPE (XPLEX), INTENT(OUT) :: va(ILO:IHI, JULO:JHI)
...
!
    ...
    ! Courant number in E-W direction
    TYPE (XPLEX),  INTENT(IN)  :: crx(ILO:IHI, JULO:JHI, K1:K2)
!
! !OUTPUT PARAMETERS:
!
...
!
    ...
    TYPE (XPLEX),  INTENT(IN)  :: qq1(ILO:IHI, JULO:JHI)

    ! Average of Courant numbers from il and il+1
    TYPE (XPLEX),  INTENT(IN)  :: ua (ILO:IHI, JULO:JHI)

    ! Average of Courant numbers from ij and ij+1
    TYPE (XPLEX),  INTENT(IN)  :: va (ILO:IHI, JULO:JHI)
...
!
    ...
    TYPE (XPLEX),  INTENT(OUT) :: qqu(ILO:IHI, JULO:JHI)

    ! concentration contribution from N-S advection [mixing ratio]
    TYPE (XPLEX),  INTENT(OUT) :: qqv(ILO:IHI, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO) 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!    
    ...
    INTEGER :: jv, iuw, iue
    TYPE (XPLEX)  :: ril, rij, riu
    TYPE (XPLEX)  :: ru
    TYPE (XPLEX)  :: qtmp(-i2/3:i2+i2/3, julo:jhi)
...
!
    ! Species density [hPa]
    TYPE (XPLEX),  INTENT(INOUT) :: dq1(ILO:IHI, JULO:JHI, K1:K2)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO) 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ...
    TYPE (XPLEX),  INTENT(IN)  :: pres1(ILO:IHI, JULO:JHI)
    
    ! Surface pressure at t1+tdt [hPa]
    TYPE (XPLEX),  INTENT(IN)  :: pres2(ILO:IHI, JULO:JHI)
!
! !OUTPUT PARAMETERS:
!
    ! Pressure thickness, the pseudo-density in a 
    ! hydrostatic system at t1 [hPa]
    TYPE (XPLEX), INTENT(OUT) :: delp1(ILO:IHI, JULO:JHI)

    ! Pressure thickness, the pseudo-density in a 
    ! hydrostatic system at t1+tdt/2 (approximate) [hPa]
    TYPE (XPLEX), INTENT(OUT) :: delpm(ILO:IHI, JULO:JHI)

    ! Pressure at edges in "u" [hPa]
    TYPE (XPLEX), INTENT(OUT) :: pu(ILO:IHI, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO) 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ...
    TYPE (XPLEX),  INTENT(IN)  :: cose (JU1_GL:J2_GL)

    ! Pressure thickness, the pseudo-density in a hydrostatic system
    ! at t1+tdt/2 (approximate) (mb)
    TYPE (XPLEX),  INTENT(IN)  :: delpm(ILO:IHI, JULO:JHI)

    ! pressure at edges in "u"  (mb)
    TYPE (XPLEX),  INTENT(IN)  :: pu   (iLO:IHI, JULO:JHI)

    ! horizontal mass flux in E-W and N-S directions [hPa]
    TYPE (XPLEX),  INTENT(IN)  :: xmass(ILO:IHI, JULO:JHI)
    TYPE (XPLEX),  INTENT(IN)  :: ymass(ILO:IHI, JULO:JHI)
!
! !OUTPUT PARAMETERS:
!
    ! Courant numbers in E-W and N-S directions
    TYPE (XPLEX),  INTENT(OUT) :: crx(ILO:IHI, JULO:JHI)
    TYPE (XPLEX),  INTENT(OUT) :: cry(ILO:IHI, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO) 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ...
    TYPE (XPLEX) , INTENT(IN)  :: geofac_pc
    ...
    ! factor in TPCORE
    TYPE (XPLEX) , INTENT(IN)  :: geofac(JU1_GL:J2_GL)
...
!
    ...
    TYPE (XPLEX),  INTENT(OUT) :: dpi(I1:I2, JU1:J2)
!      
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ...
    ! Concentration contribution from N-S advection [mixing ratio]
    TYPE (XPLEX),  INTENT(IN)  :: qqv(ILO:IHI, JULO:JHI)
...
!
    ...
    TYPE (XPLEX),  INTENT(OUT) :: adx(ILO:IHI, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ...
    ! Concentration contribution from E-W advection [mixing ratio]
    TYPE (XPLEX),  INTENT(IN)  :: qqu(ILO:IHI, JULO:JHI)
...
!
    ...
    TYPE (XPLEX),  INTENT(OUT) :: ady(ILO:IHI, JULO:JHI)
...
! 
    ...
    TYPE (XPLEX)  :: qquwk(ilo:ihi, julo-2:jhi+2)   
...
!
    ...
    TYPE (XPLEX),  INTENT(OUT) :: qquwk(ILO:IHI, JULO-2:JHI+2)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ! Cross term due to N-S advection (mixing ratio)
    TYPE (XPLEX),  INTENT(INOUT) :: ady(ILO:IHI, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ! Species density [hPa]
    TYPE (XPLEX),  INTENT(INOUT) :: dq1(ILO:IHI, JULO:JHI)

    ! Concentration contribution from N-S advection [mixing ratio]
    TYPE (XPLEX),  INTENT(INOUT) :: qqv(ILO:IHI, JULO:JHI)
!
! !OUTPUT PARAMETERS:
!
    ! E-W flux [mixing ratio]
    TYPE (XPLEX),  INTENT(OUT)   :: fx(ILO:IHI, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
! 
    ...
    ! Arrays
    INTEGER :: isav(i1:i2)
    TYPE (XPLEX)  :: dcx(-i2/3:i2+i2/3, julo:jhi)
    TYPE (XPLEX)  :: qtmp(-i2/3:i2+i2/3, julo:jhi)
...
!
    ...
    ! Concentration contribution from N-S advection [mixing ratio]
    TYPE (XPLEX),  INTENT(IN)  :: qqv(-I2/3:I2+I2/3, JULO:JHI)
!
! !OUTPUT PARAMETERS:
!
    ! Slope of concentration distribution in E-W direction [mixing ratio]
    TYPE (XPLEX),  INTENT(OUT) :: dcx(-I2/3:I2+I2/3, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ...
    ! Courant number in E-W direction
    TYPE (XPLEX),  INTENT(IN)    :: crx(I1:I2, JULO:JHI)
!
! !INPUT/OUTPUT PARAMETERS: 
!
      ! Concentration contribution from N-S advection [mixing ratio]
...
!
    ! Slope of concentration distribution in E-W direction (mixing ratio)
    TYPE (XPLEX),  INTENT(OUT)   :: dcx(ILO:IHI, JULO:JHI)

    ! E-W flux [mixing ratio]
    TYPE (XPLEX),  INTENT(OUT)   :: fx(I1:I2, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
! 
    ...
    !
    !! Arrays
    !TYPE (XPLEX), ALLOCATABLE, SAVE :: a6(:)
    !TYPE (XPLEX), ALLOCATABLE, SAVE :: al(:)
    !TYPE (XPLEX), ALLOCATABLE, SAVE :: ar(:)
    !TYPE (XPLEX), ALLOCATABLE, SAVE :: a61(:)
    !TYPE (XPLEX), ALLOCATABLE, SAVE :: al1(:)
    !TYPE (XPLEX), ALLOCATABLE, SAVE :: ar1(:)
    !TYPE (XPLEX), ALLOCATABLE, SAVE :: dcxi1(:)
    !TYPE (XPLEX), ALLOCATABLE, SAVE :: qqvi1(:)
    !------------------------------------------------------------------------

    ! Arrays
    TYPE (XPLEX)                    :: a6( ILO:IHI )
    TYPE (XPLEX)                    :: al( ILO:IHI )
    TYPE (XPLEX)                    :: ar( ILO:IHI )
    TYPE (XPLEX)                    :: a61(   (IHI-1) - (ILO+1) + 1 )
    TYPE (XPLEX)                    :: al1(   (IHI-1) - (ILO+1) + 1 )
    TYPE (XPLEX)                    :: ar1(   (IHI-1) - (ILO+1) + 1 )
    TYPE (XPLEX)                    :: dcxi1( (IHI-1) - (ILO+1) + 1 )
    TYPE (XPLEX)                    :: qqvi1( (IHI-1) - (ILO+1) + 1 )
...
!
    ...
    TYPE (XPLEX),  INTENT(INOUT) :: a6(lenx)

    ! Left edge value of the test parabola
    TYPE (XPLEX),  INTENT(INOUT) :: al(lenx)

    ! Right edge value of the test parabola
    TYPE (XPLEX),  INTENT(INOUT) :: ar(lenx)

    ! 0.5 * mismatch
    TYPE (XPLEX),  INTENT(INOUT) :: dc(lenx)

    ! Cell-averaged value
    TYPE (XPLEX),  INTENT(INOUT) :: qa(lenx)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ...
    TYPE (XPLEX),  INTENT(OUT)   :: fy(ILO:IHI, JULO:JHI+1)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
! 
    ...
    ! Arrays
    TYPE (XPLEX)  :: dcy(ilo:ihi, julo:jhi)
...
!
    ...
    TYPE (XPLEX),  INTENT(OUT) :: dcy(ILO:IHI, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
! 
    ...
    ! pole functions. (ccc)
    TYPE (XPLEX) :: qtmp(ilo:ihi, julo-2:jhi+2)
...
!
    ...
    ! Concentration contribution from E-W advection [mixing ratio]
    TYPE (XPLEX),  INTENT(IN)  :: qqu(ILO:IHI, JULO-2:JHI+2)
!
! !OUTPUT PARAMETERS:
!
    ! Slope of concentration distribution in N-S direction [mixing ratio]
...
!
    ...
    ! Slope of concentration distribution in N-S direction [mixing ratio]
    TYPE (XPLEX),  INTENT(IN)  :: dcy(ILO:IHI, JULO:JHI)
...
! 
    ...
    ! Arrays
    TYPE (XPLEX)  :: a61 (ilong*((JHI-1)-(JULO+1)+1))
    TYPE (XPLEX)  :: al1 (ilong*((JHI-1)-(JULO+1)+1))
    TYPE (XPLEX)  :: ar1 (ilong*((JHI-1)-(JULO+1)+1))
    TYPE (XPLEX)  :: dcy1(ilong*((JHI-1)-(JULO+1)+1))
    TYPE (XPLEX)  :: qqu1(ilong*((JHI-1)-(JULO+1)+1))
    TYPE (XPLEX)  :: a6(ILO:IHI, JULO:JHI)
    TYPE (XPLEX)  :: al(ILO:IHI, JULO:JHI)
    TYPE (XPLEX)  :: ar(ILO:IHI, JULO:JHI)
...
!
    ...
    TYPE (XPLEX),  INTENT(INOUT) :: al(ILO:IHI, JULO:JHI)
    TYPE (XPLEX),  INTENT(INOUT) :: ar(ILO:IHI, JULO:JHI)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
!
    ...
    TYPE (XPLEX),  INTENT(INOUT) :: fy (ILO:IHI, JULO:JHI+1)
!
! !AUTHOR:
!   Original code from Shian-Jiann Lin, DAO 
!   John Tannahill, LLNL (jrt@llnl.gov)
...
! 
    ...
    TYPE (XPLEX)  :: dq_np
    TYPE (XPLEX)  :: dq_sp
    TYPE (XPLEX)  :: dqik(2)  ! 2 elements array for each pole value.
    TYPE (XPLEX)  :: sumnp
    TYPE (XPLEX)  :: sumsp
...
!
    ...
    ! Pressure thickness, the pseudo-density in a 
    ! hydrostatic system at t1 [hPa]
    TYPE (XPLEX),  INTENT(IN)    :: delp1(ILO:IHI, JULO:JHI, K1:K2)

    ! Large scale mass flux (per time step tdt) in the vertical
    ! direction as diagnosed from the hydrostatic relationship [hPa]
    TYPE (XPLEX),  INTENT(IN)    :: wz(I1:I2, JU1:J2, K1:K2)

    ! Species concentration [mixing ratio]
    TYPE (XPLEX),  INTENT(IN)    :: qq1(ILO:IHI, JULO:JHI, K1:K2)
!
! !INPUT/OUTPUT PARAMETERS: 
!
    ! Species density [hPa]
...
!
    ...
    TYPE (XPLEX),  INTENT(OUT)   :: fz(ILO:IHI, JULO:JHI,  K1:K2)
...
! 
    ...
    ! Arrays
    TYPE (XPLEX)  :: a61  (ilong*(ivert-4))
    TYPE (XPLEX)  :: al1  (ilong*(ivert-4))
    TYPE (XPLEX)  :: ar1  (ilong*(ivert-4))
    TYPE (XPLEX)  :: dca1 (ilong*(ivert-4))
    TYPE (XPLEX)  :: qq1a1(ilong*(ivert-4))
    TYPE (XPLEX)  :: a6   (i1:i2, k1:k2)
    TYPE (XPLEX)  :: al   (i1:i2, k1:k2)
    TYPE (XPLEX)  :: ar   (i1:i2, k1:k2)
    TYPE (XPLEX)  :: dca  (i1:i2, k1:k2)
    TYPE (XPLEX)  :: dlp1a(i1:i2, k1:k2)
    TYPE (XPLEX)  :: qq1a (i1:i2, k1:k2)
    TYPE (XPLEX)  :: wza  (i1:i2, k1:k2)
    TYPE (XPLEX)  :: dc   (i1:i2, ju1:j2, k1:k2)
    ! Work array 
    TYPE (XPLEX)  :: dpi(I1:I2, JU1:J2, K1:K2)
...
!
    ...
    TYPE (XPLEX),  INTENT(IN)   :: AREA_1D(JU1:J2)
!
! !INPUT/OUTPUT PARAMETERS: 
!
    ! Surface pressure [hPa]
    TYPE (XPLEX), INTENT(INOUT) :: press(ILO:IHI, JULO:JHI)
!
! !AUTHOR:
...
! 
    ...
    SUM_AREA = SUM( rel_area( JU1:JU1+1 ) ) * XPLX( I2 )
    ...
    SUM_AREA = SUM( rel_area( J2-1:J2 ) ) * XPLX( I2 )
