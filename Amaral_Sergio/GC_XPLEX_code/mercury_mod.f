! $Id: mercury_mod.f,v 1.1.1.1 2009/06/09 21:51:53 daven Exp $
      MODULE MERCURY_MOD
!
!******************************************************************************
...
!  (4 ) COSZM    (TYPE (XPLEX) ) : Max daily value of COS( S. Z. Angle ) [unitless]
!  (5 ) DRYHg2   (INTEGER) : Index for Hg2 in DEPSAV array (drydep freq)
!  (6 ) DRYHgP   (INTEGER) : Index for HgP in DEPSAV array (drydep freq)
!  (7 ) EHg0_an  (TYPE (XPLEX) ) : Anthropogenic Hg0 emissions [kg/s]
!  (8 ) EHg2_an  (TYPE (XPLEX) ) : Anthropogenic Hg2 emissions [kg/s]
!  (9 ) EHgP_an  (TYPE (XPLEX) ) : Anthropogenic HgP emissions [kg/s]
!  (10) EHg0_oc  (TYPE (XPLEX) ) : Hg0 emissions from oceans [kg/s]
!  (11) EHg0_ln  (TYPE (XPLEX) ) : Re-emissions of Hg0 from land [kg/s]
!  (12) EHg0_nt  (TYPE (XPLEX) ) : Hg0 emissions from natural land sources [kg/s] 
!  (13) TCOSZ    (TYPE (XPLEX) ) : Sum of COS( Solar Zenith Angle ) [unitless]
!  (14) TTDAY    (TYPE (XPLEX) ) : Total daylight time at location (I,J) [minutes]
!  (15) T44      (TYPE (XPLEX) ) : Local array for drydep diagnostic
!  (15) N_HgTAGS (INTEGER) : Number of tagged sources (1 or 8)
!  (16) ZERO_DVEL(TYPE (XPLEX) ) : Array with zero dry deposition velocity
...
!
      ...
      ! Parameters
      TYPE (XPLEX),  PARAMETER   :: SMALLNUM = xplex(1d-32,0d0)
      ...
      TYPE (XPLEX),  ALLOCATABLE :: EHg0_an(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: EHg2_an(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: EHgP_an(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: EHg0_oc(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: EHg0_ln(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: EHg0_nt(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: T44(:,:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: TCOSZ(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: TTDAY(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: ZERO_DVEL(:,:)
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) V_DEP_Hg2 (TYPE (XPLEX)) : Dry deposition velocity for Hg(II) [cm/s]
!
...
#     include "CMN_SIZE"     ! Size parameters
      ...
      ! Arguments
      TYPE (XPLEX), INTENT(IN)    :: V_DEP_Hg2(IIPAR,JJPAR)

      ! Local variables
      INTEGER               :: I, J, L, N, NN
      TYPE (XPLEX)                :: AREA_CM2,    C_O3,        C_OH
      TYPE (XPLEX)                :: DRYDEP,      DTCHEM,      E_RKT
      TYPE (XPLEX)                :: E_K1,        E_K2,        FA
      TYPE (XPLEX)                :: FC,          K1,          K2          
      TYPE (XPLEX)                :: Ko,          Kr,          Kt
      TYPE (XPLEX)                :: Kr_Kt,       NK1,         NK2
      TYPE (XPLEX)                :: LOST_Hg0_O3, LOST_Hg0_OH, LWC
      TYPE (XPLEX)                :: F_UNDER_TOP, RKT,         E_SALT
      TYPE (XPLEX)                :: LOST_Hg0(N_Hg_CATS)
      TYPE (XPLEX)                :: LOST_SS(N_Hg_CATS)
      TYPE (XPLEX)                :: Ox_Hg0(N_Hg_CATS)

      ! K for reaction Hg0 + O3 in cm3 molec-1 s-1 (Source: Hall '95)
      TYPE (XPLEX), PARAMETER     :: K       = xplex(3.0d-20,0d0)

      ! K for reaction Hg2 + OH in cm3 s-1 
      TYPE (XPLEX), PARAMETER     :: K_HG_OH = xplex(8.7d-14,0d0)

      ! Henry's Law constant for Hg2
      TYPE (XPLEX), PARAMETER     :: HL      = xplex(1.4d6,0d0)

      ! Gas constant??
      TYPE (XPLEX), PARAMETER     :: R       = xplex(8.2d-2,0d0)

      ! K for reduction (scaled to budget and OH conc)
      TYPE (XPLEX), PARAMETER     :: Kred    = xplex(8.4d-10,0d0)
     
      ! K for sea salt (eck, bmy, 4/6/06)
      TYPE (XPLEX), PARAMETER     :: KSalt   = xplex(3.8d-5,0d0)
...
!  (6  ) RKT      (TYPE (XPLEX) ) : Value of R * k * Temp for this rxn [unitless]
!  (7  ) E_RKT    (TYPE (XPLEX) ) : Value of EXP( - RKT ) [unitless]
!  (8  ) Kr_Kt    (TYPE (XPLEX) ) : Ratio of Kr / Kt (redux K / total K) [unitless]
!
!  Arguments as Output:
!  ============================================================================
!  (9  ) LOST_Hg0 (TYPE (XPLEX) ) : Loss term: Hg(0) before - Hg(0) after [kg]
!
!  NOTES:
...
!
      ...
      TYPE (XPLEX),  INTENT(IN)  :: RKT,      E_RKT, Kr_Kt, Kt,    Kr  
      TYPE (XPLEX),  INTENT(IN)  :: Ko,       DTCHEM
      TYPE (XPLEX),  INTENT(OUT) :: LOST_Hg0, Ox_Hg0

      ! Local variables
      TYPE (XPLEX)               :: OLD_Hg0, OLD_Hg2, NEW_Hg0
...
!  (5  ) E_K1     (TYPE (XPLEX) ) : Value of EXP( -K1 ) [unitless]
!  (6  ) E_K2     (TYPE (XPLEX) ) : Value of EXP( -K2 ) [unitless]
!
!  Arguments as Output:
!  ============================================================================
!  (7  ) LOST_Hg0 (TYPE (XPLEX) ) : Loss term: Hg(0) before - Hg(0) after [kg]
!
!  NOTES:
!******************************************************************************
!
      ...
      TYPE (XPLEX),  INTENT(IN)  :: E_K1, E_K2
      TYPE (XPLEX),  INTENT(OUT) :: LOST_Hg0

      ! Local variables
      TYPE (XPLEX)               :: OLD_Hg0, NEW_Hg0
...
!  (5  ) RKT      (TYPE (XPLEX) ) : Value of R * k * Temp for drydep [unitless]
!  (6  ) E_RKT    (TYPE (XPLEX) ) : Value of EXP( - RKT ) [unitless]
!  (7  ) E_SALT   (TYPE (XPLEX) ) : Value of EXP( -KSALT * DTCHEM ) 
!  (8  ) LOST_Hg0 (TYPE (XPLEX) ) : Amount of Hg(0) that became Hg(II) [kg]
!  (9  ) DTCHEM   (INTEGER) : Chemistry timestep [s]
!
!  Arguments as Output:
!  ============================================================================
!  (10 ) LOST_SS  (TYPE (XPLEX) ) : Hg2 lost to sea salt rxn [kg]
!
!  NOTES:
...
#     include "CMN_DIAG"          ! ND44
      ...
      ! Arguments
      INTEGER, INTENT(IN)        :: I,   J,     L,      N
      TYPE (XPLEX),  INTENT(IN) :: RKT, E_RKT, E_SALT, LOST_Hg0, DTCHEM
      TYPE (XPLEX),  INTENT(OUT)       :: LOST_SS

      ! Local variables
      INTEGER                    :: NN
      TYPE (XPLEX)                     :: AREA_CM2, DRYDEP,  OLD_Hg2
      TYPE (XPLEX)                     :: NEW_Hg2,  TEMP_Hg2
...
!  (1-3) I, J, L  (INTEGER) : GEOS-CHEM lon, lat, alt grid box indices
!  (4  ) N        (INTEGER) : Index for Hg(II) total or tagged tracers
!  (5  ) LOST_Hg0 (TYPE (XPLEX) ) : Amount of Hg(0) that has become Hg(II) [kg]
!
!  NOTES:
!******************************************************************************
!
      ...
      ! Arguments
      INTEGER, INTENT(IN)  :: I, J, L, N
      TYPE (XPLEX),  INTENT(IN)  :: LOST_Hg0

      ! Local variables
      TYPE (XPLEX)               :: OLD_Hg2, NEW_Hg2
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) V_DEP_HgP (TYPE (XPLEX)) : Dry deposition velocity for Hg(II) [cm/s]
!     
!  NOTES:
...
#     include "CMN_SIZE"  ! Size parameters
      ...
      ! Arguments
      TYPE (XPLEX), INTENT(IN) :: V_DEP_HgP(IIPAR,JJPAR)

      ! Local variables
      INTEGER            :: I,      J,    L,          NN,  PBL_MAX
      TYPE (XPLEX)             :: DTCHEM, E_KT, F_UNDER_TOP
...
!  (5  ) E_KT     (TYPE (XPLEX) ) : Value of EXP( -KT ) for drydep rxn [unitless]
...
#     include "CMN_DIAG"   ! ND44 
      ...
      ! Arguments
      INTEGER, INTENT(IN) :: I, J, L, N
      TYPE (XPLEX),  INTENT(IN) :: DTCHEM, E_KT

      ! Local variables
      INTEGER             :: NN
      TYPE (XPLEX)              :: AREA_CM2, DRYDEP, OLD_HgP, NEW_HgP
...
!  ============================================================================
!  (1 ) TC (TYPE (XPLEX)) : Tracer concentration of Hg(0) [kg]
!
!  NOTES:
...
#     include "CMN_SIZE"     ! Size parameters
      ...
      TYPE (XPLEX)            :: DTSRCE, E_Hg, F_OF_PBL, T_Hg, T_Hg_An
...
!  Arguments as Input/Output:
!  ============================================================================
!  (1 ) TC (TYPE (XPLEX)) : Tracer concentration of Hg(II) [kg]
!
!  NOTES:
!  (1 ) Now use diagnostic arrays from "diag03_mod.f" (bmy, 1/21/05)
!  (2 ) Now references GET_FRAC_OF_PBL and GET_PBL_MAX_L from "pbl_mix_mod.f".
...
#     include "CMN_SIZE"     ! Size parameters
      ...
      ! Local variables
      INTEGER               :: I,      J,        L,    N,   PBL_MAX
      TYPE (XPLEX)                :: DTSRCE, F_OF_PBL, E_Hg 
...
#     include "CMN_SIZE"       ! Size parameters
      ...
      TYPE (XPLEX)                  :: XTAU
      TYPE (XPLEX), PARAMETER::SEC_PER_YR =xplex(365.25d0 * 86400d0,0d0)
...
!
      ...
      TYPE (XPLEX),PARAMETER::ZRT=xplex(0.60d0,0d0),
     &                        ZRS=xplex(0.99d0,0d0)
      ...
      R0   = ZRT + ( ZRS - ZRT ) * EXP( 1d0 - ( PSFC / PRES )**2.5d0 )
      B0   = ( RH2 - R0 ) / ( 1d0 - R0 )
...
#     include "CMN_GCTM"  ! Physical constants
      ...
      B2 = 0.000907d0
      B3 = 0.000148d0
      R  = 2.* PI *xplx( GET_DAY_OF_YEAR() - 1 ) / 365.d0

      DEC = A0 - A1*cos(  R) + B1*sin(  R)
     &         - A2*cos(2*R) + B2*sin(2*R)
     &         - A3*cos(3*R) + B3*sin(3*R)

      LHR0 = int(xplx( GET_NHMSb() )/10000.d0)

      ! Only do the following at the start of a new day
      IF ( FIRST .or. GET_GMT() < 1d-5 ) THEN 
...
!$OMP+PRIVATE( I, J, YMID_R, IJLOOP, TIMLOC, AHR )
            ...
            DO I = 1, IIPAR
               ...
               ! Increment IJLOOP
               IJLOOP = ( (J-1) * IIPAR ) + I
               TIMLOC =(LHR0)+xplx(NT)/3600.d0+GET_XMID(I)/15.d0
               ...
               AHR = abs(TIMLOC - 12.d0) * 15.d0 * PI_180
