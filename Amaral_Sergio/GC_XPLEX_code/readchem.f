! $Id: readchem.f,v 1.1.1.1 2009/06/09 21:51:53 daven Exp $
      SUBROUTINE READCHEM 
!
!******************************************************************************
...
!        NKSPECG for DMS+OH+O2 rxn.  Now also force TYPE (XPLEX) with
...
C
      ...
      TYPE (XPLEX) C1,CSTRAT,CTROPL,CTROPS,CURBAN,QTHERMG
      REAL*8 D_C1,D_CSTRAT,D_CTROPL,D_CTROPS, D_CURBAN,D_ARRT(MXCOF),
     & D_BRRT(MXCOF), D_FCVT(MXCOF), D_FCT1T(MXCOF), D_FCT2T(MXCOF),
     & D_PINP(20), D_ASTKCF, D_RUARSL, D_RH100
...
C
 ...
 10   READ(KGLC,11) (XINP(I), I=1,3),D_C1,D_CSTRAT,D_CTROPL,D_CTROPS,
     &                               D_CURBAN
 11   FORMAT(A1,1X,A14,A2,1X,0PF6.2,4(1PE10.3))
      C1 = xplx(D_C1)
      CSTRAT = xplx(D_CSTRAT)
      CTROPL = xplx(D_CTROPL)
      CTROPS = xplx(D_CTROPS)
      CURBAN = xplx(D_CURBAN)
...
C
 ...
 24   FORMAT(I3,1X,   A14,2F7.2,1P2E9.2)
...
C
 310  READ(KGLC,330) DINP,IORD,D_ARRT(1),D_BRRT(1),
     &               KCRRT(1),NCOF,SPECL(1),
     1               D_FCVT(1),D_FCT1T(1),D_FCT2T(1),COMMENT
      ARRT(1) = xplx(D_ARRT(1))
      BRRT(1) = xplx(D_BRRT(1))
      FCVT(1) = xplx(D_FCVT(1))
      FCT1T(1) = xplx(D_FCT1T(1))
      FCT2T(1) = xplx(D_FCT2T(1))
...
C
      DO 350 I = 2, NCOF + 1
       READ(KGLC,330) JST,JORD,D_ARRT(I),D_BRRT(I),
     &                KCRRT(I),NDUM,SPECL(I),
     1                D_FCVT(I),D_FCT1T(I),D_FCT2T(I),COMMENT
       ARRT(I) = xplx(D_ARRT(I))
       BRRT(I) = xplx(D_BRRT(I))
       FCVT(I) = xplx(D_FCVT(I))
       FCT1T(I) = xplx(D_FCT1T(I))
       FCT2T(I) = xplx(D_FCT2T(I))
 350  CONTINUE
C
      ! Now read 20 entries instead of 16 (bdf, bmy, 4/1/03)
      READ(KGLC,332) (RINP(I),D_PINP(I),XINP(I),I=1,20)
      PINP(:) = (D_PINP(:))
C
...
C
 ...
 510  FORMAT(I3,1X,2ES7.1,1X,2ES7.1,I6,1X,0P2F3.2,1X,
     1       A6,2(A1,A6),14(A1,0P2F3.1,A6))  
...
C
 ...
 620  FORMAT(I10)
 630  FORMAT(F10.2)
      ASTKCF = xplx(D_ASTKCF)
      RUARSL = xplx(D_RUARSL)
      RH100 = xplx(D_RH100)
C
C *********************************************************************
