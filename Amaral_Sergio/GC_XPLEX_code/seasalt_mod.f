...
!  (5 ) REDGE    (TYPE (XPLEX) ) : Array for edges of seasalt radius bins
!  (6 ) RMID     (TYPE (XPLEX) ) : Array for centers of seasalt radius bins
!  (7 ) SRC      (TYPE (XPLEX) ) : Array for baseline seasalt emission/bin [kg/m2]
!  (7 ) SRC_N    (TYPE (XPLEX) ) : Array for baseline seasalt emission/bin [#/m2]
!  (8 ) SS_DEN   (TYPE (XPLEX) ) : Sea salt density [kg/m3]
!  (9 ) ALK_EMIS (TYPE (XPLEX) ) : Array for alkalinity [kg]
!  (10) N_DENS   (TYPE (XPLEX) ) : Number density of seasalt emissions [#/m3]
!
!  Module Routines:
!  ============================================================================
...
!
      ...
      ! Arrays
      INTEGER              :: IDDEP(NSALT)
      TYPE (XPLEX),  ALLOCATABLE :: REDGE(:,:)   
      TYPE (XPLEX),  ALLOCATABLE :: RMID(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: SRC(:,:)   
      TYPE (XPLEX),  ALLOCATABLE :: SRC_N(:,:)   
      TYPE (XPLEX),  ALLOCATABLE :: ALK_EMIS(:,:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: N_DENS(:,:,:,:)    
      TYPE (XPLEX):: SS_DEN(NSALT)= (/ xplex(2200.d0,0d0),
     & xplex( 2200.d0,0d0) /)
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) TC (TYPE (XPLEX) ) : Sea salt tracer [kg]
...
!  (3 ) Bug fix: DTCHEM has to be TYPE (XPLEX), not integer. (bmy, 9/7/06)
!  (4 ) Now limit relative humidity to [tiny(TYPE (XPLEX)),0.99] range for DLOG
!         argument (phs, 5/1/08)
!******************************************************************************
!
      ! References to F90 modules
...
#     include "CMN_DIAG"     ! ND44
      ...
      TYPE (XPLEX)                 :: OLD,      NEW,   G,      REFF
      TYPE (XPLEX)                 :: DIAM,     U_TS0, REYNOL, ALPHA 
      TYPE (XPLEX)                 :: BETA,     GAMMA, DENS,   FLUX 
      TYPE (XPLEX)                 :: AREA_CM2, TOT1,  TOT2
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) TC (TYPE (XPLEX) ) : Sea salt tracer array [v/v]
!  (2 ) N  (INTEGER) : N=1 denotes accumulation mode; N=2 denotes coarse mode
!
!  Arguments as Output:
!  ============================================================================
!  (1 ) TC (TYPE (XPLEX) ) : Contains modified sea salt concentration [v/v]
!
!  References:
!  ============================================================================
!  (1 ) Chin, M., P. Ginoux, S. Kinne, B. Holben, B. Duncan, R. Martin,
...
#     include "CMN_GCTM"      ! PI
      ...
      INTEGER                :: I,     J,      L
      INTEGER                :: R,     NR,     NTOP
      TYPE (XPLEX)                 :: W10M,  DTEMIS, R0
      TYPE (XPLEX)                 :: R1,    CONST,  CONST_N
      TYPE (XPLEX)                 :: FEMIS, A_M2
      TYPE (XPLEX)                 :: SALT(IIPAR,JJPAR)
      TYPE (XPLEX)                 :: SALT_N(IIPAR,JJPAR)

      ! Increment of radius for Emission integration (um)
      TYPE (XPLEX), PARAMETER      :: DR    = xplex(5.d-2,0d0)
      !-----------------------------------------------------------------------
      ! Prior to 11/23/09:    
      ...
      !TYPE (XPLEX), PARAMETER      :: BETHA = 1.d0
      !-----------------------------------------------------------------------
      TYPE (XPLEX), PARAMETER      :: BETHA = xplex(2.d0,0d0)
...
!      if (DTEMIS%i/=0d0) rint*,'DTEMIS',DTEMIS
      ! Constant [volume * time * other stuff??] 
      CONST   = 4d0/3d0 * PI * DR * DTEMIS * 1.d-18 * 1.373d0
!      if (CONST%i/=0d0) print*,'CONST',CONST
!      if (DR%i/=0d0) print*,'DR',DR
!      if (PI%i/=0d0) print*,'PI',PI
      ! Constant for converting to [#/m2] (bec, bmy, 4/13/05)
      CONST_N = DTEMIS * DR * 1.373d0
!      if (CONST_N%i/=0d0) print*,'CONST_N',CONST_N
      ...
      SELECT CASE( N ) 
         ...
         CASE( 1 )
            R0 = SALA_REDGE_um(1)
!            if (R0%i/=0d0) print*,'R0',R0
            R1 = SALA_REDGE_um(2)
!            if (R1%i/=0d0) print*,'R1',R1 
         ! Coarse mode
         CASE( 2 )
            R0 = SALC_REDGE_um(1)
!            if (R0%i/=0d0) print*,'R0',R0
...
!            if (RMID(R,N)%i/=0d0) print*,'RMID',RMID(R,N)
            ! Upper edge of IRth bin
            REDGE(R,N) = REDGE(R-1,N) + DR 
!            if (REDGE(R,N)%i/=0d0) print*,'REDGE',REDGE(R,N)
!------------------------------------------------------------------------------
! Prior to 11/23/09:
...
!------------------------------------------------------------------------------
     ...
     &           / BETHA**2
!            if (SRC(R,N)%i/=0d0) print*,'SRC,BETHA',SRC(R,N),BETHA
            ! Sea salt base source [#/m2] (bec, bmy, 4/13/05)
            SRC_N(R,N) = CONST_N * (1.d0/RMID(R,N)**3)
     &           * (1.d0+0.057d0*(BETHA*RMID(R,N))**1.05d0)
     &           * 10d0**(1.19d0*EXP(-((0.38d0-LOG10(BETHA*RMID(R,N)))
     &           /0.65d0)**2))/ BETHA**2
!            if (SRC_N(R,N)%i/=0d0) print*,'SRC,BETHA',SRC_N(R,N),BETHA
!### Debug
         ...
         DO I = 1, IIPAR
            ...
            IF ( IS_WATER(I,J) ) THEN
               ...
               ! Wind speed at 10 m altitude [m/s]
                ...
                !  print*,'SFCWIND',SFCWINDSQR(I,J)
               W10M = SQRT( SFCWINDSQR(I,J) )
               !if (W10M%i/=0d0) print*,'W10M',W10M
               ! Loop over size bins
               DO R = 1, NR
...
!
      ...
      ! Return value
      TYPE (XPLEX), INTENT(OUT)  :: ALK1, ALK2 ! [kg]
      TYPE (XPLEX), INTENT(OUT)  :: Kt1, Kt2, Kt1N, Kt2N ! [s-1]
 
      TYPE (XPLEX),  PARAMETER :: PI = xplex(3.14159265,0d0)
      TYPE (XPLEX)             :: N1, N2, Kt
      TYPE (XPLEX)             :: HGF, ALK
      TYPE (XPLEX)             :: RAD1, RAD2, RAD3
      TYPE (XPLEX)             :: term1a, term2a, term3a
      TYPE (XPLEX)             :: term1b, term2b, term3b
      TYPE (XPLEX)             :: term1aN, term2aN, term3aN
      TYPE (XPLEX)             :: term1bN, term2bN, term3bN
      TYPE (XPLEX)             :: const1, const2, const1N, const2N
      TYPE (XPLEX)             :: a1, a2, b1, b2, a1N, a2N, b1N, b2N
      TYPE (XPLEX),  PARAMETER :: MINDAT = xplex(1.d-20,0d0)
      INTEGER            :: IRH
      TYPE (XPLEX),  PARAMETER   :: gamma_SO2 = xplex(0.11d0,0d0) !from Worsnop et al. (1989)
      TYPE (XPLEX),  PARAMETER   :: gamma_HNO3 = xplex(0.2d0,0d0) !from JPL [2001] 
      TYPE (XPLEX),  PARAMETER   :: Dg = xplex(0.2d0,0d0) !gas phase diffusion coeff. [cm2/s]
      TYPE (XPLEX),  PARAMETER   :: v = xplex(3.0d4,0d0) !cm/s
