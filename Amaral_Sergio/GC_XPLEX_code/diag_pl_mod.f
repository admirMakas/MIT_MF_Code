...
!  (9 ) TAUb       (TYPE (XPLEX)  ) : TAU value at start of GEOS-CHEM simulation
!  (10) TAUe       (TYPE (XPLEX)  ) : TAU value at end   of GEOS-CHEM simulation
!  (11) TAU0       (TYPE (XPLEX)  ) : TAU value at start of diagnostic interval
!  (12) TAU1       (TYPE (XPLEX)  ) : TAU value at end of diagnostic interval
!  (13) AD65       (TYPE (XPLEX)  ) : Array for prod/loss diagnostic (a.k.a. ND65)
!  (14) PL24H      (TYPE (XPLEX)  ) : Array for saving P(Ox), L(Ox)  (a.k.a. ND20)
!  (15) FAM_PL     (TYPE (XPLEX)  ) : Array to archive prod & loss from SMVGEAR
!  (16) FAM_COEF   (TYPE (XPLEX)  ) : Coefficient for each prod/loss family member 
...
!      
      ...
      INTEGER                        :: NFAM
      INTEGER                        :: YYYYMMDD
      TYPE (XPLEX)                         :: TAUb, TAUe, TAU0, TAU1
      CHARACTER(LEN=255)             :: FILENAME

      ! Arrays
      INTEGER,           ALLOCATABLE :: FAM_NMEM(:), COUNT(:,:,:)
      TYPE (XPLEX),            ALLOCATABLE :: AD65(:,:,:,:)
      TYPE (XPLEX),            ALLOCATABLE :: FAM_PL(:,:,:,:)
      TYPE (XPLEX),            ALLOCATABLE :: FAM_COEF(:,:)
      TYPE (XPLEX),            ALLOCATABLE :: PL24H(:,:,:,:)
...
#     include "comode.h"
      ...
      TYPE (XPLEX)             :: PL
      TYPE (XPLEX)             :: COEFMEM(MAXMEM,MAXFAM)
      TYPE (XPLEX)             :: COEFPL(MAXPL,MAXFAM)
...
#     include "CMN_DIAG"   ! LD65
      ...
      TYPE (XPLEX)              :: ARRAY(IIPAR,JJPAR,LLTROP)
...
!  (5 ) COEF  (TYPE (XPLEX)   ) : Coefficients for each prod/loss family member
!
!  NOTES:
...
#     include "comode.h"           ! LFAMILY, NFAMILIES
      ...
      TYPE (XPLEX),            INTENT(IN) :: COEF(MAXMEM,MAXFAM)
