! $Id: diag51_mod.f,v 1.1.1.1 2009/06/09 21:51:52 daven Exp $
      MODULE DIAG51_MOD
!
!******************************************************************************
...
!  (12) ND51_HR1         (TYPE (XPLEX)  ) : Starting hour of user-defined LT interval
!  (13) ND51_HR2         (TYPE (XPLEX)  ) : Ending hour of user-defined LT interval
...
!  (26) Q                (TYPE (XPLEX)  ) : Accumulator array for various quantities
!  (27) TAU0             (TYPE (XPLEX)  ) : Starting TAU used to index the bpch file
!  (28) TAU1             (TYPE (XPLEX)  ) : Ending TAU used to index the bpch file
!  (29) HALFPOLAR        (INTEGER ) : Used for bpch file output
!  (30) CENTER180        (INTEGER ) : Used for bpch file output
!  (31) LONRES           (TYPE (XPLEX)  ) : Used for bpch file output
!  (32) LATRES           (TYPE (XPLEX)  ) : Used for bpch file output
...
!
      ...
      TYPE (XPLEX)               :: ND51_HR1,       ND51_HR2
      TYPE (XPLEX)               :: ND51_HR_WRITE
      ...
      !TYPE (XPLEX),  PARAMETER   :: HR1        = 13d0
      !TYPE (XPLEX),  PARAMETER   :: HR2        = 17d0
      ...
      !TYPE (XPLEX),  PARAMETER   :: HR1        = 10d0
      !TYPE (XPLEX),  PARAMETER   :: HR2        = 12d0
...
!  (6 ) Bug fix: LT should be TYPE (XPLEX) and not INTEGER (ccarouge, 12/10/08)
!******************************************************************************
!
...
#     include "CMN_SIZE"   ! Size parameters
      ...
      !------------------------------------------
      ! Prior to 12/10/08:
      ! LT should be TYPE (XPLEX) (ccarouge, 12/10/08)
      !INTEGER :: I, LT
      !------------------------------------------
      INTEGER :: I
      TYPE (XPLEX)  :: LT
...
#     include "CMN_GCTM"  ! SCALE_HEIGHT
      ...
      TYPE (XPLEX)            :: C1, C2, PBLDEC, TEMPBL, TMP, SCALE400nm
...
!  Arguments as Output:
!  ============================================================================
!  (1 ) TAU_W (TYPE (XPLEX)) : TAU value at time of writing to disk
!
!  NOTES:
...
!
     ...
     &                     GET_TAUb, GET_TAUe,   GET_TS_DYN
      ...
      ! Arguments
      TYPE (XPLEX), INTENT(OUT) :: TAU_W

      ! Local variables
      LOGICAL             :: ITS_TIME
      TYPE (XPLEX)              :: TAU, HOUR, DYN
...
!$OMP END PARALLEL DO
     ...
     &             ( Q(1:ND51_NI, 1:ND51_NJ, 1:GMNL, W) ) )
      ENDDO
...
!  (4 ) HR_WRITE (TYPE (XPLEX)  ) : GMT hour of day at which to write bpch file
!  (5 ) HR1      (TYPE (XPLEX)  ) : Lower limit of local time averaging bin
!  (6 ) HR2      (TYPE (XPLEX)  ) : Upper limit of local time averaging bin
...
#     include "CMN_SIZE"   ! Size parameters
      ...
      TYPE (XPLEX),             INTENT(IN) :: HR1,    HR2
      TYPE (XPLEX),             INTENT(IN) :: HR_WRITE
