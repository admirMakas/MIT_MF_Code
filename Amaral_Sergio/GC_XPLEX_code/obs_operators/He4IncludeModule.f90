! $Id: He4IncludeModule.f90,v 1.1 2009/06/18 19:53:07 daven Exp $
MODULE He4IncludeModule

  !========================================================================
  ! Module He4IncludeModule contains the various parameter settings for
  ! the HDF-EOS4 library routines and F90 modules. (bmy, 1/17/06)  
  !
  ! Original Code From:
  !
  !    NCSA HDF 
  ...
  !    Contents: Tag definitions
  !              Error return codes
  !              Logical constants
  !    Remarks:  This file can be included with Fortran user programs.  
  !              As a general rule, don't use DFNT constants that don't 
  !              include a number in their name.  E.g., don't use 
  !              DFNT_FLOAT, use DFNT_FLOAT32 or DFNT_FLOAT64.  The DFNT 
  !              constants that don't include numbers are for backward 
  !              compatibility only.  Also, there are no current plans to 
  !              support 128-bit number types.  For more information about 
  !              constants in this file, see the equivalent constant 
  !              declarations in the C include file 'hdf.h' 
  !
  ...
  !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  !%%%                                                          %%%
  !%%%        Original HDF-EOS4 parameters begin here!          %%%
  !%%%                                                          %%%
  !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  ! Define synonym for FAIL 
  INTEGER, PARAMETER :: FAILURE = FAIL
...
#include "He4Define.h"
  ...
  INTEGER, PARAMETER :: HE4_INT = KIND( 1e0 )
#elif defined( NEED_INT_64 )
  INTEGER, PARAMETER :: HE4_INT = KIND( 1d0 )
#endif

  !-----------------------------------------------------------------
  ! NOTE: The size of the following parameters may be changed 
  ! arbitrarily. You need these to dimension Fortran arrays.
  ! (bmy, 1/17/06)
  !-----------------------------------------------------------------

  ! Max # of attributes per HDF-EOS file
  INTEGER, PARAMETER :: HE4_MAX_ATRS = 100

  ! Max character string length for HDF-EOS file routines
  INTEGER, PARAMETER :: HE4_MAX_CHAR = 1999

  ! Max # of dimensions per HDF-EOS5 file
  INTEGER, PARAMETER :: HE4_MAX_DIMS = 50

  ! Max # of fields per HDF-EOS5 file
  INTEGER, PARAMETER :: HE4_MAX_FLDS = 200

END MODULE He4IncludeModule
