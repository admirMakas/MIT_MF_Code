! $Id: pjc_pfix_mod.f,v 1.1.1.1 2009/06/09 21:51:50 daven Exp $
!------------------------------------------------------------------------------
...
!                               Declare all REAL variables as TYPE (XPLEX).
!EOP
!------------------------------------------------------------------------------
!BOC
!
...
!
      ...
      TYPE (XPLEX)  :: pmet2(ilo_gl:ihi_gl, julo_gl:jhi_gl)

      ! CTM surface pressure at t1 [hPa]
      TYPE (XPLEX)  :: pctm1(ilo_gl:ihi_gl, julo_gl:jhi_gl)

      ! CTM surface pressure at t1+tdt [hPa]
      TYPE (XPLEX)  :: pctm2(ilo_gl:ihi_gl, julo_gl:jhi_gl)
...
!
      ...
      ! Change of surface pressure from met field pressure [hPa]
      TYPE (XPLEX), INTENT(IN)   :: dps(i1:i2, ju1:j2)

      ! Relative surface area of grid box (fraction)
      TYPE (XPLEX), INTENT(IN)   :: rel_area(i1:i2, ju1:j2)

      ! Horizontal mass fluxes in E-W and N-S directions [hPa]
      TYPE (XPLEX), INTENT(IN)   :: xmass(ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX), INTENT(IN)   :: ymass(ilo:ihi, julo:jhi, k1:k2)
!
! !OUTPUT PARAMETERS:
!
      ! Sum over vertical of dpi calculated from original mass fluxes [hPa]
      TYPE (XPLEX),  INTENT(OUT) :: dps_ctm(i1:i2, ju1:j2)

      ! Horizontal mass flux in E-W and N-S directions after fixing [hPa]
      TYPE (XPLEX), INTENT(OUT) :: xmass_fixed(ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX), INTENT(OUT) :: ymass_fixed(ilo:ihi, julo:jhi, k1:k2)
!
! !AUTHOR:
!  Philip Cameron-Smith and John Tannahill, GMI project @ LLNL (2003)
!
...
!
      ...
      TYPE (XPLEX),  INTENT(IN)  :: cosp(ju1_gl:j2_gl)

      ! Wind velocity in E-W (UU) and N-S (VV) directions at t1+tdt/2 [m/s]
      TYPE (XPLEX),  INTENT(IN)  :: uu  (ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX),  INTENT(IN)  :: vv  (ilo:ihi, julo:jhi, k1:k2)
!
! !OUTPUT PARAMETERS:
!
      ! Courant number in E-W (CRX) and N-S (CRY) directions
      TYPE (XPLEX),  INTENT(OUT) :: crx (ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX),  INTENT(OUT) :: cry (ilo:ihi, julo:jhi, k1:k2)
...
!
      ...
      TYPE (XPLEX), INTENT(IN)   :: delpm(ilo:ihi, julo:jhi, k1:k2)
...
!
      ...
      TYPE (XPLEX), INTENT(OUT)  :: xmass(ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX), INTENT(OUT)  :: ymass(ilo:ihi, julo:jhi, k1:k2)
...
!
      ...
      TYPE (XPLEX),  INTENT(INOUT) :: dpi   (i1:i2, ju1:j2, k1:k2)
!
! !AUTHOR:
!  Philip Cameron-Smith and John Tannahill, GMI project @ LLNL (2003)
! 
...
!
      ! Pressure difference across layer from (ai * pt) term [hPa]
      TYPE (XPLEX), INTENT(IN)  :: dap  (k1:k2)
...
!
      ...
      TYPE (XPLEX), INTENT(OUT) :: delp1(ilo:ihi, julo:jhi, k1:k2)

      ! Pressure thickness, the psudo-density in a 
      ! hydrostatic system at t1+tdt/2 (approximate)  [hPa]
      TYPE (XPLEX), INTENT(OUT) :: delpm(ilo:ihi, julo:jhi, k1:k2)

      ! Pressure at edges in "u" [hPa]
      TYPE (XPLEX), INTENT(OUT) :: pu   (ilo:ihi, julo:jhi, k1:k2)
!
! !AUTHOR:
!  Philip Cameron-Smith and John Tannahill, GMI project @ LLNL (2003)
! 
...
! !REVISION HISTORY: 
!   02 Dec 2008 - R. Yantosca - Updated documentation and added ProTeX headers.
!                               Now make all REAL variables TYPE (XPLEX).
!EOP
!------------------------------------------------------------------------------
!BOC
!
