! $Id: aerosol_mod.f,v 1.1.1.1 2009/06/09 21:51:50 daven Exp $
      MODULE AEROSOL_MOD
!
!******************************************************************************
...
!  Module Variables:
!  ============================================================================
!  (1 ) BCPI        (TYPE (XPLEX)) : Hydrophilic black carbon aerosol   [kg/m3]
!  (2 ) BCPO        (TYPE (XPLEX)) : Hydrophobic black carbon aerosol   [kg/m3]
!  (3 ) OCPI        (TYPE (XPLEX)) : Hydrophilic organic carbon aerosol [kg/m3]
!  (4 ) OCPO        (TYPE (XPLEX)) : Hydrophilic organic carbon aerosol [kg/m3]
!  (5 ) SALA        (TYPE (XPLEX)) : Accumulation mode seasalt aerosol  [kg/m3] 
!  (6 ) SALC        (TYPE (XPLEX)) : Coarse mode seasalt aerosol        [kg/m3]
!  (7 ) SO4_NH4_NIT (TYPE (XPLEX)) : Lumped SO4-NH4-NIT aerosol         [kg/m3]
!  (8 ) SOILDUST    (TYPE (XPLEX)) : Mineral dust aerosol from soils    [kg/m3]
!
!  Module Routines:
!  ============================================================================
...
!
      ...
      ! MODULE VARIABLES
      !=================================================================
      TYPE (XPLEX), ALLOCATABLE :: BCPI(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: BCPO(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: OCPI(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: OCPO(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: SALA(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: SALC(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: SO4_NH4_NIT(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: SOILDUST(:,:,:,:)
...
#     include "comode.h"  ! AD, AVG, WTAIR, other SMVGEAR variables
      ...
      INTEGER              :: I, J, L, JLOOP
      TYPE (XPLEX)               :: CONSEXP, TK, VPRESH2O
...
#     include "CMN_SIZE"  ! Size parameters
      ...
      ! (tmf, 2/10/09)
      !TYPE (XPLEX),  PARAMETER :: OCF = 1.4d0
      !-----------------------------------------------------------------
      TYPE (XPLEX),  PARAMETER :: OCF = xplex(2.1d0,0d0)

      ! For SOAG, assume the total aerosol mass/glyoxal mass = 1.d0 
      ! for now (tmf, 1/7/09)
      TYPE (XPLEX),  PARAMETER :: OCFG = xplex(1.d0,0d0)

      ! For SOAM, assume the total aerosol mass/methylglyoxal mass = 1.d0 
      ! for now (tmf, 1/7/09)
      TYPE (XPLEX),  PARAMETER :: OCFM = xplex(1.d0,0d0)
      !=================================================================
...
#     include "comode.h"   ! NTLOOP
      ...
      TYPE (XPLEX)              :: MSDENS(NAER), XTAU, DRYAREA


      ! Mass of hydrophobic aerosol from Mian Chin
      TYPE (XPLEX), SAVE        :: DAERSL(IIPAR,JJPAR,LLPAR,2)       

      ! Mass of hydrophilic aerosol from Mian Chin
      TYPE (XPLEX), SAVE        :: WAERSL(IIPAR,JJPAR,LLPAR,NAER)    
      ...
      TYPE (XPLEX)		  :: SCALEOD(IIPAR,JJPAR,LLPAR,NRH) 

      ! Change in Vol vs RH 
      TYPE (XPLEX)		  :: SCALEVOL(IIPAR,JJPAR,LLPAR)  

      ! Relative Humidities
      TYPE (XPLEX),  SAVE  ::RH(NRH)= ( /xplex(0d0,0d0),
     &                                    xplex(0.5d0,0d0),
     &                                    xplex(0.7d0,0d0),
     &                                    xplex(0.8d0,0d0),
     &                                    xplex(0.9d0,0d0)/)
