!$Id: lidort_lc_corrections.f90,v 1.1 2010/07/30 23:47:04 daven Exp $
! ###########################################################
! #                                                         #
! #                    THE LIDORT FAMILY                    #
! #                                                         #
...
!  other help variables
      ...
      REAL(kind=8)   :: L_FINAL_SOURCE, L_SS_LAYERSOURCE, LEGPOLY
      REAL(kind=8)   :: L_SSCORRECTION, L_TR_CUMSOURCE, HELP3
      REAL(kind=8)   :: VAR_TMS(MAX_ATMOSWFS,MAXLAYERS), L_PHASMOM
...
!  All linearized multipliers
       ...
       REAL(kind=8)   :: L_UP_MULTIPLIERS_UT (MAX_PARTLAYERS,MAX_ATMOSWFS,MAX_GEOMETRIES)
       REAL(kind=8)   :: L_DN_MULTIPLIERS_UT (MAX_PARTLAYERS,MAX_ATMOSWFS,MAX_GEOMETRIES)
...
!  -------
      ...
      real(kind=8), intent(out) :: L_multipliers (maxlayers,max_atmoswfs)
      ...
      real(kind=8), intent(out) :: L_multipliers_p (max_PARTLAYERS,max_atmoswfs)
      real(kind=8), intent(out) :: L_lostrans_p    (max_PARTLAYERS,max_atmoswfs)

      real(kind=8), intent(out) :: boa_attn
      real(kind=8), intent(out) :: L_boa_attn(max_atmoswfs)
...
!  Local attenuation factors, linearizations
      ...
      real(kind=8)  :: l_attn_fine ( maxlayers, maxfinelayers, max_atmoswfs )
      real(kind=8)  :: l_attn_p    ( max_PARTLAYERS, max_atmoswfs  )
...
!  Solar beam attenuation to BOA (required for exact DB calculation)
      ...
      REAL(kind=8), intent(in)  :: L_BOA_ATTN(MAX_ATMOSWFS,MAX_GEOMETRIES)
...
!  Column WFs for the Direct Bounce
      ...
      REAL(kind=8), intent(out) :: COLUMNWF_DB ( MAX_ATMOSWFS, MAX_USER_LEVELS,MAX_GEOMETRIES )
