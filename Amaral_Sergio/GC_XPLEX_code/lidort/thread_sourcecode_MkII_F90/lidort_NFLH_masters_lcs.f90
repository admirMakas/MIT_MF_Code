!$Id: lidort_NFLH_masters_lcs.f90,v 1.1 2010/07/30 23:47:04 daven Exp $
! ###########################################################
! #                                                         #
! #                    THE LIDORT FAMILY                    #
! #                                                         #
...
! #                                                             #
! #            LIDORT_LCS_MASTER_NFLH (top-level master)        #
! #            LIDORT_LCS_FOURIER_MASTER                        #
! #                                                             #
! ###############################################################

SUBROUTINE LIDORT_LCS_MASTER_NFLH                                       & 
           ( THREAD,                 DO_BRDF_SURFACE,                   & ! Input
            DO_UPWELLING,            DO_DNWELLING,                      & ! Input
            DO_FULLRAD_MODE,         DO_SSFULL,                         & ! Input
            DO_SOLAR_SOURCES,        DO_PLANE_PARALLEL,                 & ! Input
            DO_RAYLEIGH_ONLY,        DO_ISOTROPIC_ONLY,                 & ! Input or Output
            DO_SSCORR_TRUNCATION,    DO_DOUBLE_CONVTEST,                & ! Input or Output
            DO_SSCORR_NADIR,         DO_SSCORR_OUTGOING,                & ! Input or Output
            DO_SOLUTION_SAVING,      DO_BVP_TELESCOPING,                & ! Input or Output
            DO_ALL_FOURIER,          DO_NO_AZIMUTH,                     & ! Input or Output
            DO_USER_STREAMS,         DO_DELTAM_SCALING,                 & ! Input of Output
            DO_ADDITIONAL_MVOUT,     DO_MVOUT_ONLY,                     & ! Input or Output
            DO_COLUMN_LINEARIZATION, DO_SURFACE_LINEARIZATION,          & ! Input
           NBEAMS, NSTREAMS, NLAYERS, NMOMENTS_INPUT,                   & ! Input (Nmoments_input re-set)
           N_USER_STREAMS, N_USER_RELAZMS, N_USER_LEVELS,               & ! Input
           BEAM_SZAS, USER_ANGLES_INPUT, USER_RELAZMS, USER_LEVELS,     & ! Input
           FLUX_FACTOR, LIDORT_ACCURACY,  EARTH_RADIUS,                 & ! Input
           LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_TOTALCOLUMN_WFS,       & ! Input
           ...
           LS_BRDF_F_0, LS_BRDF_F, LS_USER_BRDF_F_0, LS_USER_BRDF_F,    & ! Input
            HEIGHT_GRID,          LH_HEIGHT_GRID, LHMASK_CWFS,          & ! Input
            ...
            COLUMNWF,  MINT_COLUMNWF,  FLUX_COLUMNWF,                   & ! Output
...
!  subroutine input arguments (Intent(in))
!  +++++++++++++++++++++++++++++++++++++++
...
!  stream angle flag
      ...
      LOGICAL,intent(inout)  :: DO_USER_STREAMS          ! May be re-set after Checking
...
!  Total number of column Jacobians
      ...
      LOGICAL, intent(in)  ::  LHMASK_CWFS ( MAX_ATMOSWFS )
...
!  2. User/File-read inputs
!  ========================

!  (a) stream/layer/moment (INTEGER input)
!      ----------------------------------
...
!  Linearized multilayer Height inputs
...
!  Surface inputs (New BRDF arrays, 23 March 2010)
!  --------------
...
!               Intent(in) to the Converge and LC_converge routines

!  Single scatter intensity

      REAL(KIND=8)    :: INTENSITY_SS ( MAX_USER_LEVELS, MAX_GEOMETRIES, MAX_DIRECTIONS )
...
!  Single scatter Column weighting functions at user angles
      ...
      REAL(kind=8)    :: COLUMNWF_SS  ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAX_DIRECTIONS )

!  Direct Bounce Column weighting functions at user angles

      REAL(kind=8)    :: COLUMNWF_DB  ( MAX_ATMOSWFS, MAX_USER_LEVELS, MAX_GEOMETRIES )
...
!  Fourier comonents User-defined solutions
      ...
      REAL(KIND=8)    :: INTENSITY_F  (MAX_USER_LEVELS,MAX_USER_STREAMS,MAXBEAMS,MAX_DIRECTIONS)

!  Fourier-component Column weighting functions at user angles

      REAL(kind=8)    :: COLUMNWF_F ( MAX_ATMOSWFS,     MAX_USER_LEVELS, &
...
!  Solar beam attenuation to BOA (required for exact DB calculation)
      ...
      REAL(kind=8)    :: L_BOA_ATTN(MAX_ATMOSWFS,MAX_GEOMETRIES)
...
! Linearized Chapman factors (from pseudo-spherical geometry)

      REAL(KIND=8)    :: L_CHAPMAN_FACTORS ( MAX_ATMOSWFS, MAXLAYERS, MAXLAYERS, MAXBEAMS )
...
!  Linearized Average-secant and initial tramsittance factors for solar beams.
      ...
      REAL(kind=8)    :: LC_INITIAL_TRANS  ( MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS  )
      REAL(kind=8)    :: LC_AVERAGE_SECANT ( MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS  )

!     Solar beam attenuation

      REAL(KIND=8)    :: SOLAR_BEAM_OPDEP ( MAXBEAMS )
      REAL(KIND=8)    :: LC_BEAMOPDEP     ( MAX_ATMOSWFS, MAXBEAMS )
...
!  Linearized transmittances, solar beam
      ...
      REAL(kind=8)    :: LC_T_DELT_MUBAR ( MAXLAYERS,      MAXBEAMS, MAX_ATMOSWFS )
      REAL(kind=8)    :: LC_T_UTDN_MUBAR ( MAX_PARTLAYERS, MAXBEAMS, MAX_ATMOSWFS )
...
!  Linearized whole layer multipliers
      ...
      REAL(kind=8)    :: LC_EMULT_UP ( MAX_USER_STREAMS, MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS )
      REAL(kind=8)    :: LC_EMULT_DN ( MAX_USER_STREAMS, MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS )

!  Linearized part layer multipliers

      REAL(kind=8)    :: LC_UT_EMULT_UP ( MAX_USER_STREAMS, MAX_PARTLAYERS, MAXBEAMS, MAX_ATMOSWFS )
      REAL(kind=8)    :: LC_UT_EMULT_DN ( MAX_USER_STREAMS, MAX_PARTLAYERS, MAXBEAMS, MAX_ATMOSWFS )
...
!  local integers
      ...
      INTEGER         :: UA, UM, IB, TESTCONV, L, Q, N, K
      INTEGER         :: LOCAL_N_USERAZM, STATUS_SUB

!  Local variables

       REAL(KIND=8)   :: SUM, SUM1, COS_SZA
...
!  Local variables (NF assumption)
!  ===============================

!  Chapman factors

      LOGICAL, parameter :: DO_CHAPMAN_FUNCTION = .TRUE.

!  No refractive geometry

      LOGICAL, parameter :: DO_REFRACTIVE_GEOMETRY = .FALSE.

! Number of fine layers subdividing all computational layers

      INTEGER, parameter :: NFINELAYERS = 0
...
!  Exception handling
      ...
      if ( fail ) then
        ...
        TRACE_3 = ' ** LIDORT_LCS_MASTER_NFLH, THREAD # '//wthread
...
!  Could be done outside the thread loop
      ...
      IF ( DO_SOLAR_SOURCES ) THEN
        DO IB = 1, NBEAMS
          COS_SZA = DCOS ( BEAM_SZAS(IB) * DEG_TO_RAD )
          DO N = 0, NLAYERS  
           SZA_LOCAL_INPUT(N,IB) = BEAM_SZAS(IB)
          ENDDO
          CALL CHAPMAN_FUNCTION_NF_PLUS                             &
          ( MAXLAYERS, MAX_ATMOSWFS, DO_PLANE_PARALLEL,             & ! Inputs
            NLAYERS, N_TOTALCOLUMN_WFS, COS_SZA, EARTH_RADIUS,      & ! Inputs
            HEIGHT_GRID, LH_HEIGHT_GRID,                            & ! Inputs
            CHAPMAN_FACTORS(1,1,IB), L_CHAPMAN_FACTORS(1,1,1,IB) )    ! outputs
        ENDDO
      ENDIF
...
!          CALL LIDORT_LCS_WRITEINPUT ( IUNIT )
!        ENDIF
!        CLOSE(IUNIT)
!      ENDIF
...
!  Exception handling on this module
      ...
      IF ( STATUS_SUB .EQ. LIDORT_WARNING ) THEN
        TRACE_2 = 'Direct call in LIDORT_LCS_MASTER_NFLH'
        TRACE_3 = ' ** LIDORT_LCS_MASTER_NFLH, THREAD # '//wthread
...
!  Linearization of Solar beam optical depth
      ...
      IF ( DO_COLUMN_LINEARIZATION ) THEN
        DO IB = 1, NBEAMS
          DO Q = 1, N_TOTALCOLUMN_WFS
            SUM = ZERO
            DO K = 1, NLAYERS
              SUM1 = CHAPMAN_FACTORS(NLAYERS,K,IB) * L_DELTAU_VERT(Q,K) &
                       + L_CHAPMAN_FACTORS(Q,NLAYERS,K,IB)
              SUM = SUM - DELTAU_VERT(K) * SUM1
            ENDDO
            LC_BEAMOPDEP(Q,IB) = SUM
          ENDDO
        ENDDO
      ENDIF

!  Linearization of pseudo-spherical setup
!     Requires the NFLH Supplement here...........

      IF ( DO_COLUMN_LINEARIZATION ) THEN
        CALL LIDORT_LC_PREPTRANS_NFLH                             & 
        ( DO_PLANE_PARALLEL, NLAYERS, NBEAMS, N_PARTLAYERS,       & ! Input
          PARTLAYERS_LAYERIDX, LAYER_VARY_NUMBER,                 & ! Input
          DELTAU_VERT, PARTAU_VERT, L_DELTAU_VERT,                & ! Input
          CHAPMAN_FACTORS, L_CHAPMAN_FACTORS,                     & ! Input
...
!  ----------------------------------
        ...
        IF ( DO_SSCORR_OUTGOING ) THEN
          ...
          CALL LIDORT_LC_SSCORR_OUTGOING_NFLH                            &
        ( DO_UPWELLING, DO_DNWELLING, DO_COLUMN_LINEARIZATION,           & ! Input
          DO_SSCORR_TRUNCATION, DO_DELTAM_SCALING,                       & ! Input     
          NLAYERS, NMOMENTS_INPUT, NMOMENTS, NBEAMS,                     & ! Input
          N_USER_STREAMS, N_USER_RELAZMS, N_USER_LEVELS,                 & ! Input
          N_TOTALCOLUMN_WFS, LHMASK_CWFS, THREAD, SS_FLUX_MULTIPLIER,    & ! Input
          BEAM_SZAS_ADJUST, USER_ANGLES_ADJUST, USER_RELAZMS_ADJUST,     & ! Input
          N_GEOMETRIES, UMOFF, STERM_LAYERMASK_UP, STERM_LAYERMASK_DN,   & ! Input
          UTAU_LEVEL_MASK_UP, UTAU_LEVEL_MASK_DN, DO_PHFUNC_VARIATION,   & ! Input
          HEIGHT_GRID, LH_HEIGHT_GRID, EARTH_RADIUS,                     & ! Input  
          TRUNC_FACTOR, DELTAU_VERT, L_DELTAU_VERT,                      & ! Input   
          OMEGA_TOTAL_INPUT,    L_OMEGA_TOTAL_INPUT,                     & ! Input
          PHASMOMS_TOTAL_INPUT, L_PHASMOMS_TOTAL_INPUT,                  & ! Input
          ...
          IF ( STATUS_SUB .ne. LIDORT_SUCCESS ) THEN
            TRACE_2 = 'Error from LIDORT_LC_SSCORR_OUTGOING_NFLH'
            TRACE_3 = ' ** LIDORT_LCS_MASTER_NFLH, THREAD # '//wthread
            STATUS_CALCULATION = LIDORT_SERIOUS
            RETURN
          ENDIF 
...
!        write(*,*)' ..calculating fourier component',FOURIER_COMPONENT
       ...
       ( DO_UPWELLING, DO_DNWELLING, DO_SOLAR_SOURCES,                   & ! Input
         ...
         COLUMNWF_F,  MINT_COLUMNWF,  FLUX_COLUMNWF,                     & ! Output
...
!        IF ( DO_COLUMN_LINEARIZATION ) THEN
!          CALL LIDORT_LC_WRITERESULTS ( RUNIT, LOCAL_N_USERAZM )
!        ENDIF
!        CLOSE(RUNIT)
!      ENDIF
...
!        IF ( DO_COLUMN_LINEARIZATION ) THEN
!          CALL LIDORT_LC_WRITESCEN ( SUNIT )
!        ENDIF
!        CLOSE(SUNIT)
!      ENDIF
...
!  Finish
      ...
      RETURN
END SUBROUTINE LIDORT_LCS_MASTER_NFLH
...
!  Particular beam solutions at user-defined stream angles
      ...
      REAL(kind=8)  :: LC_U_WPOS1(MAX_USER_STREAMS,MAXLAYERS,MAX_ATMOSWFS)
      REAL(kind=8)  :: LC_U_WNEG1(MAX_USER_STREAMS,MAXLAYERS,MAX_ATMOSWFS)

!  Linearizations of Holding arrays for Multiplier coefficients

      REAL(kind=8)  :: LC_GAMMA_M (MAXSTREAMS,MAXLAYERS,MAX_ATMOSWFS)
      REAL(kind=8)  :: LC_GAMMA_P (MAXSTREAMS,MAXLAYERS,MAX_ATMOSWFS)
...
!  Linearized TOA and BOA source terms
      ...
      REAL(kind=8)  :: LC_TOA_SOURCE  (MAX_USER_STREAMS,MAX_ATMOSWFS)
      REAL(kind=8)  :: LC_BOA_MSSOURCE(MAX_USER_STREAMS,MAX_ATMOSWFS)
      REAL(kind=8)  :: LC_BOA_DBSOURCE(MAX_USER_STREAMS,MAX_ATMOSWFS)

!  Column weighting functions at quadrature angles

      REAL(kind=8)  :: QUADCOLUMNWF ( MAX_ATMOSWFS, MAX_USER_LEVELS, &
...
!  Linearized Green's function multipliers for off-grid optical depths
      ...
      REAL(kind=8)  :: LC_UT_GMULT_UP (MAXSTREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)
      REAL(kind=8)  :: LC_UT_GMULT_DN (MAXSTREAMS,MAX_PARTLAYERS,MAX_ATMOSWFS)
...
!  Local simulation only
      ...
      DO_SIMULATION_ONLY = ( .NOT. DO_COLUMN_LINEARIZATION .AND. .NOT. DO_SURFACE_LINEARIZATION )
...
!  Get the telescoped boundary value result
       ...
       ( DO_INCLUDE_SURFACE, DO_LOCAL_DIRECTBEAM(IBEAM), NSTREAMS, NLAYERS, & ! Input
         NSTREAMS_2, NTOTAL, N_SUPDIAG, N_SUBDIAG, QUAD_STRMWTS,            & ! Input       
         FOURIER_COMPONENT, IBEAM, SURFACE_FACTOR, ALBEDO,                  & ! Input
...
!  Get the linearized BVP solution for this beam component
            ...
            ( DO_INCLUDE_SURFACE, DO_LOCAL_DIRECTBEAM(IBEAM),              & ! Input
              ...
              NSTREAMS_2, NTOTAL, N_SUBDIAG, N_SUPDIAG, N_TOTALCOLUMN_WFS, & ! Input
...
!  Linearized BOA source term
         ...
         ( DO_INCLUDE_SURFACE, DO_INCLUDE_DIRECTBEAM, DO_BRDF_SURFACE,      & ! Input
           ...
           FOURIER_COMPONENT, IBEAM, N_TOTALCOLUMN_WFS, SURFACE_FACTOR,     & ! Input
           ...
           LC_BOA_MSSOURCE, LC_BOA_DBSOURCE )                                 ! output
...
!  TOA source term linearized
              ...
              CALL GET_L_TOASOURCE  ( N_USER_STREAMS, LC_TOA_SOURCE, N_TOTALCOLUMN_WFS )
...
!  Downwelling weighting functions
           ...
           ( DO_USER_STREAMS,  DO_PLANE_PARALLEL,                          & ! Input
             ...
             PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX,                      & ! Input
             UTAU_LEVEL_MASK_DN, PARTLAYERS_LAYERIDX,                      & ! Input
             FOURIER_COMPONENT, IBEAM, N_TOTALCOLUMN_WFS, FLUX_MULTIPLIER, & ! Input
             INITIAL_TRANS, LAYER_PIS_CUTOFF, DO_LAYER_SCATTERING,         & ! Input    
             T_DELT_EIGEN, T_UTUP_EIGEN, T_UTDN_EIGEN,                     & ! Input
             T_DELT_MUBAR, T_UTDN_MUBAR, T_DELT_USERM, T_UTDN_USERM,       & ! Input
             AGM, BGP, XPOS, LCON, LCON_XVEC, MCON, MCON_XVEC,             & ! Input  
             U_XPOS, U_XNEG, U_WNEG1,                                      & ! Input
