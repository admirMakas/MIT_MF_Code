!$Id: lidort_aux.f90,v 1.1 2010/07/30 23:47:04 daven Exp $
! ###########################################################
! #                                                         #
! #                    THE LIDORT FAMILY                    #
! #                                                         #
...
! #      lidort_status:         R.J.D. Spurr, RTS, 2005         #
...
SUBROUTINE LIDORT_STATUS                                              &
        ( THREAD, ERRORFILE, ERRORUNIT, OPENFILEFLAG,                 &
...
!     Message Length should be at least 120 Characters
      ...
      INTEGER      , intent(in)    :: NCHECKMESSAGES
      CHARACTER*(*), intent(in)    :: CHECKMESSAGES(0:MAX_MESSAGES)
      CHARACTER*(*), intent(in)    :: ACTIONS (0:MAX_MESSAGES)

!  Exception handling for Model Calculation. New code, 18 May 2010

      INTEGER      , intent(in)    :: STATUS_CALCULATION
      CHARACTER*(*), intent(in)    :: MESSAGE, TRACE_1, TRACE_2, TRACE_3

!  Local

      INTEGER          :: N, W, LEN_STRING, NA, NF
...
!  input/output arguments
      ...
      REAL(KIND=8), intent(inout) :: AAD(MAXSTREAMS,MAXSTREAMS)
      REAL(KIND=8), intent(inout) :: WKD(MAXSTREAMS_2)

      REAL(KIND=8), intent(out)   :: EVALD(MAXSTREAMS)
      REAL(KIND=8), intent(out)   :: EVECD(MAXSTREAMS,MAXSTREAMS)
...
!  local variables (explicit declaration
      ...
      DATA               C5 / 16.0D0 /
      DATA               C6 / 256.0D0 /
      REAL(KIND=8)        :: TOL, DISCRI, SGN, RNORM, W, F, G, H, P, Q, R
      REAL(KIND=8)        :: REPL, COL, ROW, SCALE, T, X, Z, S, Y, UU, VV
...
SUBROUTINE GAULEG(X1,X2,X,W,N)
      ...
      INTEGER        :: I, M, J
      REAL(KIND=8)   :: XM,XL,P1,P2,P3,PP,Z,Z1
...
!     ---------------
      ...
      Real(Kind=8), intent(in) :: x          ! abscissa
...
!     ---------------
      ...
      Integer       :: i, ll
      Real(Kind=8)  :: fact, fll, fmm, fmmp1, somx2

!     ---------------
!     Output Variables
!     ---------------

      Real(Kind=8), intent(out) ::  cfplg (0:maxmom) ! array of Plgs
...
!     .. External Subroutines ..
      EXTERNAL           DCOPY_LIDORT, DGBTF2, DGEMM, DGER, DLASWP, DSCAL_LIDORT, &
                         DSWAP, DTRSM, XERBLA
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
!                 Compute multipliers
!
                  CALL DSCAL_LIDORT( KM, ONE / AB( KV+1, JJ ), AB( KV+2, JJ ), 1 )
!
...
!
               NW = MIN( JJ-J+1, I3 )
               IF( NW.GT.0 ) &
                 CALL DCOPY_LIDORT( NW, AB( KV+KL+1-JJ+J, JJ ), 1, &
                             WORK31( 1, JJ-J+1 ), 1 )
   80       CONTINUE
            IF( J+JB.LE.N ) THEN
!
...
!
               NW = MIN( I3, JJ-J+1 )
               IF( NW.GT.0 ) &
                 CALL DCOPY_LIDORT( NW, WORK31( 1, JJ-J+1 ), 1, &
...
!     ..
!     .. External Subroutines ..
      EXTERNAL           DGER, DSCAL_LIDORT, DSWAP, XERBLA
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
!     ..
...
!              Compute multipliers.
!
               CALL DSCAL_LIDORT( KM, ONE / AB( KV+1, J ), AB( KV+2, J ), 1 )
!
...
!
            IF( J.LT.M )  &
               CALL DSCAL_LIDORT( M-J, ONE / A( J, J ), A( J+1, J ), 1 )
!
...
subroutine dcopy_lidort(n,dx,incx,dy,incy)
!
...
!           Unchanged on exit.
!
!  !      - REAL(KIND=8)   :: array of DIMENSION ( LDC, n ).
...
subroutine dscal_lidort(n,da,dx,incx)
