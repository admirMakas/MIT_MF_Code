!$Id: lidort_geometry.f90,v 1.1 2010/07/30 23:47:04 daven Exp $
! ###########################################################
! #                                                         #
! #                    THE LIDORT FAMILY                    #
! #                                                         #
...
!  Coarse grids of heights, pressures and temperatures
      ...
      REAL(KIND=8)    :: HEIGHTS     (0:MAXLAYERS)
      REAL(KIND=8)    :: PRESSURES   (0:MAXLAYERS)
      REAL(KIND=8)    :: TEMPERATURES(0:MAXLAYERS)
...
!  solar zenith angles at nadir
      ...
      REAL(KIND=8)    :: SZA_LEVEL_OUTPUT(0:MAXLAYERS,MAXBEAMS)
...
!  fine layer gridding for refraction
      ...
      REAL(KIND=8)    :: ZRFINE(LOCAL_MAXLAYERS,0:LOCAL_MAXFINELAYERS)
      REAL(KIND=8)    :: PRFINE(LOCAL_MAXLAYERS,0:LOCAL_MAXFINELAYERS)
      REAL(KIND=8)    :: TRFINE(LOCAL_MAXLAYERS,0:LOCAL_MAXFINELAYERS)

!  local height arrays

      REAL(KIND=8)    :: H(0:LOCAL_MAXLAYERS)
      REAL(KIND=8)    :: DELZ(LOCAL_MAXLAYERS)

!  help variables

      INTEGER         :: N, J, NRFINE, K, ITER, MAXF,IB
      LOGICAL         :: LOOP
      REAL(KIND=8)    :: GM_TOA, TH_TOA, MU_TOA, MU_NEXT
      REAL(KIND=8)    :: Z1, Z0, Z, T1, T0, T, P1, P0, Q1, Q0, Q
      REAL(KIND=8)    :: FU, FL, DEG_TO_RAD

      REAL(KIND=8)    :: LAYER_DIST, MU_PREV, STH1, SINTH1, STH2, SINTH2, LOCAL_SUBTHICK, &
...
!  Standard temperature (K) and pressure (mbar).
      ...
      REAL(KIND=8), PARAMETER  :: T_STANDARD = 273.16D0
      REAL(KIND=8), PARAMETER  :: P_STANDARD = 1013.25D0
      REAL(KIND=8), PARAMETER  :: STP_RATIO = T_STANDARD / P_STANDARD

!  Loschmidt's number (particles/cm2/km).

      REAL(KIND=8), PARAMETER  :: RHO_STANDARD = 2.68675D+24
...
subroutine outgoing_sphergeom_fine                                 &
       ( maxlayers, maxfine, maxpartials,                          & ! Input
         ...
         partials_fineidx, cosscat_up, cosscat_dn, fail, message )   ! Output
...
!  inputs
      ...
      real(kind=8)    :: eradius, heights (0:maxlayers)
      real(kind=8)    :: heights_p(maxpartials)
...
!  Fine level output (geometry)
      ...
      real(kind=8)    :: sunpaths_fine (maxlayers,maxlayers,maxfine)
      real(kind=8)    :: alpha_fine    (maxlayers,maxfine)

!  Partial layer output (geometry)

      integer         :: ntraverse_p(maxpartials)
      real(kind=8)    :: sunpaths_p (maxpartials,maxlayers)
      real(kind=8)    :: alpha_p    (maxpartials)
...
!   Local arrays
      ...
      real(kind=8)    :: radii_fine    (maxlayers,maxfine)
      real(kind=8)    :: radii_p    (maxpartials)
      real(kind=8)    :: lospaths(maxlayers)
      real(kind=8)    :: lospaths_p_up(maxpartials)
      real(kind=8)    :: lospaths_p_dn(maxpartials)
...
!  Local
      ...
      real(kind=8)    :: ksi, cksi, sksi, xicum, tangr, fac
      real(kind=8)    :: ctheta, stheta, calpha, salpha, cphi
      real(kind=8)    :: b, sth0, th0, ks1, sth1, th1, theta_p
...
!  Local arrays associated with fine grid output
      ...
      real(kind=8)    :: difz, dfine1, saf, xicum0, path
      real(kind=8)    :: thetaf(maxlocalfine), xicumf, difa
      real(kind=8)    :: cthetaf(maxlocalfine)
      real(kind=8)    :: sthetaf(maxlocalfine)
      real(kind=8)    :: ksif(maxlocalfine)
...
!  Finish
      ...
      return
end subroutine outgoing_sphergeom_fine 
...
!  inputs
      ...
      real(kind=8)    :: eradius, hsurface
      logical         :: do_adjust_surface
      real(kind=8)    :: alpha_boa (max_vza)
      real(kind=8)    :: theta_boa (max_sza)
      real(kind=8)    :: phi_boa   (max_azm)

!  outputs

      real(kind=8)    :: alpha_ssa (max_vza)
      real(kind=8)    :: theta_ssa (max_vza,max_sza,max_azm)
      real(kind=8)    :: phi_ssa   (max_vza,max_sza,max_azm)
      logical         :: fail
      character*(*)   :: message, trace
...
!  Local
      ...
      real(kind=8)    :: salpha_boa, calpha_boa, sphi_boa
      real(kind=8)    :: stheta_boa, ctheta_boa, cphi_boa
      real(kind=8)    :: ksi, cksi, sksi, xicum, cosscat_up
      real(kind=8)    :: phi_all, alpha_all, theta_all
      real(kind=8)    :: ctheta, stheta, calpha, salpha, cphi
      real(kind=8)    :: b,rssa
      character*2     :: c2
