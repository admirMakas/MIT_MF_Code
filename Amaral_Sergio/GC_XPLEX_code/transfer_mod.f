! $Id: transfer_mod.f,v 1.1.1.1 2009/06/09 21:51:51 daven Exp $
      MODULE TRANSFER_MOD
!
!******************************************************************************
!  Module TRANSFER_MOD contains routines used to copy data from TYPE (XPLEX) to
!  TYPE (XPLEX) arrays after being read from disk.  Also, vertical levels will be
...
!  (6 ) TRANSFER_ZONAL_R4 : Transfers GEOS zonal fields, regrids (TYPE (XPLEX))
!  (7 ) TRANSFER_ZONAL_R8 : Transfers GEOS zonal fields, regrids (TYPE (XPLEX)) 
...
!  (10) TRANSFER_2D_R4    : Transfers GEOS 2-D   fields (TYPE (XPLEX) argument)
!  (11) TRANSFER_2D_R8    : Transfers GEOS 2-D   fields (TYPE (XPLEX) argument)
!  (12) TRANSFER_TO_1D    : Transfers GEOS 2-D   fields to a 1-D array
!  (13) LUMP_2_R4         : Combines 2 levels into 1 thick level (TYPE (XPLEX)) 
!  (14) LUMP_2_R8         : Combines 2 levels into 1 thick level (TYPE (XPLEX)) 
!  (15) LUMP_4_R4         : Combines 4 levels into 1 thick level (TYPE (XPLEX))
!  (16) LUMP_4_R8         : Combines 4 levels into 1 thick level (TYPE (XPLEX))
...
!
      ...
      ! Arrays
      TYPE (XPLEX), ALLOCATABLE :: EDGE_IN(:)
...
!
!******************************************************************************
!  Subroutine TRANSFER_A6 transfers A-6 data from a TYPE (XPLEX) array to a COMPLEX*16
!  array.  Vertical layers are collapsed (from LGLOB to LLPAR) if necessary.
!  (mje, bmy, 9/21/01, 11/6/08)
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) IN  (TYPE (XPLEX)) : Input field,  dimensioned (IGLOB,JGLOB,LGLOB)
!
!  Arguments as Output:
!  ============================================================================
!  (2 ) OUT (TYPE (XPLEX)) : Output field, dimensioned (LLPAR,IIPAR,JJPAR)
!
!  NOTES:
...
#     include "CMN_SIZE"    ! Size parameters
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: IN(IGLOB,JGLOB,LGLOB)
      TYPE (XPLEX),  INTENT(OUT) :: OUT(LLPAR,IIPAR,JJPAR)

      ! Local variables
      INTEGER              :: I, J, L
      TYPE (XPLEX)               :: INCOL(LGLOB)
...
!
!******************************************************************************
!  Subroutine TRANSFER_3D transfers A-6 data from a TYPE (XPLEX) array to a COMPLEX*16
!  array.  Vertical layers are collapsed (from LGLOB to LLPAR) if necessary.
!  (mje, bmy, 9/21/01, 2/8/07)
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) IN  (TYPE (XPLEX)) : Input field,  of dimension (IGLOB,JGLOB,LGLOB)
!
!  Arguments as Output:
!  ============================================================================
!  (2 ) OUT (TYPE (XPLEX)) : Output field, of dimension (IIPAR,JJPAR,LLPAR)
!
!  NOTES:
...
#     include "CMN_SIZE"    ! Size parameters
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: IN(IIPAR,JJPAR,LGLOB)
      TYPE (XPLEX),  INTENT(OUT) :: OUT(IIPAR,JJPAR,LLPAR)
...
#     include "CMN_SIZE"    ! Size parameters
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: IN(IIPAR,JJPAR,LGLOB+1)
      TYPE (XPLEX),  INTENT(OUT) :: OUT(IIPAR,JJPAR,LLPAR+1)
...
!
!******************************************************************************
!  Subroutine TRANSFER_3D_Lp1 transfers 3-D data from a TYPE (XPLEX) array of 
!  dimension (IGLOB,JGLOB,LGLOB+1) to a TYPE (XPLEX) array of dimension 
!  (IIPAR,JJPAR,LLPAR+1).  Regrid in the vertical if needed.
!  (bmy, 9/21/01, 2/8/07)
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) IN  (TYPE (XPLEX)) : Input field,  of dimension (IGLOB,JGLOB,LGLOB+1)
!
!  Arguments as Output:
!  ============================================================================
!  (2 ) OUT (TYPE (XPLEX)) : Output field, of dimension (IIPAR,JJPAR,LLPAR+1)
!
!  NOTES:
!******************************************************************************
!
#     include "CMN_SIZE"
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: IN(IGLOB,JGLOB,LGLOB+1)
      TYPE (XPLEX),  INTENT(OUT) :: OUT(IIPAR,JJPAR,LLPAR+1)
...
!
!******************************************************************************
!  Subroutine TRANSFER_3D_TROP transfers tropospheric 3-D data from a TYPE (XPLEX) 
!  array to a TYPE (XPLEX) array. (mje, bmy, 9/21/01, 2/8/07)
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) IN  (TYPE (XPLEX)) : Input field,  of dimension (IGLOB,JGLOB,LLTROP)
!
!  Arguments as Output:
!  ============================================================================
!  (2 ) OUT (TYPE (XPLEX)) : Output field, of dimension (IIPAR,LLPAR,LLTROP)
!
!  NOTES:
...
#     include "CMN_SIZE"    ! Size parameters
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: IN(IGLOB,JGLOB,LLTROP_FIX)
      TYPE (XPLEX),  INTENT(OUT) :: OUT(IIPAR,JJPAR,LLTROP_FIX)
      ...
      ! Cast to TYPE (XPLEX) abd resize up to LLTROP
      DO L = 1, LLTROP_FIX
...
!
!******************************************************************************
!  Subroutine TRANSFER_ZOJAL_R4 transfers zonal-mean data from a TYPE (XPLEX) array 
!  to a TYPE (XPLEX) array.  Vertical levels are collapsed (from LGLOB to LLPAR) if 
!  necessary. (mje, bmy, 9/21/01, 2/8/07)
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) IN  (TYPE (XPLEX)) : Input  field, of dimension (JGLOB,LGLOB)
!
!  Arguments as Output:
!  ============================================================================
!  (2 ) OUT (TYPE (XPLEX)) : Output field, of dimension (JJPAR,LLPAR)
!
!  NOTES:
!  (1 ) Lump levels together in groups of 2 or 4, as dictated by Mat Evans.
!        (bmy, 9/21/01)
...
#     include "CMN_SIZE"    ! Size parameters
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: IN(JGLOB,LGLOB)
      TYPE (XPLEX),  INTENT(OUT) :: OUT(JJPAR,LLPAR)
...
!  TYPE (XPLEX) array of dimension (JGLOB,LGLOB) to a COMPLEX*16 array of dimension 
...
!
!******************************************************************************
!  Subroutine TRANSFER_2D_INT transfers 2-D data from a TYPE (XPLEX) array of 
!  dimension (IGLOB,JGLOB) to an INTEGER array of dimension (IIPAR,JJPAR). 
!  (bmy, 9/21/01, 3/11/03)
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) IN  (TYPE (XPLEX) ) : Input field,  of dimension (IGLOB,JGLOB)
!
!  Arguments as Output:
!  ============================================================================
...
#     include "CMN_SIZE"
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: IN(IGLOB,JGLOB)
...
!
!******************************************************************************
!  Subroutine TRANSFER_2D_R4 transfers 2-D data from a TYPE (XPLEX) array of 
!  dimension (IGLOB,JGLOB) to a TYPE (XPLEX) array of dimension (IIPAR,JJPAR). 
!  (bmy, 1/25/02, 3/11/03)
!
!  Arguments as Input:
!  ============================================================================
...
!  Arguments as Output:
!  ============================================================================
!  (2 ) OUT (TYPE (XPLEX)) : Output field, of dimension (IIPAR,JJPAR)
!
!  NOTES:
!  (1 ) Use parallel DO loops to speed things up (bmy, 9/21/01)
!  (2 ) Now use functions GET_XOFFSET and GET_YOFFSET from "grid_mod.f"
...
#     include "CMN_SIZE"
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: IN(IGLOB,JGLOB)
      TYPE (XPLEX),  INTENT(OUT) :: OUT(IIPAR,JJPAR)
...
!
!******************************************************************************
!  Subroutine TRANSFER_2D_R8 transfers 2-D data from a TYPE (XPLEX) array of 
!  dimension (IGLOB,JGLOB) to a TYPE (XPLEX) array of dimension (IIPAR,JJPAR). 
!  (bmy, 9/21/01, 3/11/03)
!
!  Arguments as Input:
...
!
!******************************************************************************
!  Subroutine TRANSFER_TO_1D transfers 2-D data from a TYPE (XPLEX) array of 
!  dimension (IGLOB,JGLOB) to 1-D a TYPE (XPLEX) array of dimension (MAXIJ),
...
!  Arguments as Output:
!  ============================================================================
!  (2 ) OUT (TYPE (XPLEX)) : Output field, of dimension (MAXIJ)
!
!  NOTES:
...
#     include "CMN_SIZE"
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: IN(IGLOB,JGLOB)
      TYPE (XPLEX),  INTENT(OUT) :: OUT(MAXIJ)
...
!  Input arguments must be TYPE (XPLEX). (bmy, 9/18/01, 10/31/03)
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) IN   (TYPE (XPLEX) ) : Column of data on input vertical grid
!  (2 ) L_IN (INTEGER) : Vertical dimension of the IN array
!  (3 ) L    (INTEGER) : Level on input grid from which to start regridding
!
!  Function Value:
!  ============================================================================
!  (4 ) OUT  (TYPE (XPLEX) ) : Data on output grid -- 2 levels merged together
! 
!  NOTES:
...
!  Function Value:
!  ============================================================================
!  (4 ) OUT  (TYPE (XPLEX) ) : Data on output grid -- 4 levels merged together
!
!  NOTES:
!  (1 ) Now references GEOS_CHEM_STOP from "error_mod.f" (bmy, 10/15/02)
!  (2 ) Renamed SIGE_IN to EDGE_IN to denote that it is not always a sigma
