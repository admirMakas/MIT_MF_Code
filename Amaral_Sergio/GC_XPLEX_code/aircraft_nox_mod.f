! $Id: aircraft_nox_mod.f,v 1.1.1.1 2009/06/09 21:51:54 daven Exp $
...
!  (3 ) AIR      (TYPE (XPLEX) ) : Array for NOx emissions on the 1-km grid
!  (4 ) AIREMIS  (TYPE (XPLEX) ) : Array for NOx emissions on the GEOS-CHEM grid
!  (5 ) AIRPRESS (TYPE (XPLEX) ) : Approx. pressure edges of the 1-km native grid
!  (6 ) EMIS_AC_NOx (TYPE (XPLEX)) : Array to pass aircraft NOx to SMVGEAR
!
!  Module Routines:
!  ============================================================================
...
!
      ...
      TYPE (XPLEX),  ALLOCATABLE :: AIR(:,:,:)
               
      ! Aircraft NOx emissions on GEOS-CHEM grid
      TYPE (XPLEX),  ALLOCATABLE :: AIREMIS(:,:,:)

      ! AIRPRESS - Approx. pressure edges of the 1-km native aircraft NOx grid
      TYPE (XPLEX)               :: AIRPRESS(NAIR+1)!%r = (/  
!     &  1013.25d0,954.61d0,845.56d0,746.83d0,657.64d0,577.28d0,505.07d0, 
!     &  440.35d0, 382.52d0,330.99d0,285.24d0,244.75d0,209.04d0,178.65d0, 
!     &  152.59d0, 130.34d0,111.33d0,95.09d0,81.22d0,69.37d0,59.26d0 /)

      ! Array to pass aircraft NOx emissions to SMVGEAR
      ! (replacement for GEMISNOX array)
      TYPE (XPLEX),  ALLOCATABLE :: EMIS_AC_NOx(:,:,:)
      AIRPRESS(:)=0d0
      AIRPRESS(:)%r = (/
     &  1013.25d0,954.61d0,845.56d0,746.83d0,657.64d0,577.28d0,505.07d0, 
     &  440.35d0, 382.52d0,330.99d0,285.24d0,244.75d0,209.04d0,178.65d0, 
...
!        I/O errors.  Use D exponent to force TYPE (XPLEX) precision. Also updated 
!        comments. (bmy, 7/5/01)
