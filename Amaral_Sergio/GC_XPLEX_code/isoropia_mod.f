! $Id: isoropia_mod.f,v 1.1.1.1 2009/06/09 21:51:53 daven Exp $
      MODULE ISOROPIA_MOD
!
!******************************************************************************
...
!  (26 ) TINYY             : A very small number (1e-20)
!  (27 ) TINYY2            : Another very small number (1e-11)
...
!
      ...
      !=================================================================
      INTERFACE KMTAB
        MODULE PROCEDURE KMTAB_R
        MODULE PROCEDURE KMTAB_C
      END INTERFACE
      ...
      ! TYPE (XPLEX) parameters
      TYPE (XPLEX),  PARAMETER   :: EPS     = xplex(1.d-3,0d0)
      TYPE (XPLEX),  PARAMETER   :: EPSACT  = xplex(5.d-2,0d0)        
      TYPE (XPLEX),  PARAMETER   :: GREAT   = xplex(1.d10,0d0)
      TYPE (XPLEX),  PARAMETER   :: ONE     = xplex(1.0d0,0d0)
      TYPE (XPLEX),  PARAMETER   :: R       = xplex(82.0567d-6,0d0)
      TYPE (XPLEX),  PARAMETER   :: TINYY    = xplex(1d-20,0d0)
      TYPE (XPLEX),  PARAMETER   :: TINYY2   = xplex(1.d-11,0d0)
      TYPE (XPLEX),  PARAMETER   :: ZERO    = xplex(0.0d0,0d0)

      ! Allocatable arrays
      TYPE (XPLEX),  ALLOCATABLE :: BNC198(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: BNC223(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: BNC248(:,:)      
      TYPE (XPLEX),  ALLOCATABLE :: BNC273(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: BNC298(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: BNC323(:,:)
       TYPE (XPLEX),  ALLOCATABLE :: ASRAT(:)
       TYPE (XPLEX),  ALLOCATABLE :: ASSO4(:)
      TYPE (XPLEX),  ALLOCATABLE :: HNO3_sav(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: GAS_HNO3(:,:,:)
...
#     include "isoropia.h"      ! ISOROPIA common blocks
      ...
      INTEGER                  :: I,    J,    L,    N
      TYPE (XPLEX)                   :: ANO3, GNO3, RHI,  TEMPI
      TYPE (XPLEX)                   :: TNa,  TCl,  TNH3, TNH4
      TYPE (XPLEX)                   :: TNIT, TNO3, TSO4, VOL
      TYPE (XPLEX)                   :: AERLIQ(NIONS+NGASAQ+2)
      TYPE (XPLEX)                   :: AERSLD(NSLDS) 
      TYPE (XPLEX)                   :: GAS(NGASAQ) 
      TYPE (XPLEX)                   :: OTHER(NOTHER)
      TYPE (XPLEX)                   :: WI(NCOMP)    
      TYPE (XPLEX)                   :: WT(NCOMP)
      CHARACTER(LEN=255)       :: X 
...
!
      ...
      ! Return value
      TYPE (XPLEX), INTENT(OUT)  :: HNO3_kg ! [kg]
...
#     include "isoropia.h"  ! ISOROPIA common blocks
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: RHI, TEMPI
      TYPE (XPLEX),  INTENT(IN)  :: WI(NCOMP)
      TYPE (XPLEX),  INTENT(OUT) :: WT(NCOMP)
      TYPE (XPLEX),  INTENT(OUT) :: GAS(NGASAQ)
      TYPE (XPLEX),  INTENT(OUT) :: AERSLD(NSLDS) 
      TYPE (XPLEX),  INTENT(OUT) :: AERLIQ(NIONS+NGASAQ+2)
      TYPE (XPLEX),  INTENT(OUT) :: OTHER(NOTHER)
      ...
      IF ( WI(1) < TINYY .AND. WI(5) < TINYY ) THEN 
      ...
      ! For reverse problem
      IF ( IPROB > 0 .AND. WATER > TINYY ) THEN 
         WT(3) = WT(3) + GNH3 
         WT(4) = WT(4) + GHNO3
         WT(5) = WT(5) + GHCL
      ENDIF
...
#     include "isoropia.h"  ! ISOROPIA common blocks
      ...
      ! Local variables
      INTEGER              :: K,  IRH
      TYPE (XPLEX)               :: IC, GII, GI0, XX 
      TYPE (XPLEX) , PARAMETER   :: LN10=xplex(2.3025851d0,0d0)
      TYPE (XPLEX)               :: T0, T0T, COEF, TCF, G130, G13I
...
#     include "isoropia.h" 
      ...
      ! Local variables ??
      INTEGER           :: K,  IRH
      TYPE (XPLEX)            :: IC, GII, GI0, XX, T0T
!      TYPE (XPLEX)            :: TEMP, RH
      TYPE (XPLEX)            :: TCF, G130, G13I, COEF
      TYPE (XPLEX), PARAMETER :: LN10 = xplex(2.3025851D0,0d0)
      TYPE (XPLEX), PARAMETER :: T0   = xplex(298.15D0,0d0)
...
!
      ! Arguments
      TYPE (XPLEX), INTENT(IN)  :: SO4I, RHI

      ! Local variables
      INTEGER            :: IA1,   INDS,  INDR, INDSL
      INTEGER            :: INDSH, IPOSL, IPOSH
      TYPE (XPLEX)             :: GETASR, RAT,  A1, WF
...
#     include "isoropia.h" 
      ...
      ! Local variables
      TYPE (XPLEX) :: KAPA, X, DELT, ALFA, DIAK
...
#     include "isoropia.h" 
      ...
      ! Local variables 
      TYPE (XPLEX) :: A1, CHI1, CHI2, BB, CC, DIAK, PSI
      ...
      PSI  =  MAX(TINYY, MIN(PSI,CHI1))! Constrict in acceptible range
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX) :: NO3I, HI, DELT

      ! Local variables
      TYPE (XPLEX) :: A42, OM1, OM2, BB, CC, DD, DEL1, DEL2
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX) :: NO3I, NO3AQ, GGNO3, HI 

      ! Local variables
      TYPE (XPLEX) :: A42, AKW, ALF1, ALF2, ALF3, BB, CC, DEL1
      ...
      NO3I  = ALF1 + DEL1
      HI    = ALF3 + DEL1
      IF (HI.LE.TINYY) HI = SQRT(AKW)   ! If solution is neutral.
      NO3AQ = ALF2 - DEL1
...
#     include "isoropia.h" 
      ...
      ! Local variables
      TYPE (XPLEX)      :: SO4I, HSO4I, AML5, TOTS4, FRNH4, FRNO3, FRCL
      INTEGER          :: K
      CHARACTER(LEN=1) :: SC
      TYPE (XPLEX)   :: CHI1, CHI2, CHI3, CHI4, CHI5, CHI6, CHI7, CHI8,
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX)  :: RHI, RHDRY, RHLIQ
      EXTERNAL   DRYCASE, LIQCASE

      ! Local variables
      TYPE (XPLEX)  :: WF, ONEMWF
      TYPE (XPLEX)  :: CNH42SO, CNH4HSO, CLCO, CNH4N3O, CNH4CLO, CNA2SO 
      TYPE (XPLEX)  :: CNAHSO, CNANO, CNACLO, GNH3O, GHNO3O, GHCLO   
      TYPE (XPLEX)  :: DAMSUL, DSOSUL, DAMBIS, DSOBIS, DLC, DAMNIT
      TYPE (XPLEX)  :: DAMCHL, DSONIT, DSOCHL, DAMG, DHAG, DNAG
      INTEGER :: K
...
#     include "isoropia.h" 
      ...
      TYPE (XPLEX)  :: WF, ONEMWF
      TYPE (XPLEX)  :: CNH42SO, CNH4HSO, CLCO, CNH4N3O, CNH4CLO, CNA2SO 
      TYPE (XPLEX)  :: CNAHSO, CNANO, CNACLO
      TYPE (XPLEX)  :: DAMBIS, DSOBIS, DLC, A8
      TYPE (XPLEX)  :: HIEQ, HIEN, A2, A3, A4
      INTEGER :: K
     ...
     &          MOLAL(2) - MOLAL(3)
      ...
      A2      = (XK2/XKW)*R*TEMP*(GAMA(10)/GAMA(5))**2.d0 ! NH3  <==> NH4+
      A3      = XK4 *R*TEMP*(WATER/GAMA(10))**2.d0        ! HNO3 <==> NO3-
      A4      = XK3 *R*TEMP*(WATER/GAMA(11))**2.d0        ! HCL  <==> CL-

      GNH3    = MOLAL(3)/MAX(MOLAL(1),TINYY)/A2
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX) :: HI, SO4I, HSO4I, DELTA

      ! Local variables
      TYPE (XPLEX) :: A8, BB, CC, DD, SQDD, DELTA1, DELTA2
      !CHARACTER ERRINF*40
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX) :: GG, HI, OHI

      ! Local variables
      TYPE (XPLEX) :: AKW, CN, BB, DD, CC
...
#     include "isoropia.h" 
      ...
      ! Local variables
      TYPE (XPLEX) , PARAMETER :: URF=xplex(0.5d0,0d0)
!      TYPE (XPLEX)             :: EX10
      TYPE (XPLEX)       :: G0(3,4),ZPL,ZMI,AGAMA,SION,H,CH,F1(3),F2(4)
!      TYPE (XPLEX)             :: TEMP
!      INTEGER            :: I, J
      INTEGER            :: K, M
      TYPE (XPLEX)             :: MPL, XIJ, YJI, ERROU, ERRIN
      ...
      IONIC = MAX(MIN(0.5d0*IONIC/WATER,20.d0), TINYY)
...
!         GAMA(I) = EX10(SNGL(GAMA(I)), 5.d0)          ! CUTOFF SET TO [-5,5]
         GAMA(K) = EX10(GAMA(K),xplx( 5.d0)  )        ! CUTOFF SET TO [-5,5]
...
!
      ! Arguments 
      TYPE (XPLEX) :: IONIC, TEMP
      TYPE (XPLEX) :: G01, G02, G03, G04, G05, G06
      TYPE (XPLEX) :: G07, G08, G09, G10, G11, G12

      ! Local variables
      TYPE (XPLEX) :: Z01, Z02, Z03, Z04, Z05, Z06
      TYPE (XPLEX) :: Z07, Z08, Z09, Z10, Z11, SION
      TYPE (XPLEX) :: TI,  CF1, CF2
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE KMTAB_R( IN,  TEMP, G01, G02, G03, G04, G05,
     &                  G06, G07,  G08, G09, G10, G11, G12 )
!
!******************************************************************************
!
...
!
      ! Arguments
      TYPE (XPLEX), INTENT(IN)  :: IN
      REAL*8, INTENT(IN) :: TEMP
      TYPE (XPLEX), INTENT(OUT) :: G01, G02, G03, G04, G05, G06
      TYPE (XPLEX), INTENT(OUT) :: G07, G08, G09, G10, G11, G12
      ...
      ! Return to calling program
      END SUBROUTINE KMTAB_R
 
      SUBROUTINE KMTAB_C( IN,  TEMP, G01, G02, G03, G04, G05,
     &                  G06, G07,  G08, G09, G10, G11, G12 )
!
!******************************************************************************
!
...
!     'BINARR' IS RETURNED WITH THE BINARY COEFFICIENTS. 
!
! *** COPYRIGHT 1996-2000, UNIVERSITY OF MIAMI, CARNEGIE MELLON
! UNIVERSITY
! *** WRITTEN BY ATHANASIOS NENES
!
!******************************************************************************
...
!
      ...
      TYPE (XPLEX),             INTENT(OUT) :: ARRAY(IMAX,JMAX)
...
!  ======================= ARGUMENTS / USAGE =============================
!
!  VAR        is the TYPE (XPLEX) variable which value is to be saved 
!  DEF        is a TYPE (XPLEX) variable, with the default value of VAR. !       
...
!#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX)            :: A1, A2, A3, ROOT
      INTEGER           :: ISLV

      ! Local variables
      TYPE (XPLEX), PARAMETER :: EXPON = xplex(1.D0 / 3.D0,0d0)
      TYPE (XPLEX), PARAMETER :: ZERO  = xplex(0.D0,0d0)
      TYPE (XPLEX), PARAMETER :: THET1 = xplex(120.D0 / 180.D0,0d0) 
      TYPE (XPLEX), PARAMETER :: THET2 = xplex(240.D0 / 180.D0,0d0)
      TYPE (XPLEX), PARAMETER :: PI    = xplex(3.14159265358932d0,0d0)
      TYPE (XPLEX), PARAMETER :: EPS   = xplex(1D-50,0d0)
      TYPE (XPLEX)            :: R
      TYPE (XPLEX)      :: X(3), D, Q, THET, COEF, SSIG, S, TSIG, T, SQD
      INTEGER           :: I, IX
...
!
      ! Arguments
      INTEGER            :: ISLV
      TYPE (XPLEX)             :: A1, A2, A3, RTLW, RTHI, ROOT
      ...
      INTEGER            :: I, K1, K2
      TYPE (XPLEX),  PARAMETER :: ZERO=xplex(0.0D0,0d0)
      TYPE (XPLEX),  PARAMETER :: EPS=xplex(1.0D-15,0d0)
      TYPE (XPLEX)             :: X1, Y1, DX, X2, Y2, Y3, X3
...
!
      ! Arguments
      TYPE (XPLEX), INTENT(IN) :: A1, A2, A3, X
...
!      TYPE (XPLEX)             :: EX10, Y
      ...
      ! For integer part
       TYPE (XPLEX), SAVE       :: AINT10(20) = (/
     & xplex(0.1000d-08,0d0), xplex(0.1000d-07,0d0), 
     & xplex(0.1000d-06,0d0), xplex(0.1000d-05,0d0), 
     & xplex(0.1000d-04,0d0),
     & xplex(0.1000d-03,0d0), xplex(0.1000d-02,0d0),
     & xplex(0.1000d-01,0d0), xplex(0.1000d+00,0d0),
     & xplex(0.1000d+01,0d0),
     & xplex(0.1000d+02,0d0), xplex(0.1000d+03,0d0),
     & xplex(0.1000d+04,0d0), xplex(0.1000d+05,0d0),
     & xplex(0.1000d+06,0d0),
     & xplex(0.1000d+07,0d0), xplex(0.1000d+08,0d0),
     & xplex(0.1000d+09,0d0), xplex(0.1000d+10,0d0),
     & xplex(0.1000d+11,0d0)/)

      ! For decimal part
       TYPE (XPLEX), SAVE       :: ADEC10(200) = (/
     & xplex(0.1023d+00,0d0), xplex(0.1047d+00,0d0),
     & xplex(0.1072d+00,0d0), xplex(0.1096d+00,0d0),
     & xplex(0.1122d+00,0d0),
     & xplex(0.1148d+00,0d0), xplex(0.1175d+00,0d0),
     & xplex(0.1202d+00,0d0), xplex(0.1230d+00,0d0),
     & xplex(0.1259d+00,0d0),
     & xplex(0.1288d+00,0d0), xplex(0.1318d+00,0d0),
     & xplex(0.1349d+00,0d0), xplex(0.1380d+00,0d0),
     & xplex(0.1413d+00,0d0),
     & xplex(0.1445d+00,0d0), xplex(0.1479d+00,0d0),
     & xplex(0.1514d+00,0d0), xplex(0.1549d+00,0d0),
     & xplex(0.1585d+00,0d0),
     & xplex(0.1622d+00,0d0), xplex(0.1660d+00,0d0),
     & xplex(0.1698d+00,0d0), xplex(0.1738d+00,0d0),
     & xplex(0.1778d+00,0d0),
     & xplex(0.1820d+00,0d0), xplex(0.1862d+00,0d0),
     & xplex(0.1905d+00,0d0), xplex(0.1950d+00,0d0),
     & xplex(0.1995d+00,0d0),
     & xplex(0.2042d+00,0d0), xplex(0.2089d+00,0d0),
     & xplex(0.2138d+00,0d0), xplex(0.2188d+00,0d0),
     & xplex(0.2239d+00,0d0),
     & xplex(0.2291d+00,0d0), xplex(0.2344d+00,0d0),
     & xplex(0.2399d+00,0d0), xplex(0.2455d+00,0d0),
     & xplex(0.2512d+00,0d0),
     & xplex(0.2570d+00,0d0), xplex(0.2630d+00,0d0),
     & xplex(0.2692d+00,0d0), xplex(0.2754d+00,0d0),
     & xplex(0.2818d+00,0d0),
     & xplex(0.2884d+00,0d0), xplex(0.2951d+00,0d0),
     & xplex(0.3020d+00,0d0), xplex(0.3090d+00,0d0),
     & xplex(0.3162d+00,0d0),
     & xplex(0.3236d+00,0d0), xplex(0.3311d+00,0d0),
     & xplex(0.3388d+00,0d0), xplex(0.3467d+00,0d0),
     & xplex(0.3548d+00,0d0),
     & xplex(0.3631d+00,0d0), xplex(0.3715d+00,0d0),
     & xplex(0.3802d+00,0d0), xplex(0.3890d+00,0d0),
     & xplex(0.3981d+00,0d0),
     & xplex(0.4074d+00,0d0), xplex(0.4169d+00,0d0),
     & xplex(0.4266d+00,0d0), xplex(0.4365d+00,0d0),
     & xplex(0.4467d+00,0d0),
     & xplex(0.4571d+00,0d0), xplex(0.4677d+00,0d0),
     & xplex(0.4786d+00,0d0), xplex(0.4898d+00,0d0),
     & xplex(0.5012d+00,0d0),
     & xplex(0.5129d+00,0d0), xplex(0.5248d+00,0d0),
     & xplex(0.5370d+00,0d0), xplex(0.5495d+00,0d0),
     & xplex(0.5623d+00,0d0),
     & xplex(0.5754d+00,0d0), xplex(0.5888d+00,0d0),
     & xplex(0.6026d+00,0d0), xplex(0.6166d+00,0d0),
     & xplex(0.6310d+00,0d0),
     & xplex(0.6457d+00,0d0), xplex(0.6607d+00,0d0),
     & xplex(0.6761d+00,0d0), xplex(0.6918d+00,0d0),
     & xplex(0.7079d+00,0d0),
     & xplex(0.7244d+00,0d0), xplex(0.7413d+00,0d0),
     & xplex(0.7586d+00,0d0), xplex(0.7762d+00,0d0),
     & xplex(0.7943d+00,0d0),
     & xplex(0.8128d+00,0d0), xplex(0.8318d+00,0d0),
     & xplex(0.8511d+00,0d0), xplex(0.8710d+00,0d0),
     & xplex(0.8913d+00,0d0),
     & xplex(0.9120d+00,0d0), xplex(0.9333d+00,0d0),
     & xplex(0.9550d+00,0d0), xplex(0.9772d+00,0d0),
     & xplex(0.1000d+01,0d0),
     & xplex(0.1023d+01,0d0), xplex(0.1047d+01,0d0),
     & xplex(0.1072d+01,0d0), xplex(0.1096d+01,0d0),
     & xplex(0.1122d+01,0d0),
     & xplex(0.1148d+01,0d0), xplex(0.1175d+01,0d0),
     & xplex(0.1202d+01,0d0), xplex(0.1230d+01,0d0),
     & xplex(0.1259d+01,0d0),
     & xplex(0.1288d+01,0d0), xplex(0.1318d+01,0d0),
     & xplex(0.1349d+01,0d0), xplex(0.1380d+01,0d0),
     & xplex(0.1413d+01,0d0),
     & xplex(0.1445d+01,0d0), xplex(0.1479d+01,0d0),
     & xplex(0.1514d+01,0d0), xplex(0.1549d+01,0d0),
     & xplex(0.1585d+01,0d0),
     & xplex(0.1622d+01,0d0), xplex(0.1660d+01,0d0),
     & xplex(0.1698d+01,0d0), xplex(0.1738d+01,0d0),
     & xplex(0.1778d+01,0d0),
     & xplex(0.1820d+01,0d0), xplex(0.1862d+01,0d0),
     & xplex(0.1905d+01,0d0), xplex(0.1950d+01,0d0),
     & xplex(0.1995d+01,0d0),
     & xplex(0.2042d+01,0d0), xplex(0.2089d+01,0d0),
     & xplex(0.2138d+01,0d0), xplex(0.2188d+01,0d0),
     & xplex(0.2239d+01,0d0),
     & xplex(0.2291d+01,0d0), xplex(0.2344d+01,0d0),
     & xplex(0.2399d+01,0d0), xplex(0.2455d+01,0d0),
     & xplex(0.2512d+01,0d0),
     & xplex(0.2570d+01,0d0), xplex(0.2630d+01,0d0),
     & xplex(0.2692d+01,0d0), xplex(0.2754d+01,0d0),
     & xplex(0.2818d+01,0d0),
     & xplex(0.2884d+01,0d0), xplex(0.2951d+01,0d0),
     & xplex(0.3020d+01,0d0), xplex(0.3090d+01,0d0),
     & xplex(0.3162d+01,0d0),
     & xplex(0.3236d+01,0d0), xplex(0.3311d+01,0d0),
     & xplex(0.3388d+01,0d0), xplex(0.3467d+01,0d0),
     & xplex(0.3548d+01,0d0),
     & xplex(0.3631d+01,0d0), xplex(0.3715d+01,0d0),
     & xplex(0.3802d+01,0d0), xplex(0.3890d+01,0d0),
     & xplex(0.3981d+01,0d0),
     & xplex(0.4074d+01,0d0), xplex(0.4169d+01,0d0),
     & xplex(0.4266d+01,0d0), xplex(0.4365d+01,0d0),
     & xplex(0.4467d+01,0d0),
     & xplex(0.4571d+01,0d0), xplex(0.4677d+01,0d0),
     & xplex(0.4786d+01,0d0), xplex(0.4898d+01,0d0),
     & xplex(0.5012d+01,0d0),
     & xplex(0.5129d+01,0d0), xplex(0.5248d+01,0d0),
     & xplex(0.5370d+01,0d0), xplex(0.5495d+01,0d0),
     & xplex(0.5623d+01,0d0),
     & xplex(0.5754d+01,0d0), xplex(0.5888d+01,0d0),
     & xplex(0.6026d+01,0d0), xplex(0.6166d+01,0d0),
     & xplex(0.6310d+01,0d0),
     & xplex(0.6457d+01,0d0), xplex(0.6607d+01,0d0),
     & xplex(0.6761d+01,0d0), xplex(0.6918d+01,0d0),
     & xplex(0.7079d+01,0d0),
     & xplex(0.7244d+01,0d0), xplex(0.7413d+01,0d0),
     & xplex(0.7586d+01,0d0), xplex(0.7762d+01,0d0),
     & xplex(0.7943d+01,0d0),
     & xplex(0.8128d+01,0d0), xplex(0.8318d+01,0d0),
     & xplex(0.8511d+01,0d0), xplex(0.8710d+01,0d0),
     & xplex(0.8913d+01,0d0),
     & xplex(0.9120d+01,0d0), xplex(0.9333d+01,0d0),
     & xplex(0.9550d+01,0d0), xplex(0.9772d+01,0d0),
     & xplex(0.1000d+02,0d0)
     & /)
...
#     include "isoropia.h" 
     ...
     &                  'MIGHT AFFECT SO4/HSO4 RATIO'
      ...
      ELSE IF ( IERR .EQ. 0020 ) THEN
         IF (W(4).GT.TINYY .AND. W(5).GT.TINYY) THEN
     ...
     &                  'DISSOLUTION'
         WRITE(IO,1000) 'A TINYY AMOUNT [',ERRINF(1:IEND),'] IS '//
...
#     include "isoropia.h" 
      ...
      TYPE (XPLEX),  INTENT(IN) :: WI(NCOMP), RHI, TEMPI
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER :: K
      TYPE (XPLEX)  :: AK1, BET, GAM, BB, CC, DD
     ...
     &             M0(9)+MOLALR(4)/M0(4)
      ...
      DO K = 1, NSWEEP
         ...
         MOLAL (6) = MAX(TINYY,MIN(W(2)-MOLAL(5),
     &                     W(2)))         ! HSO4I
         MOLAL (1) = MAX(TINYY,MIN(AK1*MOLAL(6)/
     &                     MOLAL(5),W(2))) ! HI
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX) :: X, Y, TLC, TNH42S4, TNH4HS4
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX)  :: TLC, TNH42S4

      ! Local variables
      INTEGER :: K
!      TYPE (XPLEX)  :: FUNCB3A
      TYPE (XPLEX)  :: ZLO, ZHI, Z1, Z2, Z3, Y1, Y2, Y3,YLO, YHI, DZ, ZK
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER :: K
      TYPE (XPLEX)  :: KK, GRAT1, DD!, FUNCB3A
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX), INTENT(IN)  :: Y, X

      ! Local variables
      INTEGER            :: K
      TYPE (XPLEX)             :: KK, GRAT1, DD
      ...
      DO K = 1, NSWEEP
         ...
         MOLAL (1) = KK                   ! HI
         MOLAL (5) = Y+KK                 ! SO4I
         MOLAL (6) = MAX (X+Y-KK, TINYY)   ! HSO4I
...
#     include "isoropia.h" 
      ...
      ! CALCULATE EQUIVALENT AMOUNT OF HSO4 AND SO4
      X = MAX(2*W(2)-W(3), TINYY)   ! Equivalent NH4HSO4
      Y = MAX(W(3)  -W(2), TINYY)   ! Equivalent NH42SO4
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: TLC, TNH42S4
...
#     include "isoropia.h" 
      ...
      ! Local variables
      TYPE (XPLEX)  :: WF, ONEMWF, CLCO, CNH42SO
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX)  :: TLC, TNH4HS4

      ! 
      INTEGER :: K
!      TYPE (XPLEX)  :: FUNCB2B
      TYPE (XPLEX)  :: ZLO, ZHI, X1, X2, X3, Y1, Y2, Y3, YLO, YHI, DX
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX), INTENT(IN)  :: X, TNH4HS4, TLC
      ...
      ! Local variables
      INTEGER :: K
      TYPE (XPLEX)  :: GRAT2, PARM, DELTA, OMEGA!, FUNCB2B
      ...
      DO K = 1, NSWEEP
         ...
         MOLAL (6) = MAX (X+TNH4HS4-OMEGA, TINYY)   ! HSO4I
...
#     include "isoropia.h" 
      ...
      ! Local variables
      TYPE (XPLEX) :: WF, ONEMWF, CLCO, CNH42SO, CNH4HSO
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER :: K
      TYPE (XPLEX)  :: LAMDA, KAPA, PSI, PARM, BB, CC
      ...
      DO K = 1, NSWEEP
         ...
         MOLAL(3) = LAMDA                                  ! NH4I
         MOLAL(5) = KAPA                                   ! SO4I
         MOLAL(6) = MAX(LAMDA+PSI-KAPA, TINYY)              ! HSO4I
...
! Local variables
      TYPE (XPLEX)  :: KLO, KHI, X1, X2, X3, Y1, Y2, Y3
!      TYPE (XPLEX)  :: FUNCC1, YLO, YHI, DX
      TYPE (XPLEX)  :: YLO, YHI, DX
      INTEGER :: K
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER            :: K
      TYPE (XPLEX)             :: LAMDA, PSI, PAR1, PAR2, BB, CC!, FUNCC1
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER :: K
      TYPE (XPLEX)  :: PSI4LO, PSI4HI, X1,X2, X3, Y1,Y2,Y3, YLO, YHI, DX
      TYPE (XPLEX)  :: P4, YY, DELTA
!      TYPE (XPLEX)  :: FUNCD3
      TYPE (XPLEX)      :: CHI1, CHI2, CHI3, CHI4, CHI5,CHI6,CHI7,CHI8,
     &               PSI1, PSI2, PSI3, PSI4, PSI5, PSI6, PSI7, PSI8,
     &               A1,   A2,   A3,   A4,   A5,   A6,   A7,   A8
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER :: K
      TYPE (XPLEX)  :: BB, DENM, ABB, AHI!, FUNCD3
      TYPE (XPLEX)      :: CHI1, CHI2, CHI3, CHI4, CHI5,CHI6,CHI7,CHI8,
     &               PSI1, PSI2, PSI3, PSI4, PSI5, PSI6, PSI7, PSI8,
     &               A1,   A2,   A3,   A4,   A5,   A6,   A7,   A8
      COMMON /SOLUT/ CHI1, CHI2, CHI3, CHI4, CHI5, CHI6, CHI7, CHI8,
...
!###        AHI  =2.0*A7/(BB+SQRT(BB*BB + 4.d0*A7)) ! Avoid overflow when HI->0
         ...
         IF (DENM.LE.TINYY) THEN       ! Avoid overflow when HI->0
            ABB  = ABS(BB)
      ...
      !=================================================================
20    CONTINUE
      VALUE= MOLAL(3)/MOLAL(1)/MAX(GNH3,TINYY)/A4 - ONE 
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER            :: K, ISLV
      TYPE (XPLEX)             :: PSI14, BB, DENM, ABB, AHI!, FUNCD2
      TYPE (XPLEX)   :: CHI1, CHI2, CHI3, CHI4, CHI5, CHI6, CHI7, CHI8,
     &               PSI1, PSI2, PSI3, PSI4, PSI5, PSI6, PSI7, PSI8,
     &               A1,   A2,   A3,   A4,   A5,   A6,   A7,   A8
      COMMON /SOLUT/ CHI1, CHI2, CHI3, CHI4, CHI5, CHI6, CHI7, CHI8,
...
!$OMP THREADPRIVATE( /SOLUT/ )
      ...
      DO K = 1, NSWEEP
         ...
         IF ( CHI2 .GT. TINYY .AND. WATER .GT. TINYY ) THEN
            PSI14 = PSI1+PSI4

            CALL POLY3 (PSI14,0.25d0*PSI14**2.d0,-A2/4.D0, PSI2, ISLV)  ! PSI2
            IF ( ISLV.EQ.0 ) THEN
                PSI2 = MIN (PSI2, CHI2)
            ELSE
                PSI2 = ZERO
...
#     include "isoropia.h" ! CNH42S4, CNH4NO3, GNH3, GHNO3
      ...
      ! Local variables
      TYPE (XPLEX)  :: PARM, X, PS, OM, OMPS, DIAK, ZE
...
#     include "isoropia.h" 
      ...
      ! Local variables
      TYPE (XPLEX)  :: LAMDA, LAMDA1, LAMDA2, KAPA, KAPA1, KAPA2
      TYPE (XPLEX)  :: ALF, BET, GAM, RTSQ, THETA1, THETA2, BB, CC
      TYPE (XPLEX)  :: DD, DQDD, DD1, DD2, SQDD1, SQDD2, SQDD
      TYPE (XPLEX)  :: A1, A2
...
#     include "isoropia.h" 
      ...
      IF (W(4).GT.TINYY) THEN        ! NO3 EXISTS, WATER POSSIBLE
         SCASE = 'G2 ; SUBCASE 1'  
         CALL CALCG2A
         SCASE = 'G2 ; SUBCASE 1' 
...
#     include "isoropia.h" ! ISOROPIA common blocks
      ...
      ! Local variables
      INTEGER ::     ISLV, K
      TYPE (XPLEX)  ::     PSI6LO, PSI6HI, X1, Y1, DX, X2, Y2
      TYPE (XPLEX)  ::     X3, Y3, DELTA

      ! Local common blocks
      TYPE (XPLEX)  ::     CHI1, CHI2, CHI3, CHI4, CHI5, CHI6, LAMDA,
...
!$OMP THREADPRIVATE( /CASEG/ )
      ...
      X1 = PSI6LO
      Y1 = FUNCG2A (X1)
      IF (CHI6.LE.TINYY) GOTO 50  
      ! Comment out
      !IF (ABS(Y1).LE.EPS .OR. CHI6.LE.TINYY) GOTO 50  
      !IF (WATER .LE. TINYY) GOTO 50               ! No water

      !=================================================================
      ! Root tracking ; for the range of HI and LO
      !=================================================================
      DX = (PSI6HI-PSI6LO)/XPLX(NDIV)
      DO 10 K=1,NDIV
         X2 = X1+DX 
         Y2 = FUNCG2A (X2)
         IF (SIGN(1.d0,Y1)*SIGN(1.d0,Y2).LT.ZERO) GOTO 20  ! (Y1*Y2.LT.ZERO)
 ...
 40   CONTINUE
      X3 = 0.5d0*(X1+X2)
      IF (X3.LE.TINYY2) THEN   ! PRACTICALLY NO NITRATES, SO DRY SOLUTION
         WATER = TINYY
      ELSE
         Y3 = FUNCG2A (X3)
      ENDIF
 ...
 50   CONTINUE
      ...
      ! Na2SO4 dissolution
      IF (CHI1.GT.TINYY .AND. WATER.GT.TINYY) THEN        ! PSI1
      ...
      ! HSO4 equilibrium
      IF (MOLAL(1).GT.TINYY .AND. MOLAL(5).GT.TINYY) THEN
         CALL CALCHS4 (MOLAL(1), MOLAL(5), 
     &                 ZERO, DELTA)
...
#     include "isoropia.h" ! ISOROPIA common blocks
      ...
      INTEGER ::     K, ISLV
      TYPE (XPLEX)  ::     VALUE
      TYPE (XPLEX)  ::     DENO, PSI20, SMIN, HI, OHI, DELT
      TYPE (XPLEX)  ::     BB, CC, DD, PSI31, PSI32
...
!$OMP THREADPRIVATE( /CASEG/ )
      ...
      DO 10 K=1,NSWEEP
         ...
         MOLAL (7) = PSI5              ! NO3I
         ! Comment out
         !MOLAL (1) = MAX(CHI5 - PSI5, TINYY)*A5/PSI5   ! HI
     ...
     &               MOLAL(4)-MOLAL(2)-MOLAL(3)
         ...
         !==============================================================
         GNH3      = MAX(CHI4 - PSI4, TINYY)
         GHNO3     = MAX(CHI5 - PSI5, TINYY)
         GHCL      = MAX(CHI6 - PSI6, TINYY)
         ...
         ! Calculate gas / solid species (liquid in MOLAL already)
         !=============================================================
         GNH3    = MAX(GNH3 - PSI3, TINYY)
         GHCL    = MAX(GHCL - PSI3, TINYY)
         CNH4CL  = PSI3
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER ::     K, ISLV
      TYPE (XPLEX)  ::     PSI6LO, PSI6HI, X1, Y1, DX
      TYPE (XPLEX)  ::     X2, Y2, X3, Y3, DELTA
...
!$OMP THREADPRIVATE( /CASEG/ )
      ...
      PSI6LO = TINYY                  
      PSI6HI = CHI6-TINYY    ! MIN(CHI6-TINYY, CHI4)
      ...
      IF (CHI6.LE.TINYY) GOTO 50  
      ! comment out
      !IF (ABS(Y1).LE.EPS .OR. CHI6.LE.TINYY .OR. WATER .LE. TINYY) GOTO 50
      !IF (WATER .LE. TINYY) RETURN                    ! No water
...
#     include "isoropia.h"  ! ISOROPIA common blocks
      ...
      ! Local variables
      INTEGER ::     K, ISLV
      TYPE (XPLEX)  ::     BB, CC, DD, PSI20, SMIN, HI, OHI
      TYPE (XPLEX)  ::     VALUE
...
!$OMP THREADPRIVATE( /CASEG/ )
      ...
      !=================================================================
      IF (CHI5.GE.TINYY) THEN
         PSI5 = PSI6*CHI5/(A6/A5*(CHI6-PSI6) + PSI6)
      ELSE
         PSI5 = TINYY
      ENDIF

      !IF(CHI4.GT.TINYY) THEN
      IF(W(2).GT.TINYY) THEN       
     ...
     &            MOLAL(4)-MOLAL(2)-MOLAL(3)
      ...
      GNH3      = MAX(CHI4 - PSI4, TINYY)              ! Gas NH3
      GHNO3     = MAX(CHI5 - PSI5, TINYY)              ! Gas HNO3
      GHCL      = MAX(CHI6 - PSI6, TINYY)              ! Gas HCl
...
#     include "isoropia.h"  ! ISOROPIA common blocks
      ...
      ! Local variables
      INTEGER ::     K, ISLV
      TYPE (XPLEX)  ::     BB, CC, DD, CLI, SO4I, HI, OHI
      TYPE (XPLEX)  ::     NAI, NH4I, NO3I, VALUE
...
10    CONTINUE
 ...
 40   CONTINUE
      ...
      !=================================================================
 50   CONTINUE
      IF (MOLAL(1).GT.TINYY .AND. MOLAL(5).GT.TINYY) THEN  ! If quadrat.called
         CALL CALCHS4 (MOLAL(1), MOLAL(5), ZERO, DELTA)
         MOLAL(1) = MOLAL(1) - DELTA              ! H+   EFFECT
         MOLAL(5) = MOLAL(5) - DELTA              ! SO4  EFFECT
         MOLAL(6) = DELTA                         ! HSO4 EFFECT
...
#     include "isoropia.h"  
      ...
      ! Local variables
      INTEGER ::     K
      TYPE (XPLEX)  ::     AKK, BB, CC, DD, SMIN, HI, OHI
      TYPE (XPLEX)  ::     VALUE
...
#     include "isoropia.h" 
      ...
      ! Local variables
      TYPE (XPLEX):: LAMDA, LAMDA1,LAMDA2,KAPA,KAPA1,KAPA2, NAFR, NO3FR
      TYPE (XPLEX)  :: CLFR, ALF, BET, GAM, RTSQ, THETA1, THETA2
      TYPE (XPLEX)  :: BB, CC, DD, SQDD, DD1, DD2, SQDD1, SQDD2
      TYPE (XPLEX)  :: A1, A2
...
#     include "isoropia.h" 
      ...
      IF ( WATER .LE. TINYY .AND. RHB .LT. DRMH2 ) THEN      

         ! DRY AEROSOL
         SCASE = 'H2 ; SUBCASE 2'  

      ELSE IF ( WATER .LE. TINYY .AND. RHB .GE. DRMH2 ) THEN  
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER ::      K
      TYPE (XPLEX)  ::     FRNA, PSI6LO, PSI6HI, X1, Y1, DX
      TYPE (XPLEX)  ::     X2, Y2, X3, Y3, DELTA
...
#     include "isoropia.h" 
      ...
      INTEGER ::     K, ISLV
      TYPE (XPLEX)  ::     VALUE
      TYPE (XPLEX)  ::     A64, A9, BB, CC, DD, DIAK, AA
      TYPE (XPLEX)  ::     SMIN, HI, OHI, DELT, PSI31, PSI32
...
!$OMP THREADPRIVATE( /SOLUT/ )
      ...
      DO 10 K=1,NSWEEP
         ...
         IF (CHI1.GT.TINYY .AND. WATER.GT.TINYY) THEN 
         ...
         IF (CHI7.GT.TINYY .AND. WATER.GT.TINYY) THEN 
            ! NACL DISSOLUTION
         ...
         IF (CHI8.GT.TINYY .AND. WATER.GT.TINYY) THEN     
            ! NANO3 DISSOLUTION
...
!$OMP THREADPRIVATE( /SOLUT/ )
      ...
      ! REGIME DEPENDS ON THE EXISTANCE OF NITRATES 
      IF ( W(4) .LE. TINYY ) THEN    ! NO3 NOT EXIST, WATER NOT POSSIBLE
         SCASE = 'H3'  
         CALL CALCH1A
         SCASE = 'H3'  
         RETURN
...
#     include "isoropia.h" 
      ...
      INTEGER ::     K, ISLV
      TYPE (XPLEX)  ::     VALUE
      TYPE (XPLEX)  ::     A9, BB, CC, DD, DIAK, AA, SMIN, HI, OHI
      TYPE (XPLEX)  ::     DELT, PSI31, PSI32
...
C
 ...
 40   CONTINUE
      ...
      !=================================================================
 50   CONTINUE
      IF ( MOLAL(1) .GT. TINYY .AND. 
     &     MOLAL(5) .GT. TINYY ) THEN
         CALL CALCHS4 (MOLAL(1), MOLAL(5), ZERO, DELTA)
         MOLAL(1) = MOLAL(1) - DELTA          ! H+   EFFECT
         MOLAL(5) = MOLAL(5) - DELTA          ! SO4  EFFECT
         MOLAL(6) = DELTA                     ! HSO4 EFFECT
...
#     include "isoropia.h" 
      ...
      INTEGER ::     K, ISLV
      TYPE (XPLEX)  ::     VALUE
      TYPE (XPLEX)  ::     A9, BB, CC, DD, AA, SMIN, HI, OHI, DELT
      TYPE (XPLEX)  ::     PSI31, PSI32
...
#     include "isoropia.h" 
      ...
      TYPE (XPLEX)  ::     VALUE
      INTEGER ::     K, ISLV
      TYPE (XPLEX)  ::     A9, BB, CC, DD, AA, SMIN, HI, OHI
...
#     include "isoropia.h" 
      ...
      INTEGER ::     K
      TYPE (XPLEX)  ::     VALUE
      TYPE (XPLEX)  ::     A9, BB, CC, DD, SMIN, HI, OHI
...
#     include "isoropia.h" 
      ...
      IF ( FRSO4 .LE. TINYY ) THEN
         CLC     = MAX(CLC - FRNH4, ZERO)
         CNH42S4 = 2.D0*FRNH4

      ELSEIF ( FRNH4 .LE. TINYY ) THEN
         CNH4HS4 = 3.D0*MIN(FRSO4, CLC)
         CLC     = MAX(CLC-FRSO4, ZERO)
         IF (CNA2SO4.GT.TINYY) THEN
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER ::     K
      TYPE (XPLEX)  ::     PSI2LO, PSI2HI, X1, Y1, YHI, DX
      TYPE (XPLEX)  ::     X2, Y2, X3, Y3
...
#     include "isoropia.h" 
      ...
      ! Arguments
      TYPE (XPLEX), INTENT(IN) :: P2

      ! Local variables
      INTEGER ::     K, ISLV
      TYPE (XPLEX)  ::     VALUE
      TYPE (XPLEX)  ::     AA, BB, CC, DD
...
!$OMP THREADPRIVATE( /SOLUT/ )
      ...
      DO 10 K=1,NSWEEP
         ...
         ! CALCULATE DISSOCIATION QUANTITIES
         IF ( CHI5 .GT. TINYY .AND. WATER .GT. TINYY ) THEN     
            PSI5 = (PSI3 + 2.D0*PSI4 - A7*(3.D0*PSI2 + PSI1))/2.D0/A7 
            PSI5 = MAX(MIN (PSI5, CHI5), TINYY)
         ENDIF

         IF ( CHI4 .GT. TINYY .AND. WATER .GT. TINYY ) THEN     
         ...
         IF (CHI3.GT.TINYY .AND. WATER.GT.TINYY) THEN     
...
#     include "isoropia.h" 
      ...
      IF ( CNH4HS4 .GT. TINYY .OR. CNAHSO4 .GT. TINYY ) THEN
         ! FULL SOLUTION
         SCASE = 'I3 ; SUBCASE 1'  
         CALL CALCI3A                     
         SCASE = 'I3 ; SUBCASE 1'  
...
#     include "isoropia.h" 
      ...
      INTEGER ::     K
      TYPE (XPLEX)  ::     VALUE
      TYPE (XPLEX)  ::     PSI4LO, PSI4HI, X1, Y1, YHI, DX
      TYPE (XPLEX)  ::     BB, CC, DD, X2, Y2, X3, Y3
...
!$OMP THREADPRIVATE( /SOLUT/ )
      ...
      DO 10 K=1,NSWEEP
         ...
         MOLAL(3) = 3.D0*PSI2 + 2.D0*PSI5 + PSI1          ! NH4I
         MOLAL(5) = PSI2 + PSI4 + PSI5 + PSI6             ! SO4I
         MOLAL(6) = MAX(PSI2 + PSI3 + PSI1 - PSI6, TINYY)  ! HSO4I
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER ::     K
      TYPE (XPLEX)  ::     PSI4LO, PSI4HI, Y1, X1, YHI, DX
      TYPE (XPLEX)  ::     X2, Y2, YLO, Y3, X3
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER ::     K
      TYPE (XPLEX)  ::     PSI1LO, PSI1HI, X1,  Y1, YHI, DX
      TYPE (XPLEX)  ::     X2,     Y2,     YLO, Y3, X3

      ! Local common blocks
      TYPE (XPLEX)  ::  CHI1,CHI2, CHI3, LAMDA, KAPA, PSI1, PSI2, PSI3, 
     &               A1,   A2,   A3
...
!$OMP THREADPRIVATE( /CASEJ/ )
      ...
      LAMDA  = MAX(W(2) - W(3) - W(1), TINYY)  ! FREE H2SO4
      PSI1   = P1
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER ::     K
      TYPE (XPLEX)  ::     PSI1LO, PSI1HI, X1, Y1, DX
      TYPE (XPLEX)  ::     X2, Y2, YL0, Y3, X3, YHI, YLO
...
#     include "isoropia.h" 
      ...
      ! Local variables
      INTEGER :: K
      TYPE (XPLEX)  :: LAMDA, KAPA
      TYPE (XPLEX)  :: BB, CC, DD
      TYPE (XPLEX)  :: CHI1, CHI2, PSI1, PSI2, A3
...
#     include "isoropia.h" 
      ...
      INTEGER  :: ISLV
      TYPE (XPLEX)   :: DELCL, DELNO, OMEGA, C1, C2, C3
      TYPE (XPLEX)   :: M1, M2, M3
      TYPE (XPLEX)   :: A3, A4, CHI3, CHI4
      CHARACTER ERRINF*40 
      ...
      ELSE IF ( W(5) .LE. TINYY .AND. W(4) .LE. TINYY ) THEN
         GOTO 60

      ! SPECIAL CASE; HCL=ZERO
      ELSE IF ( W(5) .LE. TINYY ) THEN
         CALL CALCNA              ! CALL HNO3 DISSOLUTION ROUTINE
         GOTO 60

      ! SPECIAL CASE; HNO3=ZERO 
      ELSE IF ( W(4) .LE. TINYY ) THEN
      ...
      IF ( ISLV .NE. 0 ) THEN
         DELCL = TINYY           ! TINYY AMOUNTS OF HCL ASSUMED WHEN NO ROOT 
         WRITE (ERRINF,'(1PE7.1)') TINYY
...
!      ENDIF
 ...
 50   CONTINUE
      ...
      ! EFFECT ON GAS PHASE
 55   CONTINUE
      GHCL     = MAX(W(5) - MOLAL(4), TINYY)
      GHNO3    = MAX(W(4) - MOLAL(7), TINYY)
