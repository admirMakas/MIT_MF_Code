...
!  (3 ) MAX_RELERR  (TYPE (XPLEX) )  : Max error for total-tag error check [unitless]
!  (4 ) MAX_ABSERR  (TYPE (XPLEX) )  : Max abs error for total-tag err chk [unitless]
!  (5 ) MAX_FLXERR  (TYPE (XPLEX) )  : Max error tol for flux error check  [unitless]
!  (6 ) CDEEP       (TYPE (XPLEX) )  : Conc. of Hg0, Hg2, HgC below MLD    [pM      ]
!  (7 ) DD_Hg2      (TYPE (XPLEX) )  : Array for Hg(II) dry dep'd to ocean [kg      ]
!  (8 ) dMLD        (TYPE (XPLEX) )  : Array for Change in ocean MLD       [cm      ]
!  (9 ) Hg0aq       (TYPE (XPLEX) )  : Array for ocean mass of Hg(0)       [kg      ]
!  (10) Hg2aq       (TYPE (XPLEX) )  : Array for ocean mass of Hg(II)      [kg      ]
!  (11) HgC         (TYPE (XPLEX) )  : Array for ocean mass of HgC         [kg      ]
!  (12) MLD         (TYPE (XPLEX) )  : Array for instantaneous ocean MLD   [cm      ]
!  (13) MLDav       (TYPE (XPLEX) )  : Array for monthly mean ocean MLD    [cm      ]
!  (14) newMLD      (TYPE (XPLEX) )  : Array for next month's ocean MLD    [cm      ]
!  (15) NPP         (TYPE (XPLEX) )  : Array for mean net primary prod.    [unitless]
!  (16) RAD         (TYPE (XPLEX) )  : Array for mean solar radiation      [W/m2    ]
!  (17) UPVEL       (TYPE (XPLEX) )  : Array for ocean upwelling velocity  [m/s     ]
!  (18) WD_Hg2      (TYPE (XPLEX) )  : Array for Hg(II) wet dep'd to ocean [kg      ]
!
!  Module Routines:
!  ============================================================================
...
!
      ...
      ! Parameters
      TYPE (XPLEX),  PARAMETER   :: MAX_RELERR = xplex(5.0d-2,0d0)
      TYPE (XPLEX),  PARAMETER   :: MAX_ABSERR = xplex(5.0d-3,0d0)
      TYPE (XPLEX),  PARAMETER   :: MAX_FLXERR = xplex(5.0d-1,0d0) 
      TYPE (XPLEX),  PARAMETER :: CDEEP(3)   = (/ xplex(6d-11,0d0), 
     & xplex(5d-10,0d0), xplex(5d-10,0d0) /) 
      TYPE (XPLEX),  PARAMETER   :: SMALLNUM   = xplex(1d-32,0d0)

      ! Arrays
      TYPE (XPLEX),  ALLOCATABLE :: DD_Hg2(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: dMLD(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: Hg0aq(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: Hg2aq(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: HgC(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: MLD(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: MLDav(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: newMLD(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: NPP(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: RAD(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: UPVEL(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: WD_Hg2(:,:,:)
...
!  (4 ) DRY_Hg2 (TYPE (XPLEX) ) : Hg(II) dry deposited out of the atmosphere [kg]
!
!  NOTES:
...
!  (2 ) J       (INTEGER) : GEOS-CHEM latitude  index
!  (3 ) N       (INTEGER) : GEOS-CHEM tracer    index
!  (4 ) WET_Hg2 (TYPE (XPLEX) ) : Hg(II) scavenged out of the atmosphere
!
!  NOTES:
!  (1 ) DD_Hg2 is now a 3-D array.  Also pass N via the argument list. Now 
!        call GET_Hg2_CAT to return the Hg category #. (cdh, bmy, 3/28/06)
...
!  Arguments as Output
!  ============================================================================
!  (1 ) FLUX (TYPE (XPLEX)) : Flux of Hg(0) from the ocean [kg/s]
!
!  NOTES:
...
#     include "CMN_DEP"       ! FRCLND
      ...
      ! Arguments 
      TYPE (XPLEX),  INTENT(OUT)  :: FLUX(IIPAR,JJPAR,N_Hg_CATS)
      ...
      TYPE (XPLEX)              :: A_M2,     DTSRCE,   MLDCM,    MLDCMC
      TYPE (XPLEX)                :: CHg0aq,   CHg0
      TYPE (XPLEX)                :: K1,       Kc,       Ks,       Kw
      TYPE (XPLEX)                :: Kcon,     Ksink,    TC,       TK       
      TYPE (XPLEX)                :: Sc,       ScCO2,    USQ,      MHg
      TYPE (XPLEX)            :: Hg2_RED,  Hg2_GONE, Hg2_CONV, HgC_SUNK
      TYPE (XPLEX)              :: FRAC_L,   FRAC_O,   H,        TOTDEP
      TYPE (XPLEX)                :: EF,       oldMLD,   XTAU
      TYPE (XPLEX)                :: E_CONV,   E_SINK,   E_RED
      TYPE (XPLEX)                :: DIFFUSION(3) 

      ! Conversion factor from [cm/h * ng/L] --> [kg/m2/s]
      TYPE (XPLEX),  PARAMETER    :: TO_KGM2S =xplex(1.0D-11/3600D0,0d0) 
...
!  (1 ) I      (INTEGER) : GEOS-CHEM longitude index
!  (2 ) J      (INTEGER) : GEOS-CHEM latitude index
!  (3 ) MLDold (TYPE (XPLEX) ) : Old ocean mixed layer depth [m]
!  (4 ) MLDnew (TYPE (XPLEX) ) : New ocean mixed layer depth [m]
!
!  NOTES:
!******************************************************************************
!
...
#     include "CMN_DEP"      ! FRCLND
      ...
      TYPE (XPLEX),  INTENT(IN)   :: MLDold, MLDnew  

      ! Local variables
      INTEGER               :: C,    NN,     N_tot_oc    
      TYPE (XPLEX)                :: A_M2, DELTAH, FRAC_O,  MHg
...
#     include "CMN_SIZE"     ! Size parameters
      ...
      TYPE (XPLEX)                :: Hg_OCEAN(IIPAR,JJPAR,1)
      CHARACTER(LEN=255)    :: FILENAME
...
#     include "CMN_SIZE"     ! Size parameters
      ...
      INTEGER               :: IFIRST,    JFIRST,   LFIRST
      INTEGER               :: N,         NN
      TYPE (XPLEX)          :: LONRES,    LATRES,   ARRAY(IGLOB,JGLOB,1)
      CHARACTER(LEN=20)     :: MODELNAME
...
#     include "CMN_SIZE"            ! Size parameters
      ...
      INTEGER                      :: I,       J,       L
      INTEGER                      :: N,       NN
      TYPE (XPLEX)                :: Hg0_tot, Hg0_tag, RELERR0, ABSERR0      
      TYPE (XPLEX)                :: Hg2_tot, Hg2_tag, RELERR2, ABSERR2
      TYPE (XPLEX)                :: HgP_tot, HgP_tag, RELERRP, ABSERRP
...
#     include "CMN_SIZE"            ! Size parameters
      ...
      LOGICAL                      :: FLAG
      INTEGER                      :: I,         J
      TYPE (XPLEX)                       :: DD_tot,    DD_tag 
      TYPE (XPLEX)                       :: DD_RELERR, DD_ABSERR      
      TYPE (XPLEX)                       :: WD_tot,    WD_tag
      TYPE (XPLEX)                       :: WD_RELERR, WD_ABSERR
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) FLUX (TYPE (XPLEX))   : Flux array (output of OCEAN_MERCURY_FLUX)
...
#     include "CMN_SIZE"            ! Size parameters
      ...
      ! Arguments
      TYPE (XPLEX),           INTENT(IN) :: FLUX(IIPAR,JJPAR,N_Hg_CATS)            
      CHARACTER(LEN=*), INTENT(IN) :: LOC

      ! Local variables
      LOGICAL                      :: FLAG
      INTEGER                      :: I,          J
      TYPE (XPLEX)                       :: FLX_tot,    FLX_tag
      TYPE (XPLEX)                       :: FLX_RELERR, FLX_ABSERR
