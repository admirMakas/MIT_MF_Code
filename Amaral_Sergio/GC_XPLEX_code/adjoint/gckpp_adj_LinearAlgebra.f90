!$Id: gckpp_adj_LinearAlgebra.f90,v 1.2 2009/06/12 01:44:48 daven Exp $
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! 
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE gckpp_adj_JacobianSP
      ...
      INTEGER  :: IER
      TYPE (XPLEX) :: JVS(LU_NONZERO), W(NVAR), a
      INTEGER  :: k, kk, j, jj
!      where (abs(dimag(JVS))<1d-99) JVS =dcmplx(dble(JVS),0d0)
!      where (abs(dimag(W))<1d-99) W =dcmplx(dble(W),0d0)
!      if (abs(dimag(a))<1d-99) a =dcmplx(dble(a),0d0)
  
      a = 0.d0 ! mz_rs_20050606
      IER = 0
      DO k=1,NVAR
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE gckpp_adj_JacobianSP
      ...
      INTEGER       :: IER
      TYPE (XPLEX) :: JVSR(LU_NONZERO), JVSI(LU_NONZERO) 
      TYPE (XPLEX) :: WR(NVAR), WI(NVAR), ar, ai, den
      INTEGER       :: k, kk, j, jj
!      where (abs(dimag(JVSR))<1d-99) JVSR =dcmplx(dble(JVSR),0d0)
!      where (abs(dimag(WR))<1d-99) WR =dcmplx(dble(WR),0d0)
!      if (abs(dimag(ar))<1d-99) ar =dcmplx(dble(ar),0d0)
!      if (abs(dimag(den))<1d-99) den =dcmplx(dble(den),0d0)
!      where (abs(dimag(JVSI))<1d-99) JVSI =dcmplx(dble(JVSI),0d0)
!      where (abs(dimag(WI))<1d-99) WI =dcmplx(dble(WI),0d0)
!      if (abs(dimag(ai))<1d-99) ai =dcmplx(dble(ai),0d0)
      IER = 0
      ar  = 0.d0
      DO k=1,NVAR
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE gckpp_adj_JacobianSP
      ...
      INTEGER  :: i, j
      TYPE (XPLEX) :: JVS(LU_NONZERO), X(NVAR), sum
!      where (abs(dimag(JVS))<1d-99) JVS =dcmplx(dble(JVS),0d0)
!      where (abs(dimag(X))<1d-99) X =dcmplx(dble(X),0d0)
      DO i=1,NVAR
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE gckpp_adj_JacobianSP
      ...
      INTEGER       :: i, j
      TYPE (XPLEX) :: JVS(LU_NONZERO), X(NVAR)
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE gckpp_adj_JacobianSP
      ...
      INTEGER       ::  i, j
      TYPE (XPLEX)::JVSR(LU_NONZERO),JVSI(LU_NONZERO),XR(NVAR),XI(NVAR),sumr,sumi,den
...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE gckpp_adj_JacobianSP
      ...
      INTEGER       ::  i, j
      TYPE (XPLEX) ::  JVSR(LU_NONZERO), JVSI(LU_NONZERO), XR(NVAR), XI(NVAR), den
...
!      complex\(kind\=dp\) :: JVS(3,3,LU_NONZERO), W(3,3,NVAR), a(3,3), E(3,3)
!      INTEGER  :: k, kk, j, jj
!
!      a = 0.0d0
!      IER = 0
...
!      complex\(kind\=dp\) :: JVS(3,3,LU_NONZERO), X(3,NVAR), sum(3)
!
!      DO i=1,NVAR
!        DO j = LU_CROW(i), LU_DIAG(i)-1 
...
!      complex\(kind\=dp\) :: JVS(3,3,LU_NONZERO), X(3,NVAR)
!
!      DO i=1,NVAR
!        ! X(i) = X(i)/JVS(LU_DIAG(i))
...
!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!!
!      complex\(kind\=dp\) :: A(3,3)
!      INTEGER       :: IPVT(3),INFO
!!      INTEGER       :: L
!!      complex\(kind\=dp\) :: t, dmax, da, TMP(3)
!      complex\(kind\=dp\), PARAMETER :: ZERO = 0.0, ONE = 1.0
!
!      info = 0
!!      t = TINY(da)
!!      
...
SUBROUTINE KppSolve ( JVS, X )
  ...
  INTEGER::i
! JVS - sparse Jacobian of variables
  TYPE (XPLEX) :: JVS(LU_NONZERO)
! X - Vector for variables
  TYPE (XPLEX) :: X(NVAR)
!  where (abs(dimag(JVS))<1d-99) JVS =dcmplx(dble(JVS),0d0)
!  where (abs(dimag(X))<1d-99) X =dcmplx(dble(X),0d0)
!  if(abs(minval(X%i))>1e-200.or.abs(maxval(X%i))>1e-200.or.abs(minval(JVS%i))>1e-200.or.abs(maxval(JVS%i))>1e-200) then
!  print*,'minr maxr mini maxi beg kpp solve',minval(X%r),maxval(X%r),minval(X%i),maxval(X%i), &
!        & minval(JVS%r),maxval(JVS%r),minval(JVS%i),maxval(JVS%i)
!  CALL GEOS_CHEM_STOP
!  endif
...
!  where (abs(dimag(JVS))<1d-99) JVS =dcmplx(dble(JVS),0d0)
!  where (abs(dimag(X))<1d-99) X =dcmplx(dble(X),0d0)
!  if (abs(minval(X%i))>1e-200.or.abs(maxval(X%i))>1e-200) then
!  print*,'minr maxr mini maxi end kppsolve',minval(X%r),maxval(X%r),minval(X%i),maxval(X%i)
!  CALL GEOS_CHEM_STOP
!  endif      
...
!      where (abs(dimag(JVS))<1d-99) JVS =dcmplx(dble(JVS),0d0)
!  where (abs(dimag(X))<1d-99) X =dcmplx(dble(X),0d0)
!  where (abs(dimag(XX))<1d-99) XX =dcmplx(dble(XX),0d0)
  XX(1) = X(1)/JVS(1)
  XX(2) = X(2)/JVS(3)
  XX(3) = X(3)/JVS(5)
  XX(4) = X(4)/JVS(7)
...
!--------------------------------------------------------------
      ...
      INTEGER  :: i,incX,incY,M,MP1,N
      TYPE (XPLEX) :: X(N),Y(N),Alpha
      TYPE (XPLEX), PARAMETER :: ZERO = xplex(0.0_dp,0d0)
...
!--------------------------------------------------------------
      ...
      TYPE (XPLEX)  :: X(N),Alpha
      TYPE (XPLEX),PARAMETER::ZERO=xplex(0.0_dp,0d0)
      TYPE (XPLEX),PARAMETER::ONE=xplex(1.0_dp,0d0)
...
!      USE gckpp_adj_Precision
      ...
      TYPE (XPLEX)  ::  Suma
      TYPE (XPLEX), PARAMETER::ONE=xplex(1.0_dp,0d0)
      TYPE (XPLEX), PARAMETER::HALF=xplex(0.5_dp,0d0)
...
!--------------------------------------------------------------
      TYPE (XPLEX) FUNCTION WDOT (N, DX, incX, DY, incY) 
!--------------------------------------------------------------
...
!
      ...
      TYPE (XPLEX) :: A(N,N)
      TYPE (XPLEX) :: t, dmax, da
      INTEGER       :: j,k,l
      TYPE (XPLEX), PARAMETER::ZERO=xplex(0.d0,0d0),ONE=xplex(1.d0,0d0)
