!$Id: geos_chem_adj_mod.f,v 1.30 2012/04/13 22:29:08 nicolas Exp $
! =============================================================
!
...
!         NYMD        = GET_NYMD()
     ...
     &      MINVAL(STT_ADJ), MAXVAL(STT_ADJ)
         print*, ' MIN / MAX loc = ', 
     &      MINLOC((STT_ADJ%r)), MAXLOC((STT_ADJ%r))
...
#     include "define_adj.h"    ! obs operators
      ...
      ! Internal variables 
      TYPE (XPLEX)              :: DIFF
      TYPE (XPLEX)              :: NEW_COST(IIPAR,JJPAR,LLPAR,N_TRACERS)
      INTEGER             :: I, J, L, N
      INTEGER             :: ADJ_EXPLD_COUNT
      INTEGER, PARAMETER  ::  MAX_ALLOWED_EXPLD    = 10
      TYPE (XPLEX),  PARAMETER::MAX_ALLOWED_INCREASE = xplex(10D10,0d0)
      TYPE (XPLEX)              :: MAX_ADJ_TMP
      TYPE (XPLEX)              :: TARGET_STT
      TYPE (XPLEX)              :: FACTOR 
      TYPE (XPLEX)              :: CF_PRIOR
      TYPE (XPLEX)              :: CF_TESNH3
      TYPE (XPLEX)              :: CF_TESO3
      TYPE (XPLEX)              :: CF_GOSCO2
      TYPE (XPLEX)              :: CF_MODIS_AOD
      TYPE (XPLEX)              :: CF_IMPRV
      TYPE (XPLEX)              :: CF_TESCH4
      TYPE (XPLEX)              :: CF_SCIACH4
      TYPE (XPLEX)              :: CF_MEMCH4
      TYPE (XPLEX)              :: CF_GEOCAPECH4
      TYPE (XPLEX)              :: CF_LEOCH4
...
#     include "comode.h"             ! IGAS, ITLOOP
      ...
      INTEGER             :: JLOOP
      TYPE (XPLEX)              :: NEW_COST_CSPEC(ITLOOP,NOBS_CSPEC)
      TYPE (XPLEX)              :: NEW_COST_AIR(ITLOOP)
      TYPE (XPLEX)              :: AIR_SUM 
      TYPE (XPLEX),PARAMETER::CONVERT_FAC=xplex(1d3/28.966d0*6.023D23,
     &                                                         0d0)    
      ! Parameter coverning temporal averaging range (total nmber of chem time steps)
      ! Now use NSPAN, set in input.gcadj
      !TYPE (XPLEX),  PARAMETER  :: NTSCHEM     = 24d0 * 30d0 
...
!      
!      ! Local variables
!      TYPE (XPLEX)  :: EMS_SF_ADJ_SAVE(IIPAR,JJPAR,MMSCL,NNEMS)   
!
!      !======================================================================
...
#     include "CMN_SIZE"
      ...
      !TYPE (XPLEX),  ALLOCATABLE     :: APCOST(:,:,:,:)
      !TYPE (XPLEX),  ALLOCATABLE     :: APGRAD(:,:,:,:)
      !TYPE (XPLEX),  ALLOCATABLE     :: ERR_PERCENT(:,:,:)
      !TYPE (XPLEX),  ALLOCATABLE     :: invSa(:,:,:)
      !INTEGER                  :: count
      ...
      !TYPE (XPLEX)                   :: TEMP2(IIPAR,JJPAR,MMSCL,NNEMS)
      TYPE (XPLEX),  ALLOCATABLE     :: APCOST(:,:,:,:)
...
#     include "CMN_SIZE"    ! Size parameters
      ...
      ! Local variables
      CHARACTER(LEN=255)   :: FILENAME
      TYPE (XPLEX)               :: ERR_PERCENT( IIPAR,JJPAR, 2 )
      TYPE (XPLEX)               :: EMS_ERROR( IIPAR,JJPAR, 2 )
