!$Id: gckpp_adj_Jacobian.f90,v 1.2 2009/06/12 01:44:48 daven Exp $
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! 
...
! F - Concentrations of fixed species (local)
  TYPE (XPLEX) :: F(NFIX)
! RCT - Rate constants (local)
  TYPE (XPLEX) :: RCT(NREACT)
! JVS - sparse Jacobian of variables
  TYPE (XPLEX) :: JVS(LU_NONZERO)
!  if(abs(minval(V%i))>1e-200.or.abs(maxval(F%i))>1e-200.or.abs(minval(RCT%i))>1e-200.or.abs(maxval(JVS%i))>1e-200)
!then
!  print*,'minr maxr mini maxi beg jacobian',minval(V%r),maxval(V%r),minval(V%i),maxval(V%i), &
!        & minval(JVS%r),maxval(JVS%r),minval(JVS%i),maxval(JVS%i) &
!        & minval(F%r),maxval(F%r),minval(F%i),maxval(F%i)&
!        & minval(RCT%r),maxval(RCT%r),minval(RCT%i),maxval(RCT%i)
!  CALL GEOS_CHEM_STOP
!  endif
...
! B - Temporary array
  TYPE (XPLEX) :: B(527)
  B=0d0
!  if(abs(minval(V%i))>1e-200.or.abs(maxval(F%i))>1e-200.or.abs(minval(RCT%i))>1e-200.or.abs(maxval(JVS%i))>1e-200) then
!  print*,'minr maxr mini maxi beg jacobian',minval(V%r),maxval(V%r),minval(V%i),maxval(V%i), &
!        & minval(JVS%r),maxval(JVS%r),minval(JVS%i),maxval(JVS%i), &
!        & minval(F%r),maxval(F%r),minval(F%i),maxval(F%i), &
!        & minval(RCT%r),maxval(RCT%r),minval(RCT%i),maxval(RCT%i)
!  CALL GEOS_CHEM_STOP
!  endif
!  where (abs(dimag(V))<1d-99) V =dcmplx(dble(V),0d0)
!  where (abs(dimag(F))<1d-99) F =dcmplx(dble(F),0d0)
!  where (abs(dimag(RCT))<1d-99) RCT =dcmplx(dble(RCT),0d0)
...
!  where (abs(dimag(JVS))<1d-99) JVS =dcmplx(dble(JVS),0d0)    
!if(abs(minval(V%i))>1e-200.or.abs(maxval(F%i))>1e-200.or.abs(minval(RCT%i))>1e-200.or.abs(maxval(JVS%i))>1e-200)then
!  print*,'minr maxr mini maxi begjacobian',minval(V%r),maxval(V%r),minval(V%i),maxval(V%i), &
!       & minval(JVS%r),maxval(JVS%r),minval(JVS%i),maxval(JVS%i), &
!       & minval(F%r),maxval(F%r),minval(F%i),maxval(F%i), &
!       & minval(RCT%r),maxval(RCT%r),minval(RCT%i),maxval(RCT%i)
!  CALL GEOS_CHEM_STOP
...
! JUV - Jacobian times user vector
  TYPE (XPLEX) :: JUV(NVAR)
!  where (abs(dimag(JVS))<1d-99) JVS =dcmplx(dble(JVS),0d0)
!  where (abs(dimag(UV))<1d-99) UV =dcmplx(dble(UV),0d0)
!  where (abs(dimag(JUV))<1d-99) JUV =dcmplx(dble(JUV),0d0)
...
! UV - User vector for variables
  TYPE (XPLEX) :: UV(NVAR)
! JTUV - Jacobian transposed times user vector
  TYPE (XPLEX) :: JTUV(NVAR)
!      where (abs(dimag(JVS))<1d-99) JVS =dcmplx(dble(JVS),0d0)
!  where (abs(dimag(UV))<1d-99) UV =dcmplx(dble(UV),0d0)
!  where (abs(dimag(JTUV))<1d-99) JTUV =dcmplx(dble(JTUV),0d0)
  JTUV(1) = JVS(1)*UV(1)
  JTUV(2) = JVS(3)*UV(2)
  JTUV(3) = JVS(5)*UV(3)
  JTUV(4) = JVS(7)*UV(4)
