...
!  (5 ) P24H     (TYPE (XPLEX) ) : 24-hr avg P(Ox) saved from fullchem run [kg/cm3/s]
!  (6 ) L24H     (TYPE (XPLEX) ) : 24-hr avg L(Ox) saved from fullchem run [ 1/cm3/s]
! 
!  Module Routines:
!  ============================================================================
...
!
      ...
      TYPE (XPLEX),  ALLOCATABLE :: P24H(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: L24H(:,:,:)
...
!  (4  ) POx   (TYPE (XPLEX) ) : P(Ox) in the stratosphere [v/v]
!
!  NOTES:
...
#     include "CMN_DIAG" ! LD65
      ...
      ! Local variables
      TYPE (XPLEX)             :: ARRAY(IGLOB,JGLOB,LD65)
      TYPE (XPLEX)             :: XTAU
...
!  Return Value
!  ============================================================================
!  (4  ) PP    (TYPE (XPLEX))  : Array containing P(Ox) for each tagged tracer
! 
!  NOTES:
!  (1 ) Updated from the old routine "chemo3_split.f" (rch, bmy, 8/20/03)
...
#     include "CMN_DIAG"       ! ND44, ND65, LD65
      ...
      ! Arguments
      INTEGER, INTENT(IN)     :: I, J, L
      TYPE (XPLEX),  INTENT(OUT)    :: PP(IIPAR,JJPAR,LD65,N_TAGGED)
...
#     include "CMN_DIAG"  ! ND44, ND65, LD65
      ...
      INTEGER           :: I, J, L, N
      TYPE (XPLEX)            :: PP(IIPAR,JJPAR,LD65,N_TAGGED)
      TYPE (XPLEX)            :: ND44_TMP(IIPAR,JJPAR,LD65)
      TYPE (XPLEX)            :: DTCHEM,  FREQ,    FLUX
      TYPE (XPLEX)            :: LL,      PL,      Ox_0
      TYPE (XPLEX)            :: Ox_LOST, PBL_MAX, F_UNDER_TOP
