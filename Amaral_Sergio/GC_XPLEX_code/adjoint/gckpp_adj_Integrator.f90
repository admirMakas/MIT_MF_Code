!$Id: gckpp_adj_Integrator.f90,v 1.7 2012/05/09 22:31:56 nicolas Exp $
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! 
...
!           the j-th cost functional w.r.t. Y(1:NVAR), j=1...NADJ
   TYPE (XPLEX)  :: Lambda(NVAR,NADJ)
!~~~> Lambda_R - Sensitivities w.r.t. rate coefficients
!     of reactions JCOEFF(1) ... JCOEFF(NCOEFF)
   TYPE (XPLEX)  :: Lambda_R(NCOEFF,NADJ)
!~~~> Time inteval   
   TYPE (XPLEX), INTENT(IN)  :: TIN  ! TIN  - Start Time
   TYPE (XPLEX), INTENT(IN)  :: TOUT ! TOUT - End Time
!~~~> Tolerances fr adjoint calculations
!     (used only for full continuous adjoint)   
   TYPE (XPLEX), INTENT(IN)  :: ATOL_adj(NVAR,NADJ), RTOL_adj(NVAR,NADJ)
!~~~> Optional input parameters and statistics
   INTEGER,       INTENT(IN),  OPTIONAL :: ICNTRL_U(20)
   TYPE (XPLEX), INTENT(IN),  OPTIONAL :: RCNTRL_U(20)
   INTEGER,       INTENT(OUT), OPTIONAL :: ISTATUS_U(20)
   TYPE (XPLEX), INTENT(OUT), OPTIONAL :: RSTATUS_U(20)

   TYPE (XPLEX) :: RCNTRL(20), RSTATUS(20)
...
!~~~>  Arguments   
   TYPE (XPLEX), INTENT(INOUT) :: Y(NVAR)
   INTEGER, INTENT(IN)          :: NADJ
   TYPE (XPLEX), INTENT(INOUT) :: Lambda(NVAR,NADJ)
   TYPE (XPLEX), INTENT(INOUT) :: Lambda_R(NCOEFF,NADJ)
   TYPE (XPLEX), INTENT(IN)    :: Tstart,Tend
   TYPE (XPLEX), INTENT(IN)    :: AbsTol(NVAR),RelTol(NVAR)
   TYPE (XPLEX), INTENT(IN)    :: AbsTol_adj(NVAR,NADJ), RelTol_adj(NVAR,NADJ)
   INTEGER, INTENT(IN)          :: ICNTRL(20)
   TYPE (XPLEX), INTENT(IN)    :: RCNTRL(20)
   INTEGER, INTENT(INOUT)       :: ISTATUS(20)
   TYPE (XPLEX), INTENT(INOUT) :: RSTATUS(20)
...
!~~~>  Parameters of the Rosenbrock method, up to 6 stages
   ...
   TYPE (XPLEX) :: ros_A(15), ros_C(15), ros_M(6), ros_E(6), &
...
!~~~>  Checkpoints in memory
   ...
   TYPE (XPLEX), DIMENSION(:),   POINTER :: chk_H, chk_T
   TYPE (XPLEX), DIMENSION(:,:), POINTER :: chk_Y, chk_K, chk_J
   TYPE (XPLEX), DIMENSION(:,:), POINTER :: chk_dY, chk_d2Y
!~~~>  Locl variables     
   TYPE (XPLEX) :: Roundoff, FacMin, FacMax, FacRej, FacSafe
   TYPE (XPLEX) :: Hmin, Hmax, Hstart
   TYPE (XPLEX) :: Texit
...
!~~~>   Parameters
   TYPE (XPLEX), PARAMETER :: ZERO = xplex(0.0d0,0d0), ONE  = xplex(1.0d0,0d0)
   TYPE (XPLEX), PARAMETER :: DeltaMin = xplex(1.0d-5,0d0)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   TYPE (XPLEX) :: T, H, Ystage(NVAR*S), K(NVAR*S)
   INTEGER       :: P(NVAR)
#ifdef FULL_ALGEBRA
   TYPE (XPLEX) :: E(NVAR,NVAR)
#else       
   TYPE (XPLEX) :: E(LU_NONZERO)
#endif
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   TYPE (XPLEX) :: T, H, Y(NVAR), dY(NVAR), d2Y(NVAR)
...
!~~~> Output: time at which the solution is returned (T=Tend if success)   
   TYPE (XPLEX), INTENT(OUT) ::  T      
!~~~> Input: tolerances      
   TYPE (XPLEX), INTENT(IN) ::  AbsTol(NVAR), RelTol(NVAR)
!~~~> Output: Error indicator
   INTEGER, INTENT(OUT) :: IERR
! ~~~~ Local variables        
   TYPE (XPLEX) :: Ynew(NVAR), Fcn0(NVAR), Fcn(NVAR) 
   TYPE (XPLEX) :: K(NVAR*ros_S), dFdT(NVAR)
   TYPE (XPLEX), DIMENSION(:), POINTER :: Ystage
#ifdef FULL_ALGEBRA    
   TYPE (XPLEX) :: Jac0(NVAR,NVAR),  Ghimj(NVAR,NVAR)
#else
   TYPE (XPLEX) :: Jac0(LU_NONZERO), Ghimj(LU_NONZERO)
#endif
   TYPE (XPLEX) :: H, Hnew, HC, HG, Fac, Tau 
   TYPE (XPLEX) :: Err, Yerr(NVAR)
...
!~~~>  Initial preparations
   ...
   RejectLastH=.FALSE.
   RejectMoreH=.FALSE.
!   print*,'T,H,Hmin,Hstart,Hmax L1007',T,H,Hmin,Hstart,Hmax
...
!~~~>  Limit H if necessary to avoid going beyond Tend   
   RSTATUS(Nhexit) = H
   H = MIN(H,ABS(Tend-T))
!   print*,'T,H,Hmin,Hstart,Hmax L1025',T,H,Hmin,Hstart,Hmax 
...
!   if(abs(minval(Y%i))>1e-200.or.abs(maxval(Fcn0%i))>1e-200.or.abs(minval(dFdT%i))>1e-200)then
!  print*,'minr maxr mini maxi integrator 1035',minval(Y%r),maxval(Y%r),minval(Y%i),maxval(Y%i), &
!        & minval(Fcn0%r),maxval(Fcn0%r),minval(Fcn0%i),maxval(Fcn0%i), &
!        & minval(dFdT%r),maxval(dFdT%r),minval(dFdT%i),maxval(dFdT%i)
!  CALL GEOS_CHEM_STOP
!  endif   
...
!   if(abs(minval(Y%i))>1e-200.or.abs(maxval(Fcn0%i))>1e-200.or.abs(minval(dFdT%i))>1e-200)then
!  print*,'minr maxr mini maxi integrator 1040',minval(Y%r),maxval(Y%r),minval(Y%i),maxval(Y%i), &
!        & minval(Fcn0%r),maxval(Fcn0%r),minval(Fcn0%i),maxval(Fcn0%i), &
!        & minval(dFdT%r),maxval(dFdT%r),minval(dFdT%i),maxval(dFdT%i)
!  CALL GEOS_CHEM_STOP
!  endif
...
!~~~> Output: Error indicator
   INTEGER, INTENT(OUT) :: IERR
! ~~~~ Local variables        
   TYPE (XPLEX) :: Ystage(NVAR*ros_S), K(NVAR*ros_S)
   TYPE (XPLEX) :: U(NVAR*ros_S,NADJ), V(NVAR*ros_S,NADJ)
#ifdef FULL_ALGEBRA
   TYPE (XPLEX), DIMENSION(NVAR,NVAR)  :: Jac, dJdT, Ghimj
#else
   TYPE (XPLEX), DIMENSION(LU_NONZERO) :: Jac, dJdT, Ghimj
#endif
   TYPE (XPLEX) :: Hes0(NHESS)
   TYPE (XPLEX) :: Tmp(NVAR), Tmp2(NVAR)
   TYPE (XPLEX) :: H, HC, HA, Tau 
...
!~~~>  Sensitivities w.r.t. reacton coefficients
   TYPE (XPLEX), INTENT(OUT) :: Lambda_R(NCOEFF,NADJ)
   INTEGER :: icoeff, vstart
   TYPE (XPLEX) :: DFDR(NVAR*NCOEFF)
   TYPE (XPLEX) :: DJDR(NVAR*NCOEFF)
   TYPE (XPLEX) :: V_R(NCOEFF*ros_S,NADJ)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~> Input: the initial condition at Tstart; Output: the solution at T   
   INTEGER, INTENT(IN) :: NADJ
   TYPE (XPLEX), INTENT(INOUT) :: Y(NVAR,NADJ)
!~~~> Input: integration interval   
   TYPE (XPLEX), INTENT(IN) :: Tstart,Tend      
...
! ~~~~ Local variables        
   TYPE (XPLEX) :: Y0(NVAR)
   TYPE (XPLEX) :: Ynew(NVAR,NADJ), Fcn0(NVAR,NADJ), Fcn(NVAR,NADJ) 
   TYPE (XPLEX) :: K(NVAR*ros_S,NADJ), dFdT(NVAR,NADJ)
#ifdef FULL_ALGEBRA
   TYPE (XPLEX), DIMENSION(NVAR,NVAR)  :: Jac0, Ghimj, Jac, dJdT
#else
   TYPE (XPLEX), DIMENSION(LU_NONZERO) :: Jac0, Ghimj, Jac, dJdT
#endif   
   TYPE (XPLEX) :: H, Hnew, HC, HG, Fac, Tau 
   TYPE (XPLEX) :: Err, Yerr(NVAR,NADJ)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  TYPE (XPLEX) FUNCTION ros_ErrorNorm ( Y, Ynew, Yerr, & 
...
! Input arguments   
   TYPE (XPLEX), INTENT(IN) :: Y(NVAR), Ynew(NVAR), &
          Yerr(NVAR), AbsTol(NVAR), RelTol(NVAR)
   LOGICAL, INTENT(IN) ::  VectorTol
! Local variables     
   TYPE (XPLEX) :: Err, Scale, Ymax   
   INTEGER  :: i
  ...
  !print*,'ros_ErrorNorm',ros_ErrorNorm  
...
!~~~> Input arguments   
   TYPE (XPLEX), INTENT(IN) :: T, Roundoff, Y(NVAR), Fcn0(NVAR) 
!~~~> Output arguments   
   TYPE (XPLEX), INTENT(OUT) :: dFdT(NVAR)   
!~~~> Local variables     
   TYPE (XPLEX) :: Delta  
   TYPE (XPLEX), PARAMETER :: ONE = xplex(1.0d0,0d0), DeltaMin =xplex(1.0d-6,0d0)
   dFdT=0d0
...
!~~~> Arguments   
   TYPE (XPLEX), INTENT(IN) :: T, Roundoff, Y(NVAR)
#ifdef FULL_ALGEBRA    
   TYPE (XPLEX), INTENT(IN)  :: Jac0(NVAR,NVAR) 
   TYPE (XPLEX), INTENT(OUT) :: dJdT(NVAR,NVAR)   
#else
   TYPE (XPLEX), INTENT(IN)  :: Jac0(LU_NONZERO) 
   TYPE (XPLEX), INTENT(OUT) :: dJdT(LU_NONZERO)   
#endif   
!~~~> Local variables     
   TYPE (XPLEX) :: Delta  
   dJdT=0d0
...
!~~~> Input arguments
   ...
   TYPE (XPLEX), INTENT(IN) ::  Jac0(LU_NONZERO)
#endif   
   TYPE (XPLEX), INTENT(IN) ::  gam
   INTEGER, INTENT(IN) ::  Direction
!~~~> Output arguments
#ifdef FULL_ALGEBRA    
   TYPE (XPLEX), INTENT(OUT) :: Ghimj(NVAR,NVAR)
#else
   TYPE (XPLEX), INTENT(OUT) :: Ghimj(LU_NONZERO)
#endif   
   LOGICAL, INTENT(OUT) ::  Singular
   INTEGER, INTENT(OUT) ::  Pivot(NVAR)
!~~~> Inout arguments
   TYPE (XPLEX), INTENT(INOUT) :: H   ! step size is decreased when LU fails
!~~~> Local variables
   INTEGER  :: i, ising, Nconsecutive
   TYPE (XPLEX) :: ghinv
   TYPE (XPLEX), PARAMETER :: ONE  = xplex(1.0_dp,0d0), HALF = xplex(0.5_dp,0d0)
...
!~~~> Inout variables
#ifdef FULL_ALGEBRA    
   TYPE (XPLEX), INTENT(INOUT) :: A(NVAR,NVAR)
#else   
   TYPE (XPLEX), INTENT(INOUT) :: A(LU_NONZERO)
#endif
!~~~> Output variables
...
#ifdef FULL_ALGEBRA    
   TYPE (XPLEX), INTENT(IN) :: A(NVAR,NVAR)
#else   
   TYPE (XPLEX), INTENT(IN) :: A(LU_NONZERO)
#endif
   INTEGER, INTENT(IN) :: Pivot(NVAR)
!~~~> InOut variables
   TYPE (XPLEX), INTENT(INOUT) :: b(NVAR)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   
   ...
   IMPLICIT NONE
!~~~> Input variables 
   TYPE (XPLEX), INTENT(IN) :: a, b, T, Ya(NVAR), Yb(NVAR)
   TYPE (XPLEX), INTENT(IN) :: Ja(NVAR), Jb(NVAR)
!~~~> Output variables     
   TYPE (XPLEX), INTENT(OUT) :: Y(NVAR)
!~~~> Local variables     
   TYPE (XPLEX) :: Tau, amb(3), C(NVAR,4)
...
!~~~> Input variables 
   TYPE (XPLEX), INTENT(IN) :: a, b, T, Ya(NVAR), Yb(NVAR)
   TYPE (XPLEX), INTENT(IN) :: Ja(NVAR), Jb(NVAR), Ha(NVAR), Hb(NVAR)
!~~~> Output variables     
   TYPE (XPLEX), INTENT(OUT) :: Y(NVAR)
!~~~> Local variables     
   TYPE (XPLEX) :: Tau, amb(5), C(NVAR,6)
   TYPE (XPLEX), PARAMETER :: ZERO = xplex(0.0d0,0d0), HALF = xplex(0.5d0,0d0)
   INTEGER :: i, j
!   tau=0d0
!   amb=0d0
!   C=0d0
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   
!~~~> Input variables     
   TYPE (XPLEX), INTENT(IN)  :: T, Y(NVAR)
!~~~> Output variables     
   TYPE (XPLEX), INTENT(OUT) :: Ydot(NVAR)
!~~~> Local variables
   TYPE (XPLEX) :: Told     
...
!~~~> Output variables
#ifdef FULL_ALGEBRA    
    TYPE (XPLEX) :: JV(LU_NONZERO), Jcb(NVAR,NVAR)
#else
    TYPE (XPLEX) :: Jcb(LU_NONZERO)
#endif   
!~~~> Local variables
    TYPE (XPLEX) :: Told
#ifdef FULL_ALGEBRA    
...
!~~~> Input variables     
    TYPE (XPLEX), INTENT(IN)  :: T, Y(NVAR)
!~~~> Output variables     
    TYPE (XPLEX), INTENT(OUT) :: Hes(NHESS)
!~~~> Local variables
    TYPE (XPLEX) :: Told     
