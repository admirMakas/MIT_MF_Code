! $Id: adj_arrays_mod.f,v 1.25 2012/05/09 22:31:56 nicolas Exp $
...
!  Module Variables:
!  ============================================================================
!  (1 ) EMS_orig     (TYPE (XPLEX)) : can store original emissions
!  (2 ) FORCING      (TYPE (XPLEX)) : holds (ym-yo)^2/err for all days
!  (3 ) MOP_MOD_DIFF (TYPE (XPLEX)) : holds (ym-yo) for all days
!  (4 ) MODEL_BIAS   (TYPE (XPLEX)) : holds (ym-yo)/ym for all days
!  (5 ) MODEL        (TYPE (XPLEX)) : holds H(ym)
!  (6 ) OBS          (TYPE (XPLEX)) : holds h(yo)
!  (7 ) COST_ARRAY   (TYPE (XPLEX)) : holds J(I,J), assumes column obs
!  (8 ) OBS_COUNT    (TYPE (XPLEX)) : holds #obs/box
!  (9 ) OBS_STT      (TYPE (XPLEX)) : Array with psedudo observations
!  (10) STT_ADJ      (TYPE (XPLEX)) : Adjoint tracer array (STT equivalent in fwd)
!  (11) WEIGHT       (TYPE (XPLEX)) : Array with obs weights for pseudo obs and sensitivity
...
!  (19) LONFD        (TYPE (XPLEX)) : lon for FD and debugging
!  (20) JFD         (INTEGER) : lat gridbox for FD and debugging
!  (21) LATFD        (TYPE (XPLEX)) : lat for FD and debugging
...
!  (26) COST FUNC    (TYPE (XPLEX)) : scalar cost function
!  (27) NOBS        (INTEGER) : Number of obs datasets used
!  (28) ICS_SF       (TYPE (XPLEX)) : array of initial conditions
!  (29) ICS_SF0      (TYPE (XPLEX)) : array of first guess for initial conditions
!  (30) ICS_SF_DEFAULT  (R*8) : scalar first guess for initial conditions
!  (31) EMS_SF       (TYPE (XPLEX)) : array of emission scaling
!  (32) EMS_SF0      (TYPE (XPLEX)) : array of first guess for emission scaling
!  (33) ICS_SF_DEFAULT  (R*8) : Initial condition scaling factors at iteration 1
!  (34) ICS_SF_ADJ   (TYPE (XPLEX)) : dJ/dICS_SF
!  (35) EMS_SF_ADJ   (TYPE (XPLEX)) : dJ/dEMS_SF
...
!  (40) REG_PARAM_EMS(TYPE (XPLEX)) : regularization parameter for a priori/background term
!  (41) REG_PARAM_ICS(TYPE (XPLEX)) : regularization parameter for a priori/background term
!  (42) ICSFD       (INTEGER) : initial condition species for FD tests
!  (43) STT_ORIG     (TYPE (XPLEX)) : Original unscaled values of STT 
!  (44) REMIS_ADJ    (TYPE (XPLEX)) : Adjoint of REMIS 
!  (45) DEPSAV_ADJ   (TYPE (XPLEX)) : Adjoint of DEPSAV
!  (46) O3_PROF_SAV  (TYPE (XPLEX)) : TOMS O3 profile from set_prof 
!  (47) EMS_ERROR    (TYPE (XPLEX)) : standard error for for a priori/background term
...
!  (55) INV_NSPAN    (TYPE (XPLEX)) : The inverse of NSPAN 
!  (56) EMS_ADJ      (TYPE (XPLEX)) : dJ/dEMS
!  (57) ICS_ERROR    (TYPE (XPLEX)) : standard error for for a priori/background term
!  (58) HMAX        (INTEGER) : Total length of 1D gradient vector
!  (59) VAR_FD       (TYPE (XPLEX)) : Concentrations for chem adjoint debugging
!  (60) RCONST_FD    (TYPE (XPLEX)) : Reaction rates for chem adjoint debugging
! 
!  Module Routines:
!  ============================================================================
...
!
      ...
      ! MODULE VARIABLES
      !================================================================= 
      TYPE (XPLEX), ALLOCATABLE :: EMS_orig(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: FORCING(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: MOP_MOD_DIFF(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: MODEL_BIAS(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: MODEL(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: SAT_DOFS(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: OBS(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: COST_ARRAY(:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: OBS_COUNT(:,:) 
      TYPE (XPLEX), ALLOCATABLE :: OBS_STT(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: STT_ADJ(:,:,:,:) 
      TYPE (XPLEX), ALLOCATABLE :: WEIGHT(:,:,:,:) 
      TYPE (XPLEX), ALLOCATABLE :: ADJ_FORCE(:,:,:,:)
      ...
      TYPE (XPLEX), ALLOCATABLE :: COST_FUNC_SAV(:)
      TYPE (XPLEX), ALLOCATABLE :: STT_ADJ_FD(:)
      INTEGER             :: NOBS
      INTEGER             :: NSPAN
      TYPE (XPLEX)              :: INV_NSPAN
      INTEGER             :: NOBS_CSPEC

      TYPE (XPLEX), ALLOCATABLE :: ICS_SF(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: STT_ORIG(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: ICS_SF0(:,:,:,:)
      !TYPE (XPLEX)              :: ICS_SF_tmp
      TYPE (XPLEX), ALLOCATABLE :: EMS_SF(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: EMS_SF0(:,:,:,:)
      !TYPE (XPLEX)              :: EMS_SF_tmp
      TYPE (XPLEX), ALLOCATABLE :: ICS_SF_ADJ(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: EMS_SF_ADJ(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: EMS_ADJ(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: REG_PARAM_EMS(:)
      TYPE (XPLEX), ALLOCATABLE :: REG_PARAM_ICS(:)
      ...
      TYPE (XPLEX), ALLOCATABLE  :: REMIS_ADJ(:,:)
      TYPE (XPLEX), ALLOCATABLE  :: DEPSAV_ADJ(:,:,:)

      TYPE (XPLEX), ALLOCATABLE  :: O3_PROF_SAV(:,:,:)

      TYPE (XPLEX), ALLOCATABLE :: ICS_SF_DEFAULT(:)
      TYPE (XPLEX), ALLOCATABLE :: EMS_SF_DEFAULT(:)
      TYPE (XPLEX), ALLOCATABLE :: IDCSPEC_ADJ(:)
      TYPE (XPLEX), ALLOCATABLE :: ID2C(:)

      ! added for apriori constraints (dkh, 01/11/11) 
      TYPE (XPLEX), ALLOCATABLE  :: EMS_ERROR(:)
      TYPE (XPLEX), ALLOCATABLE  :: ICS_ERROR(:)
      ...
      INTEGER             :: NSTPL
      INTEGER             :: STRFD
      TYPE (XPLEX), ALLOCATABLE :: PROD_SF(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: PROD_SF0(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: PROD_SF_ADJ(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: P_ADJ(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: LOSS_SF(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: LOSS_SF0(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: LOSS_SF_ADJ(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: k_ADJ(:,:,:,:)
      TYPE (XPLEX), ALLOCATABLE :: REG_PARAM_PROD(:)
      TYPE (XPLEX), ALLOCATABLE :: REG_PARAM_LOSS(:)

      TYPE (XPLEX), ALLOCATABLE :: VAR_FD(:,:)
      TYPE (XPLEX), ALLOCATABLE :: RCONST_FD(:,:)
      ...
      TYPE (XPLEX), ALLOCATABLE :: PROD_SF_DEFAULT(:)
      TYPE (XPLEX), ALLOCATABLE :: LOSS_SF_DEFAULT(:)
      TYPE (XPLEX), ALLOCATABLE  :: PROD_ERROR(:)
      TYPE (XPLEX), ALLOCATABLE  :: LOSS_ERROR(:)
...
!  (5 ) VALUE (TYPE (XPLEX) ) : Value to store in (I,J,K,N)th element of array
!
!  NOTES:
!******************************************************************************
!
...
!  (4 ) VALUE (TYPE (XPLEX) ) : Value to store in (I,J,D)th element of array
!
!  NOTES:
!******************************************************************************
!
...
!  (5 ) VALUE (TYPE (XPLEX) ) : Value to store in (I,J,D)th element of array
!
!  NOTES:
!******************************************************************************
!
