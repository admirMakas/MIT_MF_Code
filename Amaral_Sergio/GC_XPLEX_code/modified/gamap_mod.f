! $Id: gamap_mod.f,v 1.7 2012/03/01 22:00:27 daven Exp $
      MODULE GAMAP_MOD
!
!******************************************************************************
...
!  (13) MWT       (TYPE (XPLEX)  ) : Tracer molecular weights
!  (14) SCALE     (TYPE (XPLEX)  ) : GAMAP scale factors (e.g. 1e9 for v/v -> ppbv)
...
#     include "CMN_DIAG"              ! Diagnostic parameters
      ...
      TYPE (XPLEX),            ALLOCATABLE :: MWT(:,:)
      TYPE (XPLEX),            ALLOCATABLE :: SCALE(:,:)
...
!      ENDDO
      ...
      IF ( LSOA ) THEN
         ...
         DO T = 1, 18
            ...
            ! Write tracers [kg] to "tracerinfo.dat"
            CALL WRITE_TINFO( NAME_NEW, NAME_NEW,  xplx(1e0), 
     &                        1,   xplx(1e0),      'kg/kg', N )
         ENDDO
      ENDIF
     ...
     &                           INDEX(T,D) )
      ...
      DO D = 1, MAXDIAG
         ...
         IF ( NTRAC_ADJ(D) > 0 ) THEN
            ...
            ! Write tracers to file
            DO T = 1, NTRAC_ADJ(D)
              CALL WRITE_TINFO(NAME_ADJ(T,D),FNAME_ADJ(T,D),xplx(0d0),       
     &                           1, xplx(1d0),UNIT_ADJ(T,D),
     &                           INDEX_ADJ(T,D) )
            ENDDO
         ENDIF
      ENDDO
...
!  (3 ) MWT   (TYPE (XPLEX)   ) : Molecular weight [kg/mole]
!  (4 ) MOLC  (TYPE (XPLEX)   ) : Moles C per mole tracer (for hydrocarbons)
!  (5 ) SCALE (TYPE (XPLEX)   ) : GAMAP scale factor (e.g. 1e+9 scales v/v --> ppbv)
...
!
      ...
      TYPE (XPLEX),           INTENT(IN) :: MWT,  SCALE
     ...
     &     MOLC,            N,                SCALE, TRIM( UNIT )
      ...
      ! FORMAT string
 100  FORMAT( a8, 1x, a30, 2es10.3, i3, i9, 2es10.3, 1x, a )
