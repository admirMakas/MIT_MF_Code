...
!  (4 ) Vud     (TYPE (XPLEX) ) : Array for updraft velocity     [m/s]
!  (5 ) CLDLIQ  (TYPE (XPLEX) ) : Array for cloud liquid water   [cm3 H2O/cm3 air]
!  (6 ) CLDICE  (TYPE (XPLEX) ) : Array for cloud ice content    [cm3 ice/cm3 air]
!  (7 ) C_H2O   (TYPE (XPLEX) ) : Array for Mixing ratio of ,      
!                            water, computed from Eice(T)  [v/v]
!  (8 ) PDOWN   (TYPE (XPLEX) ) : Precip thru bottom of grid box [cm3 H2O/cm2 area/s]
!  (9 ) QQ      (TYPE (XPLEX) ) : Rate of new precip formation   [cm3 H2O/cm3 air/s]
!  (10) EPSILON (TYPE (XPLEX) ) : A very small positive number   [unitless]
!  (11) H2O2s   (TYPE (XPLEX) ) : Array to save H2O2 for wetdep  [v/v]
!  (12) SO2s    (TYPE (XPLEX) ) : Array to save SO2 for wetdep   [v/v]
!
!  Module Routines:
!  ============================================================================
...
!
      ...
      USE COMPLEXIFY
      IMPLICIT NONE
      INTERFACE COMPUTE_L2G
        MODULE PROCEDURE COMPUTE_L2G_R
        MODULE PROCEDURE COMPUTE_L2G_C
      END INTERFACE

      INTERFACE WASHFRAC_LIQ_GAS
        MODULE PROCEDURE WASHFRAC_LIQ_GAS_R
        MODULE PROCEDURE WASHFRAC_LIQ_GAS_C
      END INTERFACE
      !=================================================================
      ...
      TYPE (XPLEX),  PARAMETER   :: EPSILON = xplex(1d-32,0d0)
      ...
      TYPE (XPLEX),  ALLOCATABLE :: Vud(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: C_H2O(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: CLDLIQ(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: CLDICE(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: PDOWN(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: QQ(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: H2O2s(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: SO2s(:,:,:)

      ! Now include adjoint of F (dkh, 10/03/08) 
      TYPE (XPLEX),  ALLOCATABLE :: QC_SO2(:,:,:,:)
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) TK (TYPE (XPLEX)) : Ambient temperature [K] 
! 
!  References:
!  ============================================================================
...
!
      ...
      ! Parameters
      TYPE (XPLEX), PARAMETER  :: A = xplex(-2663.5d0,0d0)
      TYPE (XPLEX), PARAMETER  :: B =  xplex(12.537d0,0d0)
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE COMPUTE_L2G_R( Kstar298, H298_R, TK, H2OLIQ, L2G )
!
!******************************************************************************
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) Kstar298 (TYPE (XPLEX)) : Eff. Henry's law constant @ 298 K   [moles/atm]
!  (2 ) H298_R   (TYPE (XPLEX)) : Molar heat of formation @ 298 K / R [K]
!  (3 ) TK       (TYPE (XPLEX)) : Temperature at grid box (I,J,L)     [K]
!  (4 ) H2OLIQ   (TYPE (XPLEX)) : Liquid water content at (I,J,L)     [cm3 H2O/cm3 air]
!
!  Arguments as Output:
!  ============================================================================
!  (5 ) L2G      (TYPE (XPLEX)) : Cliq/Cgas ratio for given tracer  [unitless]
!
!  References (see above for full citations):
!  ===========================================================================
!  (1 ) Jacob et al, 2000
...
!
      ...
      TYPE (XPLEX), INTENT(IN)  :: TK, H2OLIQ
      TYPE (XPLEX), INTENT(OUT) :: L2G
      
      ! Local variables
      TYPE (XPLEX)              :: Kstar

      ! R = universal gas constant [atm/moles/K]
      TYPE (XPLEX), PARAMETER   :: R = xplex(8.32d-2,0d0)

      ! INV_T0 = 1/298 K
      TYPE (XPLEX), PARAMETER   :: INV_T0 = xplex(1d0 / 298d0,0d0)
      ...
      ! Return to calling program
      END SUBROUTINE COMPUTE_L2G_R

      SUBROUTINE COMPUTE_L2G_C( Kstar298, H298_R, TK, H2OLIQ, L2G )
!
!******************************************************************************
!  Subroutine COMPUTE_L2G computes the ratio L2G = Cliq / Cgas, which is 
!  the mixing ratio of tracer in the liquid phase, divided by the mixing 
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) Kstar298 (TYPE (XPLEX)) : Eff. Henry's law constant @ 298 K
!  [moles/atm]
!  (2 ) H298_R   (TYPE (XPLEX)) : Molar heat of formation @ 298 K / R [K]
!  (3 ) TK       (TYPE (XPLEX)) : Temperature at grid box (I,J,L)     [K]
!  (4 ) H2OLIQ   (TYPE (XPLEX)) : Liquid water content at (I,J,L)     [cm3
!  H2O/cm3 air]
!
!  Arguments as Output:
!  ============================================================================
!  (5 ) L2G      (TYPE (XPLEX)) : Cliq/Cgas ratio for given tracer
!  [unitless]
!
!  References (see above for full citations):
!  ===========================================================================
...
!
      ! Arguments
      TYPE (XPLEX), INTENT(IN)  :: KStar298, H298_R, TK, H2OLIQ
      TYPE (XPLEX), INTENT(OUT) :: L2G
...
!  Arguments as Output:
!  ============================================================================
!  (2 ) F    (TYPE (XPLEX))  : Fraction of tracer scavenged in cloud updraft [0-1]
...
#     include "CMN_SIZE"    ! Size parameters
      ...
      ! Arguments
      INTEGER, INTENT(IN)  :: N
      TYPE (XPLEX),  INTENT(OUT) :: F(IIPAR,JJPAR,LLPAR)
      INTEGER, INTENT(OUT) :: ISOL

      ! Local variables 
      INTEGER              :: I, J, L, NN
      TYPE (XPLEX)        :: L2G, I2G, C_TOT, F_L, F_I, K, TMP, SO2LOSS

      ! Kc is the conversion rate from cloud condensate to precip [s^-1]
      TYPE (XPLEX), PARAMETER    :: KC        = xplex(5d-3,0d0)
      ...
      TYPE (XPLEX), PARAMETER::CONV_H2O2 = xplex(8.27042925126d-1,0d0)
      ...
      TYPE (XPLEX), PARAMETER::CONV_NH3  = xplex(5.69209978831d-1,0d0)
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) KC (TYPE (XPLEX)) : Conversion rate from cloud condensate to precip [s^-1]
!
!  Arguments as Output:
!  ============================================================================
!  (2 ) F  (TYPE (XPLEX)) : Fraction of aerosol scavenged in updrafts [unitless]
!
!  NOTES:
!  (1 ) Split off 
!******************************************************************************
...
!  (5  ) K_RAIN   (TYPE (XPLEX) ) : Rainout rate constant for tracer N [s^-1]
!  (6  ) DT       (TYPE (XPLEX) ) : Timestep for rainout event         [s]
!  (7  ) F        (TYPE (XPLEX) ) : Fraction of grid box precipitating [unitless]
!
!  Arguments as Output:
!  ============================================================================
!  (8  ) RAINFRAC (TYPE (XPLEX))  : Fraction of tracer lost to rainout [unitless]
!
!  References (see above for full citations):
!  ============================================================================
!  (1 ) Jacob et al, 2000
...
#     include "CMN_SIZE"   ! Size parameters
      ...
      TYPE (XPLEX),  INTENT(IN)  :: K_RAIN, DT, F
      TYPE (XPLEX),  INTENT(OUT) :: RAINFRAC

      ! Local variables 
      TYPE (XPLEX)     :: L2G, I2G, C_TOT, F_L, F_I, K, TK, SO2LOSS
      ...
      !TYPE (XPLEX), PARAMETER    :: CONV = 8.27042925126d-1
      !------------------------------------------------------------------------
...
!  Arguments as Input:
!  =========================================================================== 
!  (1 ) K  (TYPE (XPLEX)) : Rainout rate constant              [1/s]
!  (2 ) DT (TYPE (XPLEX)) : Timestep for rainout event         [s]
!  (3 ) F  (TYPE (XPLEX)) : Fraction of grid box precipitating [unitless]
!
!  NOTES:
...
!  (5  ) PP       (TYPE (XPLEX) ) : Precip rate thru at bottom    
!                             of grid box (I,J,L)          [cm3 H2O/cm2 air/s]
!  (6  ) DT       (TYPE (XPLEX) ) : Timestep for rainout event   [s]
!  (7  ) F        (TYPE (XPLEX) ) : Fraction of grid box 
!                             that is precipitating        [unitless]
!
!  Arguments as Output:
!  ============================================================================
!  (8  ) WASHFRAC (TYPE (XPLEX))  : Fraction of tracer lost to rainout [unitless]
...
#     include "CMN_SIZE"   ! Size parameters
      ...
      ! Arguments
      INTEGER, INTENT(IN)  :: I, J, L, N
      TYPE (XPLEX),  INTENT(IN)  :: PP, DT, F
      TYPE (XPLEX),  INTENT(OUT) :: WASHFRAC
      LOGICAL, INTENT(OUT) :: AER

      ! Local variables 
      TYPE (XPLEX)               :: L2G, DZ, TK, SO2LOSS

      ! First order washout rate constant for HNO3, aerosols = 1 cm^-1
      TYPE (XPLEX), PARAMETER    :: K_WASH = xplex(1d0,0d0)
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) TK       (TYPE (XPLEX) ) : Temperature in grid box            [K]
!  (2 ) F        (TYPE (XPLEX) ) : Fraction of grid box 
!                             that is precipitating             [unitless]
!  (3 ) K_WASH   (TYPE (XPLEX) ) : 1st order washout rate constant    [1/cm]
!  (3 ) PP       (TYPE (XPLEX) ) : Precip rate thru at bottom    
!                             of grid box (I,J,L)           [cm3 H2O/cm2 air/s]
!
!  NOTES:
...
!   
      ! Arguments
      TYPE (XPLEX), INTENT(IN) :: DT, F, K_WASH, PP, TK
...
!------------------------------------------------------------------------------
      ...
      FUNCTION WASHFRAC_LIQ_GAS_R( Kstar298, H298_R, PP, DT, 
     &                           F,        DZ,     TK, K_WASH ) 
     &         RESULT( WASHFRAC )
!
!******************************************************************************
...
!  ============================================================================
!  (1 ) Kstar298 (TYPE (XPLEX) ) : Eff. Henry's law constant @ 298 K  [moles/atm]
!  (2 ) H298_R   (TYPE (XPLEX) ) : Henry's law coefficient            [K]
!  (3 ) PP       (TYPE (XPLEX) ) : Precip rate thru at bottom    
!                             of grid box (I,J,L)           [cm3 H2O/cm2 air/s]
!  (4 ) DT       (TYPE (XPLEX) ) : Dynamic timestep                   [s]
!  (5 ) F        (TYPE (XPLEX) ) : Fraction of grid box 
!                             that is precipitating             [unitless]
!  (6 ) DZ       (TYPE (XPLEX) ) : Height of grid box                 [cm]
!  (7 ) TK       (TYPE (XPLEX) ) : Temperature in grid box            [K]
!  (8 ) K_WASH   (TYPE (XPLEX) ) : 1st order washout rate constant    [1/cm]
!
!  NOTES:
...
!
      ! Arguments
      REAL*8,INTENT(IN)::Kstar298, H298_R
      TYPE (XPLEX), INTENT(IN):: PP, DT, F, DZ, TK, K_WASH

      ! Local variables
      TYPE (XPLEX)             :: L2G, LP, WASHFRAC, WASHFRAC_F_14
      ...
      ! Return to calling program
      END FUNCTION WASHFRAC_LIQ_GAS_R

      FUNCTION WASHFRAC_LIQ_GAS_C( Kstar298, H298_R, PP, DT,
     &                           F,        DZ,     TK, K_WASH )
     &         RESULT( WASHFRAC )
!
!******************************************************************************
!  Function WASHFRAC_LIQ_GAS returns the fraction of soluble liquid/gas
!  phase 
!  tracer lost to washout. (bmy, 11/8/02, 7/20/04)
!
!  Arguments as Input:
...
!  (6 ) DZ       (TYPE (XPLEX) ) : Height of grid box                 [cm]
!  (7 ) TK       (TYPE (XPLEX) ) : Temperature in grid box            [K]
!  (8 ) K_WASH   (TYPE (XPLEX) ) : 1st order washout rate constant
!  [1/cm]
!
!  NOTES:
!  (1 ) WASHFRAC_LIQ_GAS used to be an internal function to subroutine
!  WASHOUT.
!        This caused NaN's in the parallel loop on Altix, so we moved it
!        to
!        the module and now pass all arguments explicitly (bmy, 7/20/04)
!******************************************************************************
!
      ! Arguments
      TYPE (XPLEX),INTENT(IN)::Kstar298, H298_R, PP, DT, F,DZ,TK,K_WASH
      ...
      ! Return to calling program
      END FUNCTION WASHFRAC_LIQ_GAS_C
...
!  (4 ) Use double-precision exponents to force TYPE (XPLEX) accuracy
!        (e.g. 1d0, bmy, 2/28/00)
...
#     include "CMN_DIAG"  ! Diagnostic arrays and switches 
      ...
      TYPE (XPLEX)              :: Q,     QDOWN,  DT,        DT_OVER_TAU
      TYPE (XPLEX)              :: K,     K_MIN,  K_RAIN,    RAINFRAC
      TYPE (XPLEX)              :: F,     FTOP,   F_PRIME,   WASHFRAC
      TYPE (XPLEX)              :: LOST,  GAINED, MASS_WASH, MASS_NOWASH
      TYPE (XPLEX)              :: ALPHA, ALPHA2, WETLOSS,   TMP

      ! DSTT is the accumulator array of rained-out 
      ! soluble tracer for a given (I,J) column
      TYPE (XPLEX)              :: DSTT(NSOL,LLPAR,IIPAR,JJPAR)
...
!            print*,'STT is NaN',STT(i,j,l,n),i,j,l,n
!           
!         endif
!      enddo
!      enddo
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) Q      (TYPE (XPLEX)) : Rate of precip formation [cm3 H2O/cm3 air/s]
!
!  Function value:
!  ============================================================================
!  (2 ) K_RAIN (TYPE (XPLEX)) : 1st order rainout rate constant [s-1]
!
!  NOTES:
...
!  Function value:
!  ============================================================================
!  (3 ) F_PRIME (TYPE (XPLEX)) : Fraction of grid box undergoing LS precip [unitless]
!
!  NOTES:
!  (1 ) Now made into a MODULE routine since we cannot call internal routines
!        from w/in a parallel loop.  Updated comments. (bmy, 3/18/04)
...
!  (1 ) Q       (TYPE (XPLEX)) : Rate of precip formation        [cm3 H2O/cm3 air/s]
!  (2 ) K_RAIN  (TYPE (XPLEX)) : 1st order rainout rate constant [s-1]
!  (3 ) DT      (TYPE (XPLEX)) : Wet deposition timestep         [s]
!
!  Function value:
!  ============================================================================
!  (4 ) F_PRIME (TYPE (XPLEX)) : Frac. of grid box undergoing CONV precip [unitless]
!
!  NOTES:
!  (1 ) Now made into a MODULE routine since we cannot call internal routines
!        from w/in a parallel loop.  Updated comments. (bmy, 3/18/04)
...
!
      ! Arguments
      TYPE (XPLEX), INTENT(IN) :: Q, K_RAIN, DT
...
#     include "CMN_SIZE"
      ...
      TYPE (XPLEX),  INTENT(IN) :: PDOWN,    QQ,       ALPHA,     ALPHA2
      TYPE (XPLEX),  INTENT(IN)::RAINFRAC,WASHFRAC,MASS_WASH,MASS_NOWASH
      TYPE (XPLEX),  INTENT(IN)::WETLOSS, GAINED,LOST,      DSTT(LLPAR)
      TYPE (XPLEX),  INTENT(IN) :: STT(LLPAR)
...
#     include "CMN_SIZE"  ! Size parameters
      ...
      DO NN = 1, NSOL
         N = IDWETD(NN)
         WRITE( 6, '(i3,3x,a14,3x,i3,3x,2f6.1)' )
