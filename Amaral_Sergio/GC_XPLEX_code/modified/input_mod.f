...
!
      ...
      PUBLIC :: READ_INPUT_FILE, READ_ONE_LINE, SPLIT_ONE_LINE
...
#     include "CMN_SIZE"  ! Size parameters
     ...
     &                    'read_tracer_menu (input_mod.f)' )
      ...
      DO T = 1, N_TRACERS
         ...
         READ( SUBSTRS(3), * ) TRACER_MW_G(T)%r 
         TRACER_MW_G(T)%i=0d0
               ...
               IF ( NAME(1:1) == '(' ) THEN 
                  ...
                  IF ( ISDIGIT( NAME(1:1) ) ) THEN
                     ...
                     ELSE
                        ...
                        ! Coefficient of emitted tracer
                        READ( NAME(1:1), * ) TRACER_COEFF(T,C)%r
                        TRACER_COEFF(T,C)%i = 0d0
                        ! Get the rest of the name
     ...
     &       'TRACER MENU (==> denotes SMVGEAR emitted species)' 
      ...
      ! Format statement
 100  FORMAT( I3, 1x, A10, 6x, 2F6.1 )
 110  FORMAT( 5x, '===> ', 2f4.1, 1x, A4  )
 120  FORMAT( 5x, '---> ', 2f4.1, 1x, A4  )
...
!
      ...
      DO T = 1, N
         READ( SUBSTRS(T), * ) SALA_REDGE_um(T)%r
         SALA_REDGE_um(T)%i = 0d0
      ENDDO
      !SALA_REDGE_um(T)%i = 0d0
      ! Coarse mode seasalt radii bin edges [um]
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 2, 'read_aerosol_menu:9' )
      DO T = 1, N
         READ( SUBSTRS(T), * ) SALC_REDGE_um(T)%r
         SALC_REDGE_um(T)%i=0d0
      ENDDO
      !SALC_REDGE_um(T)%i=0d0
...
!
      ...
      USE AIRCRAFT_MOD, ONLY : LAIRCRAFT !jkoo 
...
!      READ( SUBSTRS(1:N), * ) LAIRNOX
      ...
      ! Use PARTNER aviation emissions?  jkoo
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 1, 'read_emissions_menu:50' )
      READ( SUBSTRS(1:N), * ) LAIRCRAFT
...
! adj_group: (zhej, dkh, 01/17/12, adj32_015) 
 ...
 150        FORMAT( 
     ...
     $     LGFED2BB .or. L8DAYBB .or. LSYNOPBB .or. L3HRBB 
      ...
      WRITE( 6, 100     ) 'Turn on PARTNER aircraft    : ', LAIRCRAFT
!jkoo
...
#     include "CMN_DIAG"     ! NDxx flags
     ...
     &         ' is not split at 0.5 um!!'
      ...
      READ( SUBSTRS(1:N), * ) HR1_JV%r, HR2_JV%r
      HR1_JV%i=0d0
      HR2_JV%i=0d0
      !--------------------------
      ...
      READ( SUBSTRS(1:N), * ) HR1_OH%r, HR2_OH%r
      HR1_OH%i=0d0
      HR2_OH%i=0d0
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 2,  'read_diagnostic_menu:45' )
      READ( SUBSTRS(1:N), * ) HR1_NO%r, HR2_NO%r
      HR1_NO%i =0d0
      HR2_NO%i =0d0
      !--------------------------
      ...
      READ( SUBSTRS(1:N), * ) HR1_OTH%r, HR2_OTH%r
      HR1_OTH%i =0d0
      HR2_OTH%i = 0d0
      !--------------------------
...
#     include "CMN_DIAG"   ! NDxx flags
      ...
      READ( SUBSTRS(1:N), * ) HR_WRITE%r
      HR_WRITE%i = 0d0
      ! HR1, HR2
      CALL SPLIT_ONE_LINE( SUBSTRS, N, 2,  'read_nd51_menu:5' )
      READ( SUBSTRS(1:N), * ) HR1%r, HR2%r
      HR1%i = 0d0
      HR2%i =0d0
      ! IMIN, IMAX
...
#     include "CMN_DIAG"    ! ND65
      ...
      TYPE (XPLEX)               :: FAM_COEF(MAXMEM,MAXFAM)
