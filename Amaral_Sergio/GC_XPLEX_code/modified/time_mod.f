...
!  (16) TAU         (TYPE (XPLEX) ) : Current TAU value (hours since 0 GMT 1/1/1985)
!  (17) TAUb        (TYPE (XPLEX) ) : TAU value at beginning of GEOS-CHEM run
!  (18) TAUe        (TYPE (XPLEX) ) : TAU value at end       of GEOS-CHEM run
!  (19) DIAGb       (TYPE (XPLEX) ) : TAU value at beginning of diagnostic interval
!  (20) DIAGe       (TYPE (XPLEX) ) : TAU value at end       of diagnostic interval
!  (21) GMT         (TYPE (XPLEX) ) : Current Greenwich Mean Time (0.0 - 23.999 hrs)
...
!  (32) JD85        (TYPE (XPLEX) ) : Astronomical Julian Day at 0 GMT 1/1/1985 
...
!
      ...
      INTEGER           :: NDIAGTIME
      TYPE (XPLEX)            :: TAU,        TAUb,        TAUe  
      TYPE (XPLEX)            :: GMT,        DIAGb,       DIAGe
      ...
      TYPE (XPLEX), PARAMETER :: JD85 = xplex(2446066.5d0,0d0)
...
#     include "define.h"
     ...
     &              ( DBLE( SECOND ) / 3600d0 )
      ...
      ! NOTE: TMP is TYPE (XPLEX) to prevent precision problems
      TMP         = ( JD1 - JD85 ) * 24d0
      TAU         = ( TMP )
...
!
     ...
     &                    'SET_BEGIN_TIME (time_mod.f)' )
      ...
      ! TAUb value (TMP is TYPE (XPLEX) to prevent precision problems)
      print*,'GET_JD(NYMDb,NHMSb)',GET_JD( NYMDb, NHMSb )
      print*,'JD85',JD85
      print*,'( GET_JD( NYMDb, NHMSb)-JD85)',(GET_JD(NYMDb,NHMSb)-JD85)
      print*,'( GET_JD( NYMDb, NHMSb ) - JD85 ) * 24d0',
     & ( GET_JD( NYMDb, NHMSb ) - JD85 ) * 24d0
      TMP   = ( GET_JD( NYMDb, NHMSb ) - JD85 ) * 24d0
      TAUb  = ( TMP )
...
!
     ...
     &                    'SET_END_TIME (time_mod.f)' )
      ...
      ! TAUe value (TMP is TYPE (XPLEX) to prevent precision problems)
      TMP   = ( GET_JD( NYMDe, NHMSe ) - JD85 ) * 24d0
      TAUe  = ( TMP )
...
!
      ! Arguments
      TYPE (XPLEX), INTENT(IN) :: THISDIAGb
...
!
      ! Arguments
      TYPE (XPLEX), INTENT(IN) :: THISDIAGe
...
!
      ...
      IF ( DIRECTION < 0 ) THEN 
         ...
         IF ( NHMS >= 230000 ) THEN 
            ...
            ! if future year not the same as present...
            IF ( YY_FWD /= YEAR .OR. DD_FWD .EQ. DD_END ) THEN 
...
!
      ...
      IF ( DIRECTION < 0 ) THEN 
         ...
         IF ( NHMS >= 230000 ) THEN 
            ...
            IF ( MM_FWD /= MONTH .OR. DD_FWD .EQ. DD_END ) THEN
