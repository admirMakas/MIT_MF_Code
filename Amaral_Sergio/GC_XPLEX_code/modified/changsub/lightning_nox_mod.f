! $Id: lightning_nox_mod.f,v 1.1.1.1 2009/06/09 21:51:54 daven Exp $
...
!
      ...
      !REAL*8,  ALLOCATABLE :: EMIS_LI_NOx_ADJ(:,:,:)!cshim
...
!      SUBROUTINE LIGHTNING
      SUBROUTINE LIGHTNING (DIRECTION) !dkh
!
!******************************************************************************
...
#     include "CMN_GCTM"     ! Physical constants
      ...
      ! Arguments
      INTEGER, INTENT(IN): DIRECTION !dkh
...
!      SUBROUTINE EMLIGHTNING( I, J )
      SUBROUTINE EMLIGHTNING( I, J, DIRECTION ) !dkh
!
!******************************************************************************
...
#     include "CMN_DIAG"  ! ND32
      ...
      ! added for adjoint (dkh)
#     include "CMN_SETUP" !size para
!     inclde "CMN_ADJ" !IDADJxx what is new?
      ...
      DO L = 1, LLCONVM 
       ...
       !add checkpointing of SLBASE (dkh)
         IF ( DIRECTION     == 1 ) THEN
         ...
         ELSE
            ...
            CALL ERROR_STOP('Bad DIRECTION', 'lightning_nox_mod.f')
         ...
         ENDIF
          ...
          ! conversion units (SLBASE(I,J,L) has [molec NOx/6h/box]
          IF (MMSCL /= 1) CALL ERROR_STOP('M not 1', 'emlightning')
          TMP        = TMP * EMS_SF(I,J,1,IDADJEMS_ENOxli) !dkh
          !EMIS_LI_NOx(I,J,L) = TMP
          EMIS_LI_NOx(I,J,L) = EMIS_LI_NOx(I,J,L) +  TMP !total? cs
...
!----------------------------------------------------------------------------
      FUNCTION GET_SLBASE( I, J, L ) RESULT( SLBASE_IJL )
!
!******************************************************************************
!  Function GET_SLBASE returns value of I,J,L element of SLBASE. (dkh,
!  02/10/07) 
! 
!
!  Arguments as Input:
!  ============================================================================
!  (1-3) I,J,L (INTEGER)    : 3-D array location
!  [unitess]
!     
!  Arguments as Output:
!  ============================================================================
!  (1 ) SLBASE_IJL (REAL*8) : hmmmm
!  [NOx/6h/box]
!     
!  NOTES:
!
...
!
      ...
      !=================================================================
      ! GET_SLBASE begins here!
      !=================================================================

      SLBASE_IJL = SLBASE(I,J,L)

      ! Return to calling program
      END FUNCTION GET_SLBASE
...
#     include "CMN_SIZE"      ! Size parameters
      ...
      ! for adjoint : dkh 
      ! Allocate SLBASE_0  (dkh, 06/29/06)  
      !ALLOCATE( SLBASE_0( IIPAR, JJPAR, LLPAR ), STAT=AS )
      ALLOCATE( SLBASE_CHK( IIPAR, JJPAR, LLPAR ), STAT=AS )
      IF ( AS /= 0 ) CALL ALLOC_ERR( 'SLBASE_CHK' )
      SLBASE_CHK = 0d0

      ALLOCATE( EMS_orig_li( IIPAR, JJPAR ), STAT=AS )
      IF ( AS /= 0 ) CALL ALLOC_ERR( 'EMS_orig_li' )
      EMS_orig_li = 0d0
      !------------------------------------------------
...
!
      ...
      IF ( ALLOCATED( EMS_orig_li  ) ) DEALLOCATE(EMS_orig_li  )
