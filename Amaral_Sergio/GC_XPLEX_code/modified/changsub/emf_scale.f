! $Id: emf_scale.f,v 1.1.1.1 2009/06/09 21:51:54 daven Exp $
...
!
      ...
      !========== any updates? dkh?=============================
      USE SULFATE_MOD, ONLY:GET_SCALE_GROUP
...
!============================================================================
      ! Check to see if this is part of an adjoint simulation. (dkh,
      ! 06/06/06)  
      IF ( LADJ ) THEN
!
!*****************************************************************************
!  Get current adjoint scaling group M
!*****************************************************************************
!
         M = GET_SCALE_GROUP()
!
!*****************************************************************************
!  Scale emissions by adjoint scaling 
!  M is current scale group
!*****************************************************************************
!
         IF ( N == IDENOX ) THEN
            EMISRN(IREF,JREF,1:NOXLEVELS) =
     &         EMISRN(IREF,JREF,1:NOXLEVELS)
     &         * EMS_ICS(IREF,JREF,M,IDADJEMS_ENOx1:IDADJEMS_ENOx2)

            ! dkh debug
            IF ( L_PRINTFD .and.  IREF == IFD .and. JREF == JFD ) THEN
               print*, 'EMF SCALE: ',
     &         EMS_ICS(IREF,JREF,M,IDADJEMS_ENOx1:IDADJEMS_ENOx2),
     &         EMISRN(IREF,JREF,1:NOXLEVELS)
            ENDIF
