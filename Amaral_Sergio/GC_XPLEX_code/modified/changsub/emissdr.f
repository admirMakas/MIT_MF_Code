! $Id: emissdr.f,v 1.1.1.1 2009/06/09 21:51:54 daven Exp $
!      SUBROUTINE EMISSDR
      SUBROUTINE EMISSDR (DIRECTION) !dkh
!
!******************************************************************************
...
!  add direction arguement (dkh)
            IF ( LLIGHTNOX ) CALL EMLIGHTNING( I, J, DIRECTION )
...
!          add direction arguement (dkh)
     &         CALL SOILNOXEMS( SUNCOS, DIRECTION )
!-----------------------------------------------------------------------------
...
!
               ...
               IF ( NO_TAGCO ) THEN
                  ...
                  ! now track anthro and bio separately (dkh)
                  !EMISRR(I,J,IDECO) = EMISRR(I,J,IDECO) + TMPVAL
                  EMISRRB(I,J,IDECO) = EMISRRB(I,J,IDECO) + TMPVAL
...
!     &                                     ( EMIS / DTSRCE ) * BIOSCAL
!                   now track anthro and biog separately (dkh)
                     EMISRRB(I,J,IDEPRPE) = EMISRRB(I,J,IDEPRPE) +
     &                                     ( EMIS / DTSRCE ) * BIOSCAL
                  ENDIF
               ENDIF
