...
!  Module Variables:
!  ============================================================================
!  (1 ) AI          (TYPE (XPLEX) )  : Vertical coord "A" for hybrid grid [hPa]
!  (2 ) BI          (TYPE (XPLEX) )  : Vertical coord "B" for hybrid grid [unitless]
!  (3 ) CLAT_FV     (TYPE (XPLEX) )  : Grid box center latitude [radians] 
!  (4 ) COSE_FV     (TYPE (XPLEX) )  : COSINE of grid box edge latitudes [radians]
!  (5 ) COSP_FV     (TYPE (XPLEX) )  : COSINE of grid box center latitudes [radians]
!  (6 ) DAP         (TYPE (XPLEX) )  : Delta-A vertical coordinate [hPa]
!  (7 ) DBK         (TYPE (XPLEX) )  : Delta-B vertical coordinate [unitless]
!  (8 ) DLAT_FV     (TYPE (XPLEX) )  : Latitude extent of grid boxes [radians]
!  (9 ) ELAT_FV     (TYPE (XPLEX) )  : Grid box edge latitudes [radians]
!  (10) GEOFAC      (TYPE (XPLEX) )  : Geometric factor for N-S advection
!  (11) GW_FV       (TYPE (XPLEX) )  : Diff of SINE btw grid box lat edges [unitless]
!  (12) MCOR        (TYPE (XPLEX) )  : Grid box surface areas [m2]
!  (13) REL_AREA    (TYPE (XPLEX) )  : Relative surface area of grid box [fraction]
!  (14) RGW_FV      (TYPE (XPLEX) )  : Reciprocal of GW_FV [radians
!  (15) SINE_FV     (TYPE (XPLEX) )  : SINE of lat at grid box edges [unitless]
!  (16) GEOFAC_PC   (TYPE (XPLEX) )  : Geometric factor for N-S advection @ poles
!  (17) DLON_FV     (TYPE (XPLEX) )  : Longitude extent of a grid box [radians]
!  (18) LOC_PROC    (TYPE (XPLEX) )  : Local processor number
...
!
      ...
      ! Allocatable arrays
      TYPE (XPLEX), ALLOCATABLE :: AI(:)
      TYPE (XPLEX), ALLOCATABLE :: BI(:)
      TYPE (XPLEX), ALLOCATABLE :: CLAT_FV(:)
      TYPE (XPLEX), ALLOCATABLE :: COSE_FV(:)
      TYPE (XPLEX), ALLOCATABLE :: COSP_FV(:)
      TYPE (XPLEX), ALLOCATABLE :: DAP(:)
      TYPE (XPLEX), ALLOCATABLE :: DBK(:)
      TYPE (XPLEX), ALLOCATABLE :: DLAT_FV(:)
      TYPE (XPLEX), ALLOCATABLE :: ELAT_FV(:)
      TYPE (XPLEX), ALLOCATABLE :: GEOFAC(:)
      TYPE (XPLEX), ALLOCATABLE :: GW_FV(:)
      TYPE (XPLEX), ALLOCATABLE :: MCOR(:,:)
      TYPE (XPLEX), ALLOCATABLE :: REL_AREA(:,:)
      TYPE (XPLEX), ALLOCATABLE :: RGW_FV(:)
      TYPE (XPLEX), ALLOCATABLE :: SINE_FV(:)
 
      ! Scalar variables
      LOGICAL             :: PR_DIAG
      INTEGER             :: LOC_PROC
      TYPE (XPLEX)              :: GEOFAC_PC
      TYPE (XPLEX)              :: DLON_FV
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) D_DYN (TYPE (XPLEX)) : Dynamic timestep [s]
!  (2 ) P1    (TYPE (XPLEX)) : True PSurface at middle of dynamic timestep [hPa]
!  (3 ) P2    (TYPE (XPLEX)) : True PSurface at end    of dynamic timestep [hPa]
!  (4 ) UWND  (TYPE (XPLEX)) : Zonal (E-W) wind [m/s]
!  (5 ) VWND  (TYPE (XPLEX)) : Meridional (N-S) wind [m/s]
!
!  Arguments as Input:
!  ============================================================================
!  (6 ) XMASS (TYPE (XPLEX)) : E-W mass fluxes [kg/s ??]
!  (7 ) YMASS (TYPE (XPLEX)) : N-S mass fluxes [kg/s ??]
!  
!  NOTES:
...
#     include "CMN_GCTM"    ! Physical constants
      ...
      ! Arguments
      TYPE (XPLEX),  INTENT(IN)  :: D_DYN
      TYPE (XPLEX),  INTENT(IN)  :: P1(IIPAR,JJPAR)
      TYPE (XPLEX),  INTENT(IN)  :: P2(IIPAR,JJPAR)
      TYPE (XPLEX),  INTENT(IN)  :: UWND(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX),  INTENT(IN)  :: VWND(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX),  INTENT(OUT) :: XMASS(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX),  INTENT(OUT) :: YMASS(IIPAR,JJPAR,LLPAR)

      ! Local variables
      LOGICAL, SAVE        :: FIRST = .TRUE.
      INTEGER              :: I, J
      TYPE (XPLEX)               :: P2_TMP(IIPAR,JJPAR)
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) XMASS    (TYPE (XPLEX)) : E-W mass flux from pressure fixer
!  (2 ) YMASS    (TYPE (XPLEX)) : N-S mass flux from pressure fixer
!  (3 ) RGW_FV   (TYPE (XPLEX)) : 1 / ( SINE(J+1) - SINE(J) ) -- latitude factor
!  (4 ) PS_NOW   (TYPE (XPLEX)) : Surface pressure - PTOP at current time
!
!  Arguments as Output:
!  ============================================================================
!  (5 ) PS_AFTER (TYPE (XPLEX)) : Surface pressure - PTOP adjusted by P-fixer
!
!  NOTES:
!******************************************************************************
!
...
#     include "CMN"       ! STT, NTRACE, LPRT, LWINDO
      ...
      ! Arguments
      TYPE (XPLEX), INTENT(IN)  :: XMASS(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX), INTENT(IN)  :: YMASS(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX), INTENT(IN)  :: PS_NOW(IIPAR,JJPAR)
      TYPE (XPLEX), INTENT(IN)  :: RGW_FV(JJPAR)
      TYPE (XPLEX), INTENT(OUT) :: PS_AFTER(IIPAR,JJPAR)

      ! Local variables
      INTEGER             :: I, J, L
      TYPE (XPLEX)              :: DELP(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX)              :: DELP1(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX)              :: PE(IIPAR,LLPAR+1,JJPAR)
...
#     include "CMN_GCTM"
      ...
      TYPE (XPLEX), intent(IN)  :: mcor    (i1_gl :i2_gl, ju1_gl:j2_gl)
      TYPE (XPLEX), intent(OUT) :: rel_area(i1_gl :i2_gl, ju1_gl:j2_gl)
      TYPE (XPLEX), intent(OUT) :: geofac  (ju1_gl:j2_gl)
      TYPE (XPLEX), intent(OUT) :: geofac_pc
      ...
      integer :: ij
      integer :: il   ! (lzh, 07/21/2010)
 
      TYPE (XPLEX)  :: dp           ! spacing in latitude (rad)
      TYPE (XPLEX)  :: ri2_gl
      TYPE (XPLEX)  :: rj2m1
      TYPE (XPLEX)  :: total_area
...
! (lzh, 07/21/2010)
        geofac(ij) =  1d0 / COSP_FV(ij)
      end do

!      geofac_pc =
!     &  dp / (2.0d0 * Sum (rel_area(1,ju1_gl:ju1_gl+1)) * ri2_gl)
! (lzh, 07/21/2010)
        geofac_pc =  1d0 / COSP_FV(ju1_gl)
...
!
      ...
      TYPE (XPLEX)  :: tdt
      TYPE (XPLEX)  :: geofac_pc
      TYPE (XPLEX)  :: geofac  (ju1_gl:j2_gl)
      TYPE (XPLEX)  :: cose    (ju1_gl:j2_gl)
      TYPE (XPLEX)  :: cosp    (ju1_gl:j2_gl)
      TYPE (XPLEX)  :: dap     (k1:k2)
      TYPE (XPLEX)  :: dbk     (k1:k2)

      !-----------------------------------------------------------------
      !rel_area : relative surface area of grid box (fraction)
      !-----------------------------------------------------------------
 
      TYPE (XPLEX) :: rel_area( i1_gl:i2_gl,   ju1_gl:j2_gl)
     ...
     &   pctm2(ilo_gl:ihi_gl, julo_gl:jhi_gl)
      ...
      TYPE (XPLEX) :: uu(ilo_gl:ihi_gl, julo_gl:jhi_gl, k1_gl:k2_gl)
      TYPE (XPLEX) :: vv(ilo_gl:ihi_gl, julo_gl:jhi_gl, k1_gl:k2_gl)
      ...
      TYPE (XPLEX) :: xmass(ilo_gl:ihi_gl, julo_gl:jhi_gl, k1_gl:k2_gl)
      TYPE (XPLEX) :: ymass(ilo_gl:ihi_gl, julo_gl:jhi_gl, k1_gl:k2_gl)
      ...
      TYPE (XPLEX)  :: dgpress
      TYPE (XPLEX)  :: press_dev

      !-------------------------------------------------------------
      !dps : change of surface pressure from met field pressure (mb)
      !-------------------------------------------------------------

      TYPE (XPLEX)  :: dps(i1_gl:i2_gl, ju1_gl:j2_gl)

      !--------------------------------------------
      !dps_ctm : CTM surface pressure tendency (mb)
      !--------------------------------------------

      TYPE (XPLEX) :: dps_ctm(i1_gl:i2_gl, ju1_gl:j2_gl)
      ...
      TYPE (XPLEX)  :: xmass_fixed(ilo_gl:ihi_gl, julo_gl:jhi_gl, k1:k2)
      TYPE (XPLEX)  :: ymass_fixed(ilo_gl:ihi_gl, julo_gl:jhi_gl, k1:k2)
...
! 
     ...
     &   pctm2(ilo_gl:ihi_gl, julo_gl:jhi_gl)
      ...
      TYPE (XPLEX)  :: crx  (ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: cry  (ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: delp1(ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: delpm(ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: pu   (ilo:ihi, julo:jhi, k1:k2)
...
!     &  (rel_area, pmet2)
     ...
     &  (.false., geofac_pc, geofac, dpi, xmass, ymass)
      ...
      dps_ctm(i1:i2,ju1:j2) = xplx(Sum(dpi(i1:i2,ju1:j2,:)%r, 3),
     &                             Sum(dpi(i1:i2,ju1:j2,:)%i, 3))
...
!
      ...
      TYPE (XPLEX)  :: geofac  (ju1_gl:j2_gl)
      TYPE (XPLEX)  :: dbk     (k1:k2)
      TYPE (XPLEX)  :: dps     (i1:i2, ju1:j2)
      TYPE (XPLEX)  :: dps_ctm (i1:i2, ju1:j2)
      TYPE (XPLEX)  :: rel_area(i1:i2, ju1:j2)
      TYPE (XPLEX)  :: xmass      (ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: ymass      (ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: xmass_fixed(ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: ymass_fixed(ilo:ihi, julo:jhi, k1:k2)
      ...
      TYPE (XPLEX)  :: fxintegral(i1:i2+1)

      TYPE (XPLEX)  :: mmfd(ju1:j2)
      TYPE (XPLEX)  :: mmf (ju1:j2)

      TYPE (XPLEX)  :: ddps(i1:i2, ju1:j2)
      ...
      TYPE (XPLEX)  :: xcolmass_fix(ilo:ihi, julo:jhi)
...
! not use buffer region (lzh, 08/02/2010) 
      xx = sum(rel_area((i1+iij):(i2-iij),(ju1+iij):(j2-iij)))
      dgpress = 
     &   sum( ddps((i1+iij):(i2-iij),(ju1+iij):(j2-iij))*
     &        rel_area((i1+iij):(i2-iij),(ju1+iij):(j2-iij) )) / xx 
...
!      do ij = j1p+iij, j2p-iij
!        mmfd(ij) = -(sum(ddps(:,ij)) / ri2 - dgpress) 
!      end do
! not use buffer region (lzh, 08/02/2010)
      do ij = j1p+iij, j2p-iij
        mmfd(ij) = -(sum(ddps((1+iij):(i2-iij),ij)) / ri2 - dgpress) 
      end do
...
!       mmf(j1p) = mmfd(ju1) / geofac_pc
       mmf(j1p+iij) = mmfd(ju1+iij) / geofac(ju1+iij)

       do ij = j1p+iij, j2p-iij-1
...
! modified for the buffer region (lzh, 08/02/2010)
      do ij = j1p+iij, j2p-iij
...
! modified for the buffer region (lzh, 08/02/2010)
     ...
     &                              xcolmass_fix(il,ij) * dbk(ik)
      ...
      do ik = k1, k2      
        do ij = j1p+iij, j2p-iij  ! (lzh, 07/26/2010) ! j2p+1
          do il = i1+iij, i2-iij
...
! 
      ...
      TYPE (XPLEX)  :: rel_area(i1:i2, ju1:j2)
      TYPE (XPLEX)  :: press   (ilo:ihi, julo:jhi)
...
#     include "CMN_GCTM" ! Re, PI
      ...
      TYPE (XPLEX)  :: crx (ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: cry (ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: uu  (ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: vv  (ilo:ihi, julo:jhi, k1:k2)
      ...
      TYPE (XPLEX), save :: dtdy
      TYPE (XPLEX), save :: dtdy5
      ...
      TYPE (XPLEX), allocatable, save :: dtdx (:)                   
...
!      do ij = j1p, j2p
      do ij = j1p, j2p-1    ! (lzh, 07/21/2010)
...
!
      ...
      TYPE (XPLEX)  :: dap  (k1:k2)
      TYPE (XPLEX)  :: dbk  (k1:k2)
      TYPE (XPLEX)  :: pres1(ilo:ihi, julo:jhi)
      TYPE (XPLEX)  :: pres2(ilo:ihi, julo:jhi)
      TYPE (XPLEX)  :: delp1(ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: delpm(ilo:ihi, julo:jhi, k1:k2)
      TYPE (XPLEX)  :: pu   (ilo:ihi, julo:jhi, k1:k2)
...
!
      ...
      TYPE (XPLEX)  :: mean_np(k1:k2)
      TYPE (XPLEX)  :: mean_sp(k1:k2)
      TYPE (XPLEX)  :: sumnp  (k1:k2)
      TYPE (XPLEX)  :: sumsp  (k1:k2)
...
!  Arguments as Input:
!  ============================================================================
!  (1 ) P  (TYPE (XPLEX))  :: 1-D vector to be averaged
!  (2 ) IM (INTEGER) :: Dimension of P
!
!  Arguments as Output:
!  ============================================================================
!  (1 ) P  (TYPE (XPLEX))  :: Contains average value of P in each element
!
!  NOTES:
!******************************************************************************
!
...
#     include "CMN_GCTM"  ! Re, PI, etc...
     ...
     &                    'INIT_PJC_FIX ("pjc_pfix_mod.f")' )
      ...
      ! Longitude spacing
      DLON_FV    = 2.d0 * PI / XPLX( 540 )        !(dan)
       
      ! Latitude edge at south pole [radians]
!      ELAT_FV(1) = -0.5d0 * PI 
! for nested NA or EA (lzh, 07/20/2010) 
      ELAT_FV(1) = CLAT_FV(1) - 0.25d0 * PI/XPLX(180)
...
! for nested NA or EA (lzh, 07/20/2010) 
      ELAT_FV(J2_GL+1) = CLAT_FV(J2_GL)+0.25d0* PI/XPLX(180)
