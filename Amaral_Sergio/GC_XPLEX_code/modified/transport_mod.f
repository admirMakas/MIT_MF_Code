...
!  Module Variables:
!  ============================================================================
!  (1 ) Ap     (TYPE (XPLEX) )       : Vertical coordinate array for TPCORE
!  (2 ) A_M2   (TYPE (XPLEX) )       : Grid box surface areas [m2]
!  (3 ) Bp     (TYPE (XPLEX) )       : Vertical coordinate array for TPCORE
!  (4 ) IORD   (TYPE (XPLEX) )       : TPCORE E/W option flag
!  (5 ) JORD   (TYPE (XPLEX) )       : TPCORE N/S option flag
!  (6 ) KORD   (TYPE (XPLEX) )       : TPCORE vertical option flag
...
!
      ...
      TYPE (XPLEX), ALLOCATABLE :: A_M2(:)
      TYPE (XPLEX), ALLOCATABLE :: Bp(:)
      TYPE (XPLEX), ALLOCATABLE :: STT_BC2(:,:,:)
...
#     include "CMN_SIZE" ! Size parameters
      ...
      ELSE
         ...
         !print*,'STT IMAG NOX DO TRANSPORT',(STT(40,40,1,IDTNOX))
...
#     include "CMN_DIAG"         ! NDxx flags
      ...
      TYPE (XPLEX)                    :: A_DIFF, D_DYN, TR_DIFF
      TYPE (XPLEX)                    :: AD_A(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX)                    :: AD_B(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX)                    :: P_TP1(IIPAR,JJPAR)
      TYPE (XPLEX)                    :: P_TP2(IIPAR,JJPAR)
      TYPE (XPLEX)                    :: P_TEMP(IIPAR,JJPAR)
      TYPE (XPLEX)                    :: TR_A(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX)                :: TR_B(IIPAR,JJPAR,LLPAR,N_TRACERS)
      TYPE (XPLEX)                    :: UTMP(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX)                    :: VTMP(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX)                    :: XMASS(IIPAR,JJPAR,LLPAR) 
      TYPE (XPLEX)                    :: YMASS(IIPAR,JJPAR,LLPAR) 
...
!            ENDIF
         ...
         TR_DIFF%r = SUM( TR_B(:,:,:,N)%r ) - SUM( TR_A%r )
         TR_DIFF%i = SUM( TR_B(:,:,:,N)%i ) - SUM( TR_A%i )
         ...
         !-------------------------------------------------------------------
         TR_DIFF = SAFE_DIV(TR_DIFF, SUMADA, xplx(0.d0)) * TCVV(N)
...
#     include "CMN_DIAG"         ! NDxx flags
      ...
      TYPE (XPLEX)                    :: UTMP(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX)                    :: VTMP(IIPAR,JJPAR,LLPAR)
      TYPE (XPLEX)                    :: WW(IIPAR,JJPAR,LLPAR) 
      ...
      TYPE (XPLEX),  PARAMETER        :: Umax=xplex(200d0,0d0)
...
#     include "CMN_GCTM"  ! Re
      ...
      ! Parameters
      INTEGER, PARAMETER  :: IGD=0, J1=3
      TYPE (XPLEX),  PARAMETER  :: Umax=xplex(150d0,0d0)
...
!  (4  ) PSURF   (TYPE (XPLEX) ) : Surface pressure at grid box (I,J,L=1)
!
!  NOTES:
!******************************************************************************
!     
...
#     include "CMN_GCTM"  ! g0_100
      ...
      TYPE (XPLEX),  INTENT(IN) :: P_SURF

      ! Local variables
      INTEGER             :: L2
      TYPE (XPLEX)              :: P_BOT, P_TOP, AIR_MASS
