...
#define PRT_PREFIX
 ...
 TYPE (XPLEX) ,ALLOCATABLE, save :: dtdx5(:)
 TYPE (XPLEX) ,ALLOCATABLE, save :: dtdy5(:)
 TYPE (XPLEX) ,ALLOCATABLE, save :: cosp(:)
 TYPE (XPLEX) ,ALLOCATABLE, save :: cose(:)
 TYPE (XPLEX) ,ALLOCATABLE, save ::  gw(:)
 TYPE (XPLEX) ,ALLOCATABLE, save :: rgw(:)
 ...
 !%%% (bdf, bmy, 5/7/03)
 !%%%
 TYPE (XPLEX), ALLOCATABLE, SAVE :: DLAT(:)
 !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 !%%% MODIFICATION by Harvard Atmospheric Chemistry Modeling Group
 !%%%
 ...
 !%%%
 TYPE (XPLEX), ALLOCATABLE, SAVE :: RGW_25(:) 
 TYPE (XPLEX), ALLOCATABLE, SAVE :: SINE_25(:)    !(dan 0803)
 !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
!-------
 ...
 TYPE (XPLEX), intent(in):: dt          ! Time step in seconds
 TYPE (XPLEX), intent(in):: ae          ! Earth's radius (m)
 TYPE (XPLEX), intent(in):: clat(0:jm+1)    ! latitude in radian  (dan)

!-----
! Local
!-----

 TYPE (XPLEX) elat(jm+1)    ! cell edge latitude in radian
 TYPE (XPLEX) sine(jm+1)
 ...
 !%%%
 !%%%TYPE (XPLEX) dlat(jm)      ! delta-latitude in radian
 !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 !%%% MODIFICATION by Harvard Atmospheric Chemistry Modeling Group
 !%%%
...
! *PPM: Piece-wise Parabolic Method
 ...
 TYPE (XPLEX), intent(in):: ak(km+1)              ! See below
 TYPE (XPLEX), intent(in):: bk(km+1)              ! See below
 TYPE (XPLEX), intent(in):: u(im,jfirst:jlast,km) ! u-wind (m/s) at mid-time-level (t=t+dt/2)
 TYPE (XPLEX), intent(inout):: v(im,jfirst-mg:jlast+mg,km) ! v-wind (m/s) at mid-time-level (t=t+dt/2)
...
! The latitudes are given by clat, input to the initialization routine: init_tpcore.
 ...
 TYPE (XPLEX), intent(in):: ps1(im,jfirst:jlast)  ! surface pressure at current time
 TYPE (XPLEX), intent(in):: ps2(im,jfirst:jlast)  ! surface pressure at future time=t+dt
 TYPE (XPLEX), intent(in):: dt                    ! Transport time step in seconds
 TYPE (XPLEX), intent(in):: ae                    ! Earth's radius (m)

 TYPE (XPLEX), intent(inout):: q(im,jfirst-ng:jlast+ng,km,nq)  ! Tracer "mixing ratios"
                                          ! q could easily be re-dimensioned
 TYPE (XPLEX), intent(out):: ps(im,jfirst:jlast)  ! "predicted" surface pressure

 TYPE (XPLEX)  delp(im,jfirst:jlast,km)    ! Predicted thickness at future time (t=t+dt)
 TYPE (XPLEX)  pe(im,km+1,jfirst:jlast)    ! Pressure at layer edges (predicted)

 TYPE (XPLEX)  fx(im,jfirst:jlast,km)     ! E-W air mass flux
 TYPE (XPLEX)  va(im,jfirst:jlast,km)     ! N-S CFL at cell center (scalar points)
 ...
 !%%%
 TYPE (XPLEX),    INTENT(IN)    :: XMASS(IM,JM,KM), YMASS(IM,JM,KM)
 !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 !%%%
 TYPE (XPLEX),    INTENT(INOUT) :: MASSFLEW(IM,JM,KM,NQ) ! east/west mass flux
 TYPE (XPLEX),    INTENT(INOUT) :: MASSFLNS(IM,JM,KM,NQ) ! north/south mass flux
 TYPE (XPLEX),    INTENT(INOUT) :: MASSFLUP(IM,JM,KM,NQ) ! up/down vertical mass flux
 TYPE (XPLEX),    INTENT(IN)    :: AREA_M2(JM)           ! box area for mass flux diag
 TYPE (XPLEX),    INTENT(IN)    :: TCVV(NQ)              ! tracer masses for flux diag
...
!-----------------------
 ...
 TYPE (XPLEX)  cx(im,jfirst-ng:jlast+ng,km)    ! E-W CFL number on C-grid
 TYPE (XPLEX) delp1(im,jfirst-mg:jlast+mg,km)  ! Pressure thickness at current time (t)
 TYPE (XPLEX)   fy(im,jfirst:jlast+mg,km)      ! N-S air mass flux
 TYPE (XPLEX)   cy(im,jfirst:jlast+mg,km)      ! N-S CFL number on C-grid
 TYPE (XPLEX)  psg(im,jfirst-mg:jlast+mg,2)    ! Was psm and psn
 TYPE (XPLEX)   q2(im,jfirst-ng:jlast+ng)      ! local 2D q array
...
! Local variables:
 ...
 !%%%
 TYPE (XPLEX) DTC(IM,JM,KM,NQ)              ! up/down flux temp array
 TYPE (XPLEX) QTEMP(IM,JM,KM,NQ)            ! up/down flux array
 TYPE (XPLEX) TRACE_DIFF                    ! up/down flux variable
 !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
!---------------------------------------------------------------
 ...
 !%%% conditions.  Start from top down (TYPE (XPLEX)ly surface up for flipped TPCORE)
 !%%%
 IF ( ND26 > 0 ) THEN
...
! (cx,cy): CFL number
 ...
 TYPE (XPLEX), intent(in):: dt
 TYPE (XPLEX), intent(in):: ae
 TYPE (XPLEX), intent(in):: ak(km+1)
 TYPE (XPLEX), intent(in):: bk(km+1)
 TYPE (XPLEX), intent(in):: psg(im,jfirst-mg:jlast+mg,2)   ! Was ps1 and ps2
 TYPE (XPLEX), intent(in):: u(im,jfirst:jlast,km)
 TYPE (XPLEX), intent(in):: v(im,jfirst-mg:jlast+mg,km)
...
! Output:
 ...
 TYPE (XPLEX), intent(out):: cx(im,jfirst-ng:jlast+ng,km)
 TYPE (XPLEX), intent(out):: delp (im,jfirst:jlast,km)

 TYPE (XPLEX), intent(out):: ps(im,jfirst:jlast)
 TYPE (XPLEX), intent(out):: fx(im,jfirst:jlast,km)
 TYPE (XPLEX), intent(out):: cy(im,jfirst:jlast+mg,km)
 TYPE (XPLEX), intent(out):: fy(im,jfirst:jlast+mg,km)
 TYPE (XPLEX), intent(out):: va(im,jfirst:jlast,km)

 TYPE (XPLEX), intent(out):: delp1(im,jfirst-mg:jlast+mg,km)

 TYPE (XPLEX), intent(out):: pe(im,km+1,jfirst:jlast)

! Local:
 TYPE (XPLEX) yms(im,jfirst:jlast+mg,km)

 TYPE (XPLEX)  tiny
 parameter (tiny = xplex(1.d-10,0d0))
 TYPE (XPLEX) dak, dbk
 TYPE (XPLEX) dtoa, vt
 integer i,j,k
 ...
 !%%%
 TYPE (XPLEX) :: DELPM(IM,JM,KM), FACTY, UT
 !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
! !INPUT PARAMETERS:
   ...
   TYPE (XPLEX), intent(in):: va(im,jfirst:jlast)   ! CFL in y at cell center 
   TYPE (XPLEX), intent(in):: dp(im,jfirst-mg:jlast+mg)
   TYPE (XPLEX), intent(in):: dpp(im,jfirst:jlast)

   TYPE (XPLEX), intent(in):: crx(im,jfirst-ng:jlast+ng) ! ( N*NG S*NG )
   TYPE (XPLEX), intent(in):: cry(im,jfirst:jlast+mg)    ! ( N like FY )

   TYPE (XPLEX), intent(in):: xfx(im,jfirst:jlast)       ! x-mass flux
   TYPE (XPLEX), intent(in):: yfx(im,jfirst:jlast+mg)     ! y-mass flux

   TYPE (XPLEX), intent(inout) :: h(im,jfirst-ng:jlast+ng)
 ...
 !%%% 
   TYPE (XPLEX),    INTENT(INOUT) :: MFLEW(IM,JM)  ! E/W mass flux array
   TYPE (XPLEX),    INTENT(INOUT) :: MFLNS(IM,JM)  ! N/S mass flux array
   TYPE (XPLEX),    INTENT(IN)    :: AREA_M2(JM)   ! Grid bos surface area [m2]
   TYPE (XPLEX),    INTENT(IN)    :: TCV           ! Mass ratio
   INTEGER, INTENT(IN)    :: ND24          ! flux diag
   INTEGER, INTENT(IN)    :: ND25          ! flux diag
   TYPE (XPLEX),    INTENT(IN)    :: DT            ! time step for flux diagnostic
 !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

! Local
   TYPE (XPLEX) fx(im,jfirst:jlast)        ! tracer flux in x ( unghosted )
   TYPE (XPLEX) fy(im,jfirst:jlast+mg)     ! tracer flux in y ( N, see tp2c )
...
! !INPUT PARAMETERS:
 ...
 logical ffsl(jfirst-ng:jlast+ng)  ! Use flux-form semi-Lagrangian trans.?
                                   ! ghosted N*ng S*ng
 TYPE (XPLEX) va(im,jfirst:jlast)          ! Courant  (unghosted)
 TYPE (XPLEX) q(im,jfirst-ng:jlast+ng)     ! transported scalar ( N*NG S*NG )
 TYPE (XPLEX) crx(im,jfirst-ng:jlast+ng)   ! Ask S.-J. ( N*NG S*NG )
 TYPE (XPLEX) cry(im,jfirst:jlast+mg)       ! Ask S.-J. ( N like FY )
 TYPE (XPLEX) xfx(im,jfirst:jlast)         ! Ask S.-J. ( unghosted like FX )
 TYPE (XPLEX) yfx(im,jfirst:jlast+mg)       ! Ask S.-J. ( N like FY )

! !OUTPUT PARAMETERS:
 TYPE (XPLEX) fx(im,jfirst:jlast)          ! Flux in x ( unghosted )
 TYPE (XPLEX) fy(im,jfirst:jlast+mg)        ! Flux in y ( N, see tp2c )

! Local:
 integer i, j, iad, jp, js2g0, js2gng, jn2g0, jn2gng
 TYPE (XPLEX) adx(im,jfirst-ng:jlast+ng)
 TYPE (XPLEX) wk1(im)
 TYPE (XPLEX) dm(-im/3:im+im/3)
 TYPE (XPLEX) qtmp(-im/3:im+im/3)
 TYPE (XPLEX) al(-im/3:im+im/3)
 TYPE (XPLEX) ar(-im/3:im+im/3)
 TYPE (XPLEX) a6(-im/3:im+im/3)
  ...
  enddo
    ...
    call ytp(im,jm,fy, adx,cry,yfx,ng,mg,jord,0,jfirst,jlast)
      ...
      do j=js2g0,jn2g0
        do i=1,im
           jp = j-va(i,j)
            IF (J .GT. JJPAR .or. JP+1 .gt. JJPAR ) THEN
                PRINT *, 'ddd jk ERROR L1968 tpcore_geos5_window J .GT. JJPAR'
            ELSE
...
! !INPUT PARAMETERS:
   ...
   TYPE (XPLEX) c(im)          ! Courant numbers
   TYPE (XPLEX) q(im)
   TYPE (XPLEX) mfx(im)
   logical ffsl
   integer iord
...
! !INPUT/OUTPUT PARAMETERS:
   TYPE (XPLEX) qtmp(-im/3:im+im/3)   ! Input work arrays:
   TYPE (XPLEX) dm(-im/3:im+im/3)
   TYPE (XPLEX) al(-im/3:im+im/3)
   TYPE (XPLEX) ar(-im/3:im+im/3)
   TYPE (XPLEX) a6(-im/3:im+im/3)
...
! Local:
   TYPE (XPLEX) cos_upw               !critical cosine for upwind
   TYPE (XPLEX) cos_van               !critical cosine for van Leer
   TYPE (XPLEX) cos_ppm               !critical cosine for ppm

   parameter (cos_upw = xplex(0.05,0d0))       !roughly at 87 deg.
   parameter (cos_van = xplex(0.25,0d0))       !roughly at 75 deg.
   parameter (cos_ppm = xplex(0.25,0d0))

   integer i, imp
   TYPE (XPLEX) qmax, qmin
   TYPE (XPLEX) rut, tmp
   integer iu, itmp, ist
   integer isave(im)
   integer iuw, iue
...
! !INPUT PARAMETERS:
 ...
 integer, intent(in):: id       ! ID = 0: density (mfx = C)
                                ! ID = 1: mixing ratio (mfx is mass flux)
 TYPE (XPLEX), intent(in):: q(-im/3:im+im/3)   ! scalar field

! !OUTPUT PARAMETERS:
 TYPE (XPLEX), intent(out):: dm(-im/3:im+im/3)   ! 

! Local
 TYPE (XPLEX) r24
 parameter( r24 = xplex(1./24.,0d0))
 integer i
 TYPE (XPLEX) qmin, qmax
...
! !INPUT PARAMETERS:
 ...
 TYPE (XPLEX), intent(in):: c(im)
 TYPE (XPLEX), intent(in):: p(-im/3:im+im/3)
 TYPE (XPLEX), intent(in):: dm(-im/3:im+im/3)
 TYPE (XPLEX), intent(in):: mfx(im)

! !INPUT/OUTPUT PARAMETERS:
 integer, intent(inout):: isave(im)

 TYPE (XPLEX), intent(out):: fx(im)
 TYPE (XPLEX), intent(out):: al(-im/3:im+im/3)
 TYPE (XPLEX), intent(out):: ar(-im/3:im+im/3)
 TYPE (XPLEX), intent(out):: a6(-im/3:im+im/3)

! LOCAL VARIABLES:
 TYPE (XPLEX) r3, r23
 parameter ( r3 = xplex(1./3.,0d0), r23 = xplex(2./3.,0d0) )
...
! !INPUT PARAMETERS:
 ...
 TYPE (XPLEX), intent(in):: p(im)
 TYPE (XPLEX), intent(in):: dm(im)

 TYPE (XPLEX), intent(inout):: a6(im)
 TYPE (XPLEX), intent(inout):: ar(im)
 TYPE (XPLEX), intent(inout):: al(im)

! !LOCAL VARIABLES:
 TYPE (XPLEX) r12
 parameter ( r12 = xplex(1./12.,0d0) )

 TYPE (XPLEX) da1, da2, fmin, a6da
 TYPE (XPLEX) dr, dl
...
! !INPUT PARAMETERS:
 ...
 TYPE (XPLEX) q(im,jfirst-ng:jlast+ng)       !  advected scalar N*jord S*jord
 TYPE (XPLEX) c(im,jfirst:jlast+mg)           !  Courant   N (like FY)
 TYPE (XPLEX) yfx(im,jfirst:jlast+mg)         !  Backgrond mass flux

! !OUTPUT PARAMETERS:
 TYPE (XPLEX) fy(im,jfirst:jlast+mg)          !  Flux      N (see tp2c)
...
! work arrays (should pass in eventually for performance enhancement):
 TYPE (XPLEX) dm(im,jfirst-ng:jlast+ng)

!     TYPE (XPLEX) ar(im,jfirst-1:jlast+1)  ! AR needs to be ghosted on NS
!     TYPE (XPLEX) al(im,jfirst-1:jlast+2)  ! AL needs to be ghosted on N2S
!     TYPE (XPLEX) a6(im,jfirst-1:jlast+1)  ! A6 needs to be ghosted on NS
   ...
   if(jord == 1) then
        do j=js2g0,jn1g1
          do i=1,im
            jt = xplx(j) - c(i,j) 
            IF (JT .GT. JJPAR ) THEN
                PRINT *, 'ddd jk ERROR L1968 tpcore_geos5_windowJT .GT. JJPAR'
            ELSE
                fy(i,j) = q(i,jt)
            ENDIF
          enddo
...
! !OUTPUT PARAMETERS:
 TYPE (XPLEX) dm(im,jfirst-ng:jlast+ng)      !  Slope only N*(ng-1) S*(ng-1) used

! Local variables

 integer i, j, jm1, im2, js2gng1, jn2gng1
 TYPE (XPLEX) qmax, qmin, tmp
...
! !INPUT PARAMETERS:
 ...
 integer iv                          !  Scalar=0, Vector=1
 TYPE (XPLEX) q(im,jfirst-ng:jlast+ng) !  mean value needed only N*2 S*2
 TYPE (XPLEX) dm(im,jfirst-ng:jlast+ng) !  Slope     needed only N*2 S*2
 TYPE (XPLEX) c(im,jfirst:jlast+mg)     !  Courant   N (like FLUX)
...
! !OUTPUT PARAMETERS:
 TYPE (XPLEX) flux(im,jfirst:jlast+mg)   !  Flux      N (see tp2c)
...
!INPUT
   ...
   TYPE (XPLEX), intent(in):: ak(km+1)
   TYPE (XPLEX), intent(in):: bk(km+1)
   TYPE (XPLEX), intent(in):: pe(im,km+1,jfirst:jlast)
 
! INPUT/OUTPUT 
   TYPE (XPLEX) q(im,jfirst-ng:jlast+ng,km,nq) ! tracers including specific humidity
   TYPE (XPLEX) ps(im,jfirst:jlast)      ! surface pressure
 
! Local arrays:
  TYPE (XPLEX) pe2(im,km+1)
...
!INPUT PARAMETERS:
 ...
 TYPE (XPLEX) pe1(im,km+1)                ! pressure at layer edges 
                                        ! (from model top to bottom surface)
                                        ! in the original vertical coordinate
 TYPE (XPLEX) pe2(im,kn+1)                ! pressure at layer edges 
                                        ! (from model top to bottom surface)
                                        ! in the new vertical coordinate
 TYPE (XPLEX) q1(im,jfirst-ng:jlast+ng,km)     ! Field input

!OUTPUT PARAMETERS:
 TYPE (XPLEX) q2(im,jfirst-ng:jlast+ng,kn)     ! Field output

! LOCAL VARIABLES:

 TYPE (XPLEX) dp1(i1:i2,km)
 TYPE (XPLEX) q4(4,i1:i2,km)
 integer i, k, l, ll, k0
 TYPE (XPLEX)  pl, pr, qsum, delp, esl
 TYPE (XPLEX)       r3, r23
 TYPE (XPLEX) temp
 parameter (r3 = xplex(1./3.,0d0), r23 = xplex(2./3.,0d0))
...
! INPUT PARAMETERS:
 ...
 integer, intent(in):: kord    ! Order (or more accurately method no.):
                               ! 
 TYPE (XPLEX), intent(in):: delp(i1:i2,km)     ! layer pressure thickness

 TYPE (XPLEX), intent(inout):: a4(4,i1:i2,km)  ! Interpolated values

! local arrays.
 TYPE (XPLEX) dc(i1:i2,km)
 TYPE (XPLEX) h2(i1:i2,km)
 TYPE (XPLEX) delq(i1:i2,km)
 TYPE (XPLEX) df2(i1:i2,km)
 TYPE (XPLEX) d4(i1:i2,km)

 TYPE (XPLEX) fac
 TYPE (XPLEX) a1, a2, c1, c2, c3, d1, d2
 TYPE (XPLEX) qmax, qmin, cmax, cmin
 TYPE (XPLEX) qm, dq, tmp
 TYPE (XPLEX) qmp, pmp
 TYPE (XPLEX) lac
 integer lmt
 integer i, k, km1
...
!INPUT PARAMETERS:
 ...
 integer i1                        ! Starting longitude
 integer i2                        ! Finishing longitude
 TYPE (XPLEX) dp(i1:i2,km)            ! grid size
 TYPE (XPLEX) dq(i1:i2,km)            ! backward diff of q
 TYPE (XPLEX) d4(i1:i2,km)            ! backward sum:  dp(k)+ dp(k-1) 
 TYPE (XPLEX) df2(i1:i2,km)            ! first guess mismatch
 TYPE (XPLEX) dm(i1:i2,km)            ! monotonic mismatch

! !INPUT/OUTPUT PARAMETERS:
 TYPE (XPLEX) a4(4,i1:i2,km)          ! first guess/steepened

! !LOCAL VARIABLES:
 integer i, k
 TYPE (XPLEX) alfa(i1:i2,km)
 TYPE (XPLEX) f(i1:i2,km)
 TYPE (XPLEX) rat(i1:i2,km)
 TYPE (XPLEX) dg2
...
! !INPUT PARAMETERS:
 ...
 TYPE (XPLEX) tiny               ! A small number to pump up value
 parameter (tiny = xplex(1.d-40,0d0))

! !INPUT/OUTPUT PARAMETERS:
 TYPE (XPLEX) q(im,jfirst-ng:jlast+ng) ! Field to adjust
...
! !LOCAL VARIABLES:
 TYPE (XPLEX) d0, d1, d2
 TYPE (XPLEX) qtmp(jfirst:jlast,im)
...
! !INPUT PARAMETERS:
   ...
   TYPE (XPLEX), intent(in) ::  dp(im,km)       ! pressure thickness

! !INPUT/OUTPUT PARAMETERS:
   TYPE (XPLEX), intent(inout) :: q(im,km,nq)   ! tracer mixing ratio
...
! !INPUT/OUTPUT PARAMETERS:
 TYPE (XPLEX) q(im)              ! Latitude-level field to adjust
 TYPE (XPLEX) qp(im)             ! Second latitude-level field to adjust (usually pole)
...
! !LOCAL VARIABLES:
 ...
 subroutine gmean(im,  jm,  jfirst,  jlast,  q, qmean)
  ...
  integer jfirst, jlast                ! Latitude strip
  TYPE (XPLEX), intent(in):: q(im,jfirst:jlast)              ! 2D field    

  TYPE (XPLEX) qmean
  TYPE (XPLEX) xsum(jfirst:jlast)
  integer i, j
  integer ierror
 ...
 integer, intent(in):: n_adj
 TYPE (XPLEX), intent(in):: tiny
 TYPE (XPLEX), intent(in)::  ak(km+1)
 TYPE (XPLEX), intent(in)::  bk(km+1)
 TYPE (XPLEX), intent(in)::  ps2(im,jfirst-mg:jlast+mg)
 TYPE (XPLEX), intent(in)::   cx(im,jfirst-ng:jlast+ng,km) 
 TYPE (XPLEX), intent(inout)::   pe(im,km+1,jfirst:jlast) 
 logical, intent(in):: ffsl(jfirst-ng:jlast+ng,km)

 TYPE (XPLEX), intent(inout):: ps0(im,jfirst:jlast)
 TYPE (XPLEX), intent(inout):: fx3(im,jfirst:jlast,km)
 TYPE (XPLEX), intent(inout):: fy3(im,jfirst:jlast+mg,km)
 TYPE (XPLEX), intent(inout):: delp(im,jfirst:jlast,km)

! Local
 TYPE (XPLEX) ps(im,jfirst-mg:jlast+mg)
 TYPE (XPLEX) fy(im,jfirst:jlast+mg)
 TYPE (XPLEX) fx(im+1)
 TYPE (XPLEX) dps(0:im)
 TYPE (XPLEX) dpy(im,jfirst-mg:jlast+mg)
 TYPE (XPLEX) er0, er1
 integer i,j,k, it
 TYPE (XPLEX) tmpf, dh
 TYPE (XPLEX) dbk
