! $Id: pbl_mix_mod.f,v 1.4 2010/03/09 15:03:47 daven Exp $
      MODULE PBL_MIX_MOD
!
!******************************************************************************
...
!  (2 ) FPBL        (TYPE (XPLEX) ) : Array for frac # of levels under PBL top
!  (3 ) F_OF_PBL    (TYPE (XPLEX) ) : Array for frac of box (I,J,L) w/in PBL
!  (4 ) F_UNDER_TOP (TYPE (XPLEX) ) : Array for frac of box (I,J,L) under PBL top
!  (5 ) PBL_TOP_hPa (TYPE (XPLEX) ) : Array for PBL top [hPa]
!  (6 ) PBL_TOP_L   (TYPE (XPLEX) ) : Array for PBL top [model levels]
!  (7 ) PBL_TOP_m   (TYPE (XPLEX) ) : Array for PBL top [m]
!  (7 ) PBL_THICK   (TYPE (XPLEX) ) : Array for PBL thickness [hPa]
!
!  Module Routines:
!  ============================================================================
...
!  (8 ) GET_IMIX              : Returns IMIX (adj_group, dkh, 07/08/09) 
!  (9 ) GET_FPBL              : Returns FPBL (adj_group, dkh, 07/08/09) 
!  (10) INIT_PBL_MIX          : Allocates and zeroes all module arrays
!  (11) CLEANUP_PBL_MIX       : Deallocates all module arrays
! 
...
!
      ...
      TYPE (XPLEX),  ALLOCATABLE :: FPBL(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: F_OF_PBL(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: F_UNDER_TOP(:,:,:)
      TYPE (XPLEX),  ALLOCATABLE :: PBL_TOP_hPa(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: PBL_TOP_L(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: PBL_TOP_m(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: PBL_THICK(:,:)
      TYPE (XPLEX),  ALLOCATABLE :: XTRA2(:,:)
...
!
      ...
      USE TIME_MOD,       ONLY : GET_NHMS,GET_NYMD, GET_TAU
...
!$OMP+PRIVATE( I, J, L, P, BLTOP, BLTHIK, LTOP, DELP )
     ...
     &                            ( P(LTOP-1) - P(LTOP) ) 
         ...
         PBL_TOP_L(I,J)   = DBLE( IMIX(I,J) - 1 ) + FPBL(I,J)
...
#     include "CMN_DIAG"       ! ND15
      ...
      TYPE (XPLEX),   INTENT(INOUT) :: TC(IIPAR,JJPAR,LLPAR,NTRC)
      TYPE (XPLEX),   INTENT(IN)    :: TCVV(NTRC)
      
      ! Local variables
      LOGICAL, SAVE           :: FIRST = .TRUE.
      INTEGER                 :: I,  J,  L,     LTOP,    N
      TYPE (XPLEX)                  :: AA, CC, CC_AA, AREA_M2, DTCONV
...
!$OMP+PRIVATE( I, J, L, N, AA, CC, CC_AA ) 
      ...
      DO I = 1, IIPAR
         ...
         DO N = 1, NTRC
            ...
            ! dkh debug
            !IF ( LPRINTFD .AND. I == IFD .AND. J == JFD ) THEN
            !   WRITE(6,*) ' FPBL fwd = ', FPBL(I,J)
            !   WRITE(6,*) ' IMIX fwd = ', IMIX(I,J)
            !ENDIF
...
!
!******************************************************************************
!  Function GET_IMIX returns IMIX  (adj_group, dkh, 07/08/09) 
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) I (INTEGER) : GEOS-CHEM longitude index
...
!
!******************************************************************************
!  Function GET_FPBL returns FPBL  (adj_group, dkh, 07/08/09) 
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) I (INTEGER) : GEOS-CHEM longitude index
